...
DFAState::PredPrediction::PredPrediction( const Ref<SemanticContext>& pred, int alt )
	: pred( pred ) {
...
DFAState::DFAState( int state )
	: DFAState() {
	stateNumber = state;
}

DFAState::DFAState( std::unique_ptr<ATNConfigSet> configs_ )
	: DFAState() {
