...
<http://www.oberlin.edu/~math/faculty/bosch/pbn-page.html> */
        ...
        check{i in 1..m}: sum{t in 1..kr[i]} sr[i,t]
<= n - (kr[i] - 1);
        /* check that the sum of the cluster sizes in each row is valid */

        check{j in 1..n}: sum{t in 1..kc[j]} sc[j,t]
<= m - (kc[j] - 1);
...
s.t. fb{i in 1..m, t in 1..kr[i]-1, j in er[i,t]..lr[i,t]}:
     y[i,t,j]
<= sum{jp in j+sr[i,t]+1..lr[i,t+1]} y[i,t+1,jp];
...
<= j} y[i,t,jp];
        ...
        z[i,j]
<= sum{t in 1..kc[j], ip in ec[j,t]..lc[j,t]:
        i-sc[j,t]+1
<= ip and ip
<= i} x[j,t,ip];
