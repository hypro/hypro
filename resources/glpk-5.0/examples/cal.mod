...
<mao@gnu.org> */
        ...
        param year, integer, >= 0001,
<= 3999, default 2010;

        param first_day{m in 1..12}, integer, >= 0,
<= 6, :=
        time2str(str2time(year & "-" & m & "-01", "%Y-%m-%d"), "%w");

        param days_in_month{m in 1..12}, integer, >= 28,
<= 31, :=
        (str2time(year + (if m
< 12 then 0 else 1) & "-" &
        (if m
< 12 then m+1 else 1) & "-01", "%Y-%m-%d") -
        ...
        if 1
<= foo[m,k,d] and foo[m,k,d]
<= days_in_month[m] then
        foo[m,k,d];
