...
static int str2int( const char* s, int* x ) { /* convert string to integer */
	long t;
	char* endptr;
...
static int str2num( const char* s, double* x ) { /* convert string to floating point */
	double t;
	char* endptr;
	t = strtod( s, &endptr );
	if ( *endptr != '\0' )
...
static void print_help( const char* my_name ) { /* print help information */
...
static int parse_cmdline( struct csa* csa, int argc, char* argv[] ) { /* parse command-line parameters */
	int k;
...
{ /* stand-alone LP/MIP solver */
	...
	err2 : {
		xprintf( "MathProg model processing error\n" );
		ret = EXIT_FAILURE;
		goto done;
	}
...
if ( csa->out_ranges != NULL ) {
	if ( csa->solution == SOL_BASIC ) {
		if ( glp_get_status( csa->prob ) == GLP_OPT ) {
			...
			ranges : {
				ret = glp_print_ranges( csa->prob, 0, NULL, 0,
										csa->out_ranges );
				if ( ret != 0 ) {
					xprintf(
