...
<vuk@cs.elte.hu> */
        ...
        */
        ...
        set TADJ := setof{(r, s) in TIME cross TIME: r
< s}(r, s) diff
        setof{(t, u, v) in TIME cross TIME cross TIME: t
< u and u
< v}(t, v);
        ...
        if p
<> 'Demand' or d = 'Up' then sum{(u, v) in TADJ: v = t} BOUND[p, u, d]
        else BOUND[p, t, 'Up'];
        ...
        var x{(t, p) in TIME cross PLANTS},
<= BOUND[p, t, 'Up'], >= BOUND[p, t, 'Down'];
        /* The schedule, dimension is MW */

        s.t. kirchhoff{t in TIME: t
<> '00:00'}: sum{p in PLANTS} x[t, p] = 0;
