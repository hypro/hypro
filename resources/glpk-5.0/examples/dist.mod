# DIST, a product distribution model
        ...
        param ds 'demand shares' {prd,whse} >= 0.0,
<= 1.0;
...
< huge and
        ...
        not (msr[d,w] and sum {p in prd} 1000*dem[p,w]/cpp[p]
< dsr[d]) };
        ...
        rtmin
<= sum {p in prd, f in fact}
        (pt[p,f] * Rprd[p,f]) / (dp[f] * hd[f])
<= rtmax;
        ...
        otmin
<= sum {p in prd, f in fact} pt[p,f] * Oprd[p,f]
<= otmax;
