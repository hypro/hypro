...
int min_st_cut( int nn, int ne, const int beg[/*1+ne*/],
	...
	while ( p <= q ) { /* pick next node, which is reachable from the source node and
...
int min_cut_sw( int nn, int ne, const int beg[/*1+ne*/],
	...
	while ( NV > 1 ) { /* build the set of edges of the auxiliary network */
		NE = 0;
		...
		for ( I = 1; I <= NV; I++ )
			SUM[I] = 0.0;
		for ( I = 1; I <= NV; I++ ) { /* DEG is number of single edges, which connects I-th
									   * supernode and some J-th supernode, where I < J */
			DEG = 0;
			/* walk thru nodes that belong to I-th supernode */
			for ( i = HEAD[I]; i != 0; i = NEXT[i] ) { /* i-th node belongs to I-th supernode */
				/* walk through edges with beg[k] = i */
				for ( k = head1[i]; k != 0; k = next1[k] ) {
					j = end[k];
