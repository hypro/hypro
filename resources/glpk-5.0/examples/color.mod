...
<mao@gnu.org> */
        ...
        nodes, E
<= V x V is a set of arcs, the Graph Coloring Problem is to
        ...
        (  /* compute z[i,0] */
        min{c in 1..z[i,1]}
        (  if not exists{j in V: j
< i and (i,j) in EE} z[j,0] = c then
        c
        else
        z[i,1] + 1
        )
...
< i and (i,j) in EE} z[j,0]
        ...
        s.t. arc{(i,j) in E, c in 1..nc}: x[i,c] + x[j,c]
<= u[c];
