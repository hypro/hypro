...
struct CPXENV { /* environment block */
	CPXLP* list;
...
static const struct errstring errstring[] =
	  {
			...
			{ CPXERR_BAD_STATUS,
			  "Invalid status entry %d for basis specificat"
			  "ion" },
			{ CPXERR_COL_INDEX_RANGE, "Column index %d out of range" },
			{ CPXERR_COUNT_RANGE,
			  "Count entry %d negative or larger than allo"
			  "wed" },
...
int CPXchgcoeflist( CPXENV* env, CPXLP* lp, int numcoefs,
	...
	errcode = 0;
	invalidate( lp );
	if ( rcnt <= ccnt ) { /* change the matrix by rows */
	...
	} else { /* change the matrix by columns */
...
int CPXpivotin( CPXENV* env, CPXLP* lp, const int rlist[], int rlen ) {
	...
	for ( i = 0; i < rlen; i++ ) {
		if ( glp_get_row_type( lp->prob, rlist[i] + 1 ) != GLP_FX ) {
			if ( glp_get_row_stat( lp->prob, rlist[i] + 1 ) != GLP_BS ) { /* not implemented yet */
				break;
			}
		}
	}
...
int CPXpivotout( CPXENV* env, CPXLP* lp, const int clist[], int clen ) {
	...
	errcode = 0;
	for ( j = 0; j < clen; j++ ) {
		if ( glp_get_col_stat( lp->prob, clist[j] + 1 ) == GLP_BS ) { /* not implemented yet */
			break;
		}
	}
done:
...
static void getfiletype( const char* filename, char type[3 + 1] ) { /* determine filetype from filename */
	int beg, end;
