...
struct glp_cov { /* cover cut generator working area */
	int n;
...
struct bnd { /* simple or variable bound */
...
static void add_term( FVS* v, int j, double a ) {
	xassert( 1 <= j && j <= v->n );
	xassert( a != 0 );
	if ( v->vec[j] == 0 ) { /* create j-th component */
		v->nnz++;
		xassert( v->nnz <= v->n );
		v->ind[v->nnz] = j;
	}
	/* perform addition */
	v->vec[j] += a;
	if ( fabs( v->vec[j] ) < 1e-9 * ( 1 + fabs( a ) ) ) { /* remove j-th component */
		v->vec[j] = DBL_MIN;
	}
	return;
}
...
static int build_ks( struct csa* csa, int n, int ind[], double a[],
	...
	for ( j = 1; j <= n; j++ ) {					/* process term a[j]*x[j] */
		k = ind[j];									/* original number of x[j] in mip */
		if ( glp_get_col_kind( P, k ) == GLP_BV ) { /* x[j] is a binary variable */
			/* include its term into resulting inequality */
			add_term( v, k, a[j] );
		} else if ( a[j] > 0 ) {		/* substitute x[j] by its lower bound */
			if ( l[k].b == -DBL_MAX ) { /* x[j] has no lower bound */
				n = -1;
				goto skip;
			} else if ( l[k].z == 0 ) { /* x[j] has simple lower bound */
				*b -= a[j] * l[k].b;
			} else { /* x[j] has variable lower bound (a * z + b) */
				add_term( v, l[k].z, a[j] * l[k].a );
				*b -= a[j] * l[k].b;
			}
		} else							/* a[j] < 0 */
		{								/* substitute x[j] by its upper bound */
			if ( u[k].b == +DBL_MAX ) { /* x[j] has no upper bound */
				n = -1;
				goto skip;
			} else if ( u[k].z == 0 ) { /* x[j] has simple upper bound */
				*b -= a[j] * u[k].b;
			} else { /* x[j] has variable upper bound (a * z + b) */
...
static void process_ineq( struct csa* csa, int n, int ind[], double a[],
	...
	if ( n <= 1 ) { /* uninteresting inequality (in principle, such inequalities
					 * should be removed by the preprocessor) */
		goto done;
	}
	if ( !can_be_active( n, a, b ) ) { /* inequality is redundant (i.e. cannot be active) */
		goto done;
	}
	if ( is_sos_ineq( n, a, b ) ) { /* packing (SOS) inequality is useless for generating cover
...
glp_cov* glp_cov_init( glp_prob* P ) { /* create and initialize cover cut generator */
	glp_cov* cov;
	struct csa csa;
	int i, k, len, *ind;
	double rhs, *val;
	...
	if ( csa.set->m == 0 ) { /* the set is empty */
		xprintf( "No 0-1 knapsack inequalities detected\n" );
		cov = NULL;
		glp_delete_prob( csa.set );
	} else { /* create the cover cut generator working area */
...
static double simple_cover( int n, const double a[], double b, const double x[], char z[] ) {
	int j, *aa, bb, *cc;
	...
	if ( solve_ks( n, aa, bb, cc, z ) == INT_MIN ) { /* no cover exists */
		s = DBL_MAX;
		goto skip;
	}
	/* determine z[j] = 1 - z'[j] */
	...
	if ( !( s >= b + eps ) ) { /* no cover found within a precision req'd */
		s = DBL_MAX;
		goto skip;
	}
...
void glp_cov_gen1( glp_prob* P, glp_cov* cov, glp_prob* pool ) { /* generate locally valid simple cover cuts */
	...
	for ( i = 1; i <= cov->set->m; i++ ) { /* retrieve 0-1 knapsack inequality */
		...
		if ( zeta > 0.95 ) { /* no violation or insufficient violation; see (2) */
			continue;
		}
...
void glp_cov_free( glp_cov* cov ) { /* delete cover cut generator workspace */
	xassert( cov != NULL );
