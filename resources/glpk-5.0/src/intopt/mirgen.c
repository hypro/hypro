...
struct glp_mir { /* MIR cut generator working area */
	/*--------------------------------------------------------------*/
...
static void set_row_attrib( glp_prob* mip, glp_mir* mir ) { /* set global row attributes */
	int m = mir->m;
	int k;
	for ( k = 1; k <= m; k++ ) {
		...
		switch ( row->type ) {
			case GLP_FR:
				mir->lb[k] = -DBL_MAX, mir->ub[k] = +DBL_MAX;
				break;
			case GLP_LO:
				mir->lb[k] = row->lb, mir->ub[k] = +DBL_MAX;
				break;
			case GLP_UP:
				mir->lb[k] = -DBL_MAX, mir->ub[k] = row->ub;
				break;
			case GLP_DB:
				mir->lb[k] = row->lb, mir->ub[k] = row->ub;
				break;
			case GLP_FX:
...
static void set_col_attrib( glp_prob* mip, glp_mir* mir ) { /* set global column attributes */
	int m = mir->m;
	int n = mir->n;
	int k;
	for ( k = m + 1; k <= m + n; k++ ) {
		...
		switch ( col->type ) {
			case GLP_FR:
				...
				break;
			case GLP_LO:
				mir->lb[k] = col->lb, mir->ub[k] = +DBL_MAX;
				break;
			case GLP_UP:
				mir->lb[k] = -DBL_MAX, mir->ub[k] = col->ub;
				break;
			case GLP_DB:
				mir->lb[k] = col->lb, mir->ub[k] = col->ub;
				break;
			case GLP_FX:
...
static void set_var_bounds( glp_prob* mip, glp_mir* mir ) { /* set variable bounds */
	int m = mir->m;
	GLPAIJ* aij;
	int i, k1, k2;
	double a1, a2;
	for ( i = 1; i <= m; i++ ) { /* we need the row to be '>= 0' or '<= 0' */
		...
		if ( !mir->isint[k1] && mir->isint[k2] )
			...
			k2 = k1, a2 = a1;
			k1 = m + aij->col->j, a1 = aij->val;
		} else { /* both terms are either continuous or integer */
			continue;
		}
		...
		if ( a1 > 0.0 ) {			   /* x1 >= - (a2 / a1) * x2 */
			if ( mir->vlb[k1] == 0 ) { /* set variable lower bound for x1 */
				mir->lb[k1] = -a2 / a1;
				mir->vlb[k1] = k2;
		...
		{							   /* x1 <= - (a2 / a1) * x2 */
			if ( mir->vub[k1] == 0 ) { /* set variable upper bound for x1 */
				mir->ub[k1] = -a2 / a1;
				mir->vub[k1] = k2;
				/* the row should not be used */
				mir->skip[i] = 1;
...
static void mark_useless_rows( glp_prob* mip, glp_mir* mir ) { /* mark rows which should not be used */
	int m = mir->m;
	GLPAIJ* aij;
	int i, k, nv;
	for ( i = 1; i <= m; i++ ) { /* free rows should not be used */
...
glp_mir* glp_mir_init( glp_prob* mip ) { /* create and initialize MIR cut generator */
	int m = mip->m;
	int n = mip->n;
	glp_mir* mir;
#if MIR_DEBUG
...
static void get_current_point( glp_prob* mip, glp_mir* mir ) { /* obtain current point */
	int m = mir->m;
	int n = mir->n;
	int k;
	for ( k = 1; k <= m; k++ )
...
#if MIR_DEBUG
static void check_current_point( glp_mir* mir ) { /* check current point */
	int m = mir->m;
	int n = mir->n;
	int k, kk;
	double lb, ub, eps;
	for ( k = 1; k <= m + n; k++ ) { /* determine lower bound */
		lb = mir->lb[k];
		kk = mir->vlb[k];
		if ( kk != 0 ) {
			xassert( lb != -DBL_MAX );
...
static void initial_agg_row( glp_prob* mip, glp_mir* mir, int i ) { /* use original i-th row as initial aggregated constraint */
	int m = mir->m;
	GLPAIJ* aij;
	xassert( 1 <= i && i <= m );
	xassert( !mir->skip[i] );
...
#if MIR_DEBUG
static void check_agg_row( glp_mir* mir ) { /* check aggregated constraint */
	int m = mir->m;
	int n = mir->n;
	int j, k;
	double r, big;
...
static void subst_fixed_vars( glp_mir* mir ) { /* substitute fixed variables into aggregated constraint */
	int m = mir->m;
	int n = mir->n;
	int j, k;
	for ( j = 1; j <= mir->agg_vec->nnz; j++ ) {
		...
		xassert( 1 <= k && k <= m + n );
		if ( mir->vlb[k] == 0 && mir->vub[k] == 0 &&
			 mir->lb[k] == mir->ub[k] ) { /* x[k] is fixed */
...
static void bound_subst_heur( glp_mir* mir ) { /* bound substitution heuristic */
	int m = mir->m;
	int n = mir->n;
	int j, k, kk;
	double d1, d2;
...
static void build_mod_row( glp_mir* mir ) { /* substitute bounds and build modified constraint */
	int m = mir->m;
	int n = mir->n;
	int j, jj, k, kk;
	...
	for ( j = mir->mod_vec->nnz; j >= 1; j-- ) {
		...
		xassert( 1 <= k && k <= m + n );
		if ( mir->isint[k] ) continue; /* skip integer variable */
		if ( mir->subst[k] == 'L' ) {  /* x[k] = (lower bound) + x'[k] */
			xassert( mir->lb[k] != -DBL_MAX );
			kk = mir->vlb[k];
			if ( kk == 0 ) { /* x[k] = lb[k] + x'[k] */
				mir->mod_rhs -= mir->mod_vec->val[j] * mir->lb[k];
			} else { /* x[k] = lb[k] * x[kk] + x'[k] */
				xassert( mir->isint[kk] );
		...
		} else if ( mir->subst[k] == 'U' ) { /* x[k] = (upper bound) - x'[k] */
			xassert( mir->ub[k] != +DBL_MAX );
			kk = mir->vub[k];
			if ( kk == 0 ) { /* x[k] = ub[k] - x'[k] */
				mir->mod_rhs -= mir->mod_vec->val[j] * mir->ub[k];
			} else { /* x[k] = ub[k] * x[kk] - x'[k] */
				xassert( mir->isint[kk] );
				jj = mir->mod_vec->pos[kk];
				if ( jj == 0 ) {
					spv_set_vj( mir->mod_vec, kk, 1.0 );
	...
	for ( j = 1; j <= mir->mod_vec->nnz; j++ ) {
		...
		if ( fabs( mir->lb[k] ) <= fabs( mir->ub[k] ) ) { /* x[k] = lb[k] + x'[k] */
			mir->subst[k] = 'L';
			mir->mod_rhs -= mir->mod_vec->val[j] * mir->lb[k];
		} else { /* x[k] = ub[k] - x'[k] */
			mir->subst[k] = 'U';
...
#if MIR_DEBUG
static void check_mod_row( glp_mir* mir ) { /* check modified constraint */
	int m = mir->m;
	int n = mir->n;
	int j, k, kk;
	double r, big, x;
	...
	for ( j = 1; j <= mir->mod_vec->nnz; j++ ) {
		k = mir->mod_vec->ind[j];
		xassert( 1 <= k && k <= m + n );
		if ( mir->subst[k] == 'L' ) { /* x'[k] = x[k] - (lower bound) */
			xassert( mir->lb[k] != -DBL_MAX );
			kk = mir->vlb[k];
			if ( kk == 0 )
				x = mir->x[k] - mir->lb[k];
...
static double cmir_sep( const int n, const double a[], const double b,
	...
	d_try[2] = delta / 4.0;
	d_try[3] = delta / 8.0;
	for ( j = 1; j <= 3; j++ ) { /* construct c-MIR inequality */
	...
	nv = 0;
	for ( j = 1; j <= n; j++ ) { /* if x[j] is close to its bounds, skip it */
		eps = 1e-9 * ( 1.0 + fabs( u[j] ) );
		if ( x[j] < eps || x[j] > u[j] - eps ) continue;
...
static double generate( glp_mir* mir ) { /* try to generate violated c-MIR cut for modified constraint */
	int m = mir->m;
	int n = mir->n;
	int j, k, kk, nint;
...
#if MIR_DEBUG
static void check_raw_cut( glp_mir* mir, double r_best ) { /* check raw cut before back bound substitution */
	int m = mir->m;
	int n = mir->n;
	int j, k, kk;
	double r, big, x;
...
static void back_subst( glp_mir* mir ) { /* back substitution of original bounds */
	int m = mir->m;
	int n = mir->n;
	int j, jj, k, kk;
	...
	for ( j = 1; j <= mir->cut_vec->nnz; j++ ) {
		...
		if ( mir->subst[k] == 'L' ) {	/* x'[k] = x[k] - lb[k] */
			xassert( mir->lb[k] != -DBL_MAX );
			xassert( mir->vlb[k] == 0 );
			mir->cut_rhs += mir->cut_vec->val[j] * mir->lb[k];
		} else if ( mir->subst[k] == 'U' ) { /* x'[k] = ub[k] - x[k] */
			xassert( mir->ub[k] != +DBL_MAX );
			xassert( mir->vub[k] == 0 );
	...
	for ( j = 1; j <= mir->cut_vec->nnz; j++ ) {
		...
		if ( mir->subst[k] == 'L' ) {  /* x'[k] = x[k] - (lower bound) */
			xassert( mir->lb[k] != -DBL_MAX );
			kk = mir->vlb[k];
			if ( kk == 0 ) { /* x'[k] = x[k] - lb[k] */
				mir->cut_rhs += mir->cut_vec->val[j] * mir->lb[k];
			} else { /* x'[k] = x[k] - lb[k] * x[kk] */
...
#if MIR_DEBUG
static void check_cut_row( glp_mir* mir, double r_best ) { /* check the cut after back bound substitution or elimination of
															  auxiliary variables */
	int m = mir->m;
	int n = mir->n;
	int j, k;
...
static void subst_aux_vars( glp_prob* mip, glp_mir* mir ) { /* final substitution to eliminate auxiliary variables */
	int m = mir->m;
	int n = mir->n;
	GLPAIJ* aij;
	int j, k, kk, jj;
...
static void add_cut( glp_mir* mir, glp_prob* pool ) { /* add constructed cut inequality to the cut pool */
	int m = mir->m;
	int n = mir->n;
	int j, k, len;
...
int glp_mir_gen( glp_prob* mip, glp_mir* mir, glp_prob* pool ) { /* main routine to generate MIR cuts */
	int m = mir->m;
	int n = mir->n;
	int i, nnn = 0;
	double r_best;
	...
	loop:;
		...
		if ( r_best == 0.0 ) {				/* failure */
			if ( mir->agg_cnt < MAXAGGR ) { /* try to aggregate another row */
#if 0										/* 29/II-2016 by Chris */
