...
static void add_edge( CFG* G, int v, int w ) { /* add clique of size 2 */
	DMP* pool = G->pool;
	int nv = G->nv;
	CFGVLE** vptr = G->vptr;
	CFGVLE* vle;
...
void cfg_add_clique( CFG* G, int size, const int ind[] ) {
	...
	for ( k = 1; k <= size; k++ ) {
		j = ind[k];
		if ( j > 0 ) { /* vertex corresponds to x[j] */
			xassert( 1 <= j && j <= n );
			if ( pos[j] == 0 ) { /* no such vertex exists; add it */
				v = pos[j] = ++( G->nv );
				xassert( v <= nv_max );
				ref[v] = j;
				vptr[v] = NULL;
				cptr[v] = NULL;
				if ( neg[j] != 0 ) { /* now both vertices for x[j] and (1 - x[j]) exist */
					add_edge( G, v, neg[j] );
				}
			}
		} else { /* vertex corresponds to (1 - x[j]) */
			j = -j;
			xassert( 1 <= j && j <= n );
			if ( neg[j] == 0 ) { /* no such vertex exists; add it */
				v = neg[j] = ++( G->nv );
				xassert( v <= nv_max );
				ref[v] = j;
				vptr[v] = NULL;
				cptr[v] = NULL;
				if ( pos[j] != 0 ) { /* now both vertices for x[j] and (1 - x[j]) exist */
					add_edge( G, v, pos[j] );
				}
			}
		}
...
int cfg_get_adjacent( CFG* G, int v, int ind[] ) {
	...
	for ( cle = cptr[v]; cle != NULL; cle = cle->next ) { /* walk thru the list of clique vertices */
...
static int intersection( int d_len, int d_ind[], int d_pos[], int len,
						 const int ind[] ) { /* compute intersection D := D inter W, where W is some specified
											  * set of vertices */
	int k, t, v, new_len;
	/* walk thru vertices in W and mark vertices in D */
	for ( t = 1; t <= len; t++ ) { /* v in W */
		v = ind[t];
	...
	new_len = 0;
	for ( k = 1; k <= d_len; k++ ) { /* v in D */
		v = d_ind[k];
		if ( v < 0 ) { /* v is marked; keep it */
			v = -v;
			new_len++;
			d_ind[new_len] = v;
			d_pos[v] = new_len;
		} else { /* v is not marked; remove it */
			d_pos[v] = 0;
		}
	}
	return new_len;
...
void cfg_check_clique( CFG* G, int c_len, const int c_ind[] ) {
	...
	xassert( c_len >= 0 );
	for ( k = 1; k <= c_len; k++ ) { /* get clique vertex v */
		v = c_ind[k];
		xassert( 1 <= v && v <= nv );
