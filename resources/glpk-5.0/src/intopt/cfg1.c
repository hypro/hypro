...
static int CDECL fcmp( const void* e1, const void* e2 ) { /* auxiliary routine called from qsort */
...
static void analyze_ineq( glp_prob* P, CFG* G, int len, int ind[],
						  double val[], double rhs, struct term t[] ) { /* analyze inequality constraint (6) */
	...
	new_len = 0;
	for ( k = 1; k <= len; k++ ) { /* get index of variable x[j] */
		j = ind[k];
		if ( is_binary( j ) ) { /* x[j] remains in relaxed constraint */
			new_len++;
			ind[new_len] = j;
			val[new_len] = val[k];
		} else if ( val[k] > 0.0 ) { /* eliminate non-binary x[j] in case a[j] > 0 */
			/* b := b - a[j] * l[j]; see (8) */
			type = P->col[j]->type;
			if ( type == GLP_FR || type == GLP_UP ) { /* x[j] has no lower bound */
				goto done;
			}
		...
		{	   /* eliminate non-binary x[j] in case a[j] < 0 */
			/* b := b - a[j] * u[j]; see (8) */
			type = P->col[j]->type;
			if ( type == GLP_FR || type == GLP_LO ) { /* x[j] has no upper bound */
				goto done;
			}
	...
	if ( len <= 1 ) { /* at least two terms are needed */
		goto done;
	}
	/* make all constraint coefficients positive; see (10) */
	for ( k = 1; k <= len; k++ ) {
		if ( val[k] < 0.0 ) { /* a[j] < 0; substitute x[j] = 1 - x'[j], where x'[j] is
	...
	xassert( p != 0 && q != 0 && p != q );
	/* check condition (14) */
	if ( val[p] + val[q] > rhs ) { /* all z[j] define a clique in the conflict graph */
	...
	xassert( p != 0 && q != 0 && p != q );
	/* check condition (18) */
	if ( val[p] + val[q] <= rhs ) { /* no valid edge inequalities exist */
		goto done;
	}
	/*** third case ***/
	xassert( len >= 3 );
...
CFG* cfg_build_graph( void* P_ ) {
	...
	for ( i = 1; i <= m; i++ ) {
		type = P->row[i]->type;
		if ( type == GLP_LO || type == GLP_DB || type == GLP_FX ) { /* i-th row has lower bound */
		...
		if ( type == GLP_UP || type == GLP_DB || type == GLP_FX ) { /* i-th row has upper bound */
...
static void build_subgraph( struct csa* csa ) { /* build induced subgraph */
	glp_prob* P = csa->P;
	int n = P->n;
	CFG* G = csa->G;
	int* ind = csa->ind;
	...
	for ( v = 1; v <= nv; v++ ) { /* determine value of binary variable z[j] that corresponds to
								   * vertex v */
		j = ref[v];
		xassert( 1 <= j && j <= n );
		if ( pos[j] == v ) { /* z[j] = x[j], where x[j] is original variable */
			z = P->col[j]->prim;
		} else if ( neg[j] == v ) { /* z[j] = 1 - x[j], where x[j] is original variable */
			z = 1.0 - P->col[j]->prim;
		} else
			xassert( v != v );
		...
		for ( k = 1; k <= len; k++ ) { /* there is an edge (v,w) in the conflict graph */
			w = ind[k];
			xassert( w != v );
...
static int sub_adjacent( struct csa* csa, int i, int adj[] ) { /* retrieve vertices of induced subgraph adjacent to specified
																* vertex */
	CFG* G = csa->G;
	int nv = G->nv;
	int* ind = csa->ind;
	...
	len = 0;
	for ( k = 1; k <= len1; k++ ) { /* there exists edge (v, w) in original graph */
		w = ind[k];
		xassert( 1 <= w && w <= nv && w != v );
		j = vtoi[w];
		if ( j != 0 ) { /* vertex w is vertex j in induced subgraph */
...
static int find_clique( struct csa* csa, int c_ind[] ) { /* find maximum weight clique in induced subgraph with exact
	...
	memset( a, 0, nb );
	for ( p = 1; p <= nn; p++ ) { /* retrieve vertices adjacent to vertex p */
		len = sub_adjacent( csa, p, ind );
		for ( k = 1; k <= len; k++ ) { /* there exists edge (p, q) in induced subgraph */
			q = ind[k];
	...
	iwt = ind;
	for ( i = 1; i <= nn; i++ ) { /* it is assumed that 0 <= wgt[i] <= 1 */
...
static int func( void* info, int i, int ind[] ) { /* auxiliary routine used by routine find_clique1 */
	struct csa* csa = info;
	xassert( 1 <= i && i <= csa->nn );
	return sub_adjacent( csa, i, ind );
}

static int find_clique1( struct csa* csa, int c_ind[] ) { /* find maximum weight clique in induced subgraph with greedy
														   * heuristic */
	int nn = csa->nn;
	double* wgt = csa->wgt;
	int len;
...
int cfg_find_clique( void* P, CFG* G, int ind[], double* sum_ ) {
	...
	} else { /* induced subgraph is large; use greedy heuristic */
