...
int glp_ios_next_node( glp_tree* tree, int p ) {
	IOSNPD* node;
	if ( p == 0 ) { /* obtain pointer to the first active subproblem */
		node = tree->head;
	} else { /* obtain pointer to the specified subproblem */
...
int glp_ios_prev_node( glp_tree* tree, int p ) {
	IOSNPD* node;
	if ( p == 0 ) { /* obtain pointer to the last active subproblem */
		node = tree->tail;
	} else { /* obtain pointer to the specified subproblem */
		if ( !( 1 <= p && p <= tree->nslots ) )
		err:
...
int glp_ios_pool_size( glp_tree* tree ) { /* determine current size of the cut pool */
...
void glp_ios_del_row( glp_tree* tree, int i ) { /* remove row (constraint) from the cut pool */
	if ( tree->reason != GLP_ICUTGEN )
...
void glp_ios_clear_pool( glp_tree* tree ) { /* remove all rows (constraints) from the cut pool */
	if ( tree->reason != GLP_ICUTGEN )
...
int glp_ios_heur_sol( glp_tree* tree, const double x[] ) {
	...
	for ( j = 1; j <= n; j++ ) {
		...
		if ( col->kind == GLP_IV ) { /* provided value must be integral */
