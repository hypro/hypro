...
static int lpx_eval_tab_row( glp_prob* lp, int k, int ind[],
							 double val[] ) { /* compute row of the simplex tableau */
...
void ios_freeze_node( glp_tree* tree ) {
	...
	node = tree->curr;
	xassert( node != NULL );
	if ( node->up == NULL ) { /* freeze the root subproblem */
		int k;
		xassert( node->p == 1 );
	...
	} else { /* freeze non-root subproblem */
...
loop: /* recursive deletion starts here */
	...
	node = temp;
	if ( node != NULL ) { /* the parent subproblem exists; decrease the number of its
							 child subproblems */
		xassert( node->count > 0 );
		node->count--;
...
void ios_eval_degrad( glp_tree* tree, int j, double* dn, double* up ) {
	...
	for ( kase = -1; kase <= +1; kase += 2 ) { /* for down-branch we introduce new upper bound floor(beta)
...
double ios_round_bound( glp_tree* tree, double bound ) {
	...
	for ( j = 1; j <= n; j++ ) {
		...
		if ( col->type == GLP_FX ) { /* fixed variable */
			s += col->coef * col->prim;
		} else { /* non-fixed variable */
...
double ios_relative_gap( glp_tree* tree ) {
	...
	} else { /* no integer feasible solution has been found yet */
		gap = DBL_MAX;
	}
	return gap;
}
...
int ios_solve_node( glp_tree* tree ) {
	...
	if ( ret == GLP_EFAIL ) { /* retry with a new basis */
		glp_adv_basis( mip, 0 );
		ret = glp_simplex( mip, &parm );
	}
#endif
...
IOSPOOL* ios_create_pool( glp_tree* tree ) { /* create cut pool */
	IOSPOOL* pool;
...
int ios_add_row( glp_tree* tree, IOSPOOL* pool,
				 ...
				 const double val[], int type, double rhs ) { /* add row (constraint) to the cut pool */
	int i;
...
#ifdef NEW_LOCAL							   /* 02/II-2018 */
IOSCUT* ios_find_row( IOSPOOL* pool, int i ) { /* find row (constraint) in the cut pool */
	xassert( 0 );
}
#else
IOSCUT* ios_find_row( IOSPOOL* pool, int i ) { /* find row (constraint) in the cut pool */
...
#ifdef NEW_LOCAL										   /* 02/II-2018 */
void ios_del_row( glp_tree* tree, IOSPOOL* pool, int i ) { /* remove row (constraint) from the cut pool */
	xassert( 0 );
}
#else
void ios_del_row( glp_tree* tree, IOSPOOL* pool, int i ) { /* remove row (constraint) from the cut pool */
...
#ifdef NEW_LOCAL									   /* 02/II-2018 */
void ios_clear_pool( glp_tree* tree, IOSPOOL* pool ) { /* remove all rows (constraints) from the cut pool */
	if ( pool->m > 0 ) {
		int i, *num;
...
#ifdef NEW_LOCAL										/* 02/II-2018 */
void ios_delete_pool( glp_tree* tree, IOSPOOL* pool ) { /* delete cut pool */
	xassert( pool != NULL );
...
void ios_process_sol( glp_tree* T ) { /* process integer feasible solution just found */
	if ( T->npp != NULL ) {			  /* postprocess solution from transformed mip */
