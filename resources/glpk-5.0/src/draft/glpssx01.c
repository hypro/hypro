...
static int basis_col( void* info, int j, int ind[], mpq_t val[] ) { /* this auxiliary routine provides row indices and numeric values
...
void ssx_chuzc( SSX* ssx ) {
	...
	for ( j = 1; j <= n; j++ ) {
		...
		if ( ( stat[k] == SSX_NF || stat[k] == SSX_NL ) && s < 0 ||
			 ( stat[k] == SSX_NF || stat[k] == SSX_NU ) && s > 0 ) { /* reduced cost of xN[j] indicates possible improving of
...
void ssx_chuzr( SSX* ssx ) {
	...
	for ( i = 1; i <= m; i++ ) {
		...
		if ( s < 0 ) {	  /* xB[i] decreases */
			k = Q_col[i]; /* x[k] = xB[i] */
			t = type[k];
			if ( t == SSX_LO || t == SSX_DB || t == SSX_FX ) { /* xB[i] has finite lower bound */
			...
			k = Q_col[i];	  /* x[k] = xB[i] */
			t = type[k];
			if ( t == SSX_UP || t == SSX_DB || t == SSX_FX ) { /* xB[i] has finite upper bound */
