...
struct info { /* estimated cut efficiency */
	IOSCUT* cut;
...
void ios_process_cuts( glp_tree* T ) {
	...
	for ( k = 1; k <= pool->m; k++ ) {
		...
		} else if ( ret == 1 ) { /* the constraint is not violated at the current point */
			info[k].eff = info[k].deg = 0.0;
		} else if ( ret == 2 ) { /* no dual feasible adjacent basis exists */
			info[k].eff = 1.0;
			info[k].deg = DBL_MAX;
		} else
			xassert( ret != ret );
