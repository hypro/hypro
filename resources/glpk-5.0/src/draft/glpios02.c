...
static int check_row_bounds( const struct f_info* f, double* L_,
	...
	if ( L != -DBL_MAX ) {
		...
		if ( LL > L - eps ) { /* it cannot be active, so remove it */
			*L_ = -DBL_MAX;
		}
	}
	...
	if ( U != +DBL_MAX ) {
		...
		if ( UU < U + eps ) { /* it cannot be active, so remove it */
			*U_ = +DBL_MAX;
		}
	}
done:
...
static int check_col_bounds( const struct f_info* f, int n,
	...
	if ( ll != -DBL_MAX ) {
		...
		if ( lj < ll - eps ) { /* it cannot be active, so tighten it */
			lj = ll;
		}
	}
	/* check if the original upper bound is redundant */
	if ( uu != +DBL_MAX ) {
		...
		if ( uj > uu + eps ) { /* it cannot be active, so tighten it */
			uj = uu;
		}
	}
...
static int basic_preprocessing( glp_prob* mip, double L[], double U[],
	...
	while ( size > 0 ) { /* get a next row from the list */
		...
		if ( check_row_bounds( &f, &L[i], &U[i] ) ) { /* the feasible region is empty */
			ret = 1;
			goto done;
		}
		...
		for ( k = 1; k <= len; k++ ) {
			...
			if ( check_col_bounds( &f, len, val, L[i], U[i], lb, ub,
								   flag, k, &ll, &uu ) ) { /* the feasible region is empty */
				ret = 1;
				goto done;
			}
...
int ios_preprocess_node( glp_tree* tree, int max_pass ) {
	...
	for ( i = 1; i <= m; i++ ) { /* consider only non-active rows to keep dual feasibility */
