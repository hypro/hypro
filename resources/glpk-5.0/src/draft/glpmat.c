...
int check_pattern( int m, int n, int A_ptr[], int A_ind[] ) {
	...
	for ( i = 1; i <= m; i++ ) { /* check pattern of row i */
...
int* adat_symbolic( int m, int n, int P_per[], int A_ptr[], int A_ind[],
	...
	S_ptr[1] = 1;
	for ( ii = 1; ii <= m; ii++ ) { /* compute pattern of ii-th row of S */
		len = 0;
...
void amd_order1( int n, int A_ptr[], int A_ind[], int P_per[] ) { /* approximate minimum degree ordering (AMD) */
	int k, ret;
...
void symamd_ord( int n, int A_ptr[], int A_ind[], int P_per[] ) { /* approximate minimum degree ordering (SYMAMD) */
	int k, ok;
...
int* chol_symbolic( int n, int A_ptr[], int A_ind[], int U_ptr[] ) {
	...
	U_ptr[1] = 1;
	for ( k = 1; k <= n; k++ ) { /* compute the pattern of k-th row of U, which is the union of
		...
		for ( i = head[k]; i != 0; i = next[i] ) { /* (ind) := (ind) union (i-th row of U) */
...
int chol_numeric( int n,
	...
	for ( k = 1; k <= n; k++ ) { /* transform k-th row of U */
		ukk = U_diag[k];
		if ( ukk > 0.0 )
