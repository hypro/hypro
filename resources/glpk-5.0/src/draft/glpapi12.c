...
static int b_col( void* info, int j, int ind[], double val[] ) {
	...
	if ( k <= m ) { /* x[k] is auxiliary variable */
		len = 1;
		ind[1] = k;
		val[1] = 1.0;
	} else { /* x[k] is structural variable */
		len = 0;
...
int glp_factorize( glp_prob* lp ) {
	...
	for ( k = 1; k <= m + n; k++ ) {
		...
		if ( stat == GLP_BS ) {
			j++;
			if ( j > m ) { /* too many basic variables */
				ret = GLP_EBADB;
				goto fini;
			}
			head[j] = k;
	...
	if ( j < m ) { /* too few basic variables */
		ret = GLP_EBADB;
		goto fini;
	}
...
int glp_eval_tab_row( glp_prob* lp, int k, int ind[], double val[] ) {
	...
	if ( !( 1 <= k && k <= m + n ) )
		xerror( "glp_eval_tab_row: k = %d; variable number out of range", k );
	...
	len = 0;
	for ( k = 1; k <= m + n; k++ ) {
		if ( k <= m ) { /* x[k] is auxiliary variable, so N[k] is a unity column */
			if ( glp_get_row_stat( lp, k ) == GLP_BS ) continue;
			/* compute alfa[i,j]; see (9) */
			alfa = -rho[k];
		} else { /* x[k] is structural variable, so N[k] is a column of the
...
int glp_eval_tab_col( glp_prob* lp, int k, int ind[], double val[] ) {
	...
	if ( !( 1 <= k && k <= m + n ) )
		xerror( "glp_eval_tab_col: k = %d; variable number out of range", k );
	if ( k <= m )
...
int glp_prim_rtest( glp_prob* P, int len, const int ind[],
	...
	for ( t = 1; t <= len; t++ ) { /* get the ordinal number of basic variable */
		k = ind[t];
		if ( !( 1 <= k && k <= m + n ) )
			xerror(
		...
		else if ( type == GLP_DB ) { /* xB[i] has both lower and upper bounds */
			if ( alfa < 0.0 )
				goto lo;
			else
				goto up;
		} else if ( type == GLP_FX ) { /* xB[i] is fixed variable */
...
int glp_dual_rtest( glp_prob* P, int len, const int ind[],
	...
	for ( t = 1; t <= len; t++ ) { /* get ordinal number of non-basic variable */
		k = ind[t];
		if ( !( 1 <= k && k <= m + n ) )
			xerror(
		...
		alfa = ( dir > 0 ? +val[t] : -val[t] );
		/* analyze main cases */
		if ( stat == GLP_NL ) { /* xN[j] is on its lower bound */
			if ( alfa < +eps ) continue;
			temp = ( obj * cost ) / alfa;
		} else if ( stat == GLP_NU ) { /* xN[j] is on its upper bound */
			if ( alfa > -eps ) continue;
			temp = ( obj * cost ) / alfa;
		} else if ( stat == GLP_NF ) { /* xN[j] is non-basic free variable */
			if ( -eps < alfa && alfa < +eps ) continue;
			temp = 0.0;
		} else if ( stat == GLP_NS ) { /* xN[j] is non-basic fixed variable */
			continue;
		} else
			xassert( stat != stat );
...
int _glp_analyze_row( glp_prob* P, int len, const int ind[],
	...
	y = 0.0;
	for ( t = 1; t <= len; t++ ) { /* determine value of x[k] = xN[j] in the current basis */
		k = ind[t];
		if ( !( 1 <= k && k <= P->m + P->n ) )
			xerror(
				  ...
				  " of range\n",
				  t, k );
		if ( k <= P->m ) { /* x[k] is auxiliary variable */
	...
	if ( type == GLP_LO ) {
		if ( y >= rhs ) { /* the constraint is not violated */
			ret = 1;
			goto done;
		}
	...
	if ( piv == 0 ) { /* no dual feasible adjacent basis exists */
		ret = 2;
		goto done;
	}
...
void glp_analyze_bound( glp_prob* P, int k, double* value1, int* var1,
	...
	xassert( 0 <= len && len <= m );
	/* perform analysis */
	for ( kase = -1; kase <= +1; kase += 2 ) { /* kase < 0 means active bound of x[k] is decreasing;
		...
		if ( piv == 0 ) { /* nothing limits changing the active bound of x[k] */
			p = 0;
		...
		if ( kase < 0 && val[piv] > 0.0 ||
			 kase > 0 && val[piv] < 0.0 ) { /* delta x[p] < 0, so x[p] goes toward its lower bound */
			xassert( ll != -DBL_MAX );
			delta = ll - xx;
		} else { /* delta x[p] > 0, so x[p] goes toward its upper bound */
			xassert( uu != +DBL_MAX );
			delta = uu - xx;
		}
...
void glp_analyze_coef( glp_prob* P, int k, double* coef1, int* var1,
	...
	/* perform analysis */
	for ( kase = -1; kase <= +1; kase += 2 ) { /* kase < 0 means objective coefficient c[k] is decreasing;
		...
		if ( rpiv == 0 ) { /* nothing limits changing c[k] */
		...
		if ( kase < 0 && rval[rpiv] > 0.0 ||
			 kase > 0 && rval[rpiv] < 0.0 ) { /* delta d[q] < 0, so d[q] being non-negative will become
												 negative, so x[q] will increase */
			dir = +1;
		} else { /* delta d[q] > 0, so d[q] being non-positive will become
		...
		if ( cpiv == 0 ) { /* non-basic variable x[q] can change unlimitedly */
			if ( dir < 0 && rval[rpiv] > 0.0 ||
				 dir > 0 && rval[rpiv] < 0.0 ) { /* delta x[k] = alfa[k,q] * delta x[q] < 0 */
				new_x = -DBL_MAX;
			} else { /* delta x[k] = alfa[k,q] * delta x[q] > 0 */
				new_x = +DBL_MAX;
			}
			goto store;
		}
		...
		if ( dir < 0 && cval[cpiv] > 0.0 ||
			 dir > 0 && cval[cpiv] < 0.0 ) { /* delta x[p] < 0, so x[p] goes toward its lower bound */
			xassert( ll != -DBL_MAX );
			delta = ll - xx;
		} else { /* delta x[p] > 0, so x[p] goes toward its upper bound */
			xassert( uu != +DBL_MAX );
