...
static void transform( NPP* npp ) { /* transform LP to the standard formulation */
	NPPROW *row, *prev_row;
	NPPCOL *col, *prev_col;
...
double glp_ipt_obj_val( glp_prob* lp ) { /*struct LPXCPS *cps = lp->cps;*/
	double z;
	z = lp->ipt_obj;
...
double glp_ipt_row_prim( glp_prob* lp, int i ) { /*struct LPXCPS *cps = lp->cps;*/
	double pval;
	if ( !( 1 <= i && i <= lp->m ) )
...
double glp_ipt_row_dual( glp_prob* lp, int i ) { /*struct LPXCPS *cps = lp->cps;*/
	double dval;
	if ( !( 1 <= i && i <= lp->m ) )
...
double glp_ipt_col_prim( glp_prob* lp, int j ) { /*struct LPXCPS *cps = lp->cps;*/
	double pval;
	if ( !( 1 <= j && j <= lp->n ) )
...
double glp_ipt_col_dual( glp_prob* lp, int j ) { /*struct LPXCPS *cps = lp->cps;*/
	double dval;
	if ( !( 1 <= j && j <= lp->n ) )
