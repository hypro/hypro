...
void glp_set_col_kind( glp_prob* mip, int j, int kind ) {
	...
	switch ( kind ) {
		...
		case GLP_BV:
			col->kind = GLP_IV;
			if ( !( col->type == GLP_DB && col->lb == 0.0 && col->ub == 1.0 ) ) glp_set_col_bnds( mip, j, GLP_DB, 0.0, 1.0 );
			break;
		default:
			xerror(
...
int glp_get_num_bin( glp_prob* mip ) {
	...
	for ( j = 1; j <= mip->n; j++ ) {
		col = mip->col[j];
		if ( col->kind == GLP_IV && col->type == GLP_DB && col->lb == 0.0 && col->ub == 1.0 ) count++;
	}
	return count;
}
...
static int preprocess_and_solve_mip( glp_prob* P, const glp_iocp* parm ) { /* solve MIP using the preprocessor */
	ENV* env = get_env_ptr();
	...
	if ( parm->msg_lev >= GLP_MSG_ALL ) {
		...
		xprintf( "%d row%s, %d column%s, %d non-zero%s\n",
				 mip->m, mip->m == 1 ? "" : "s", mip->n, mip->n == 1 ? "" : "s", mip->nnz, mip->nnz == 1 ? "" : "s" );
		if ( nb == 0 )
			strcpy( s, "none of" );
		else if ( ni == 1 && nb == 1 )
			strcpy( s, "" );
...
int glp_intopt( glp_prob* P, const glp_iocp* parm ) { /* solve MIP problem with the branch-and-bound method */
	glp_iocp _parm;
	int i, j, ret;
#if 0 /* 04/IV-2016 */
...
#if 1					 /* 28/V-2010 */
	if ( parm->alien ) { /* use alien integer optimizer */
...
#endif
		else ret = preprocess_and_solve_mip( P, parm );
#if 1 /* 12/III-2013 */
	if ( ret == GLP_ENOPFS )
		P->mip_stat = GLP_NOFEAS;
#endif
...
double glp_mip_obj_val( glp_prob* mip ) { /*struct LPXCPS *cps = mip->cps;*/
	double z;
	z = mip->mip_obj;
...
double glp_mip_row_val( glp_prob* mip, int i ) { /*struct LPXCPS *cps = mip->cps;*/
	double mipx;
	if ( !( 1 <= i && i <= mip->m ) )
...
double glp_mip_col_val( glp_prob* mip, int j ) { /*struct LPXCPS *cps = mip->cps;*/
	double mipx;
	if ( !( 1 <= j && j <= mip->n ) )
