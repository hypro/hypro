...
int ios_choose_var( glp_tree* T, int* next ) {
	int j;
	if ( T->parm->br_tech == GLP_BR_FFV ) { /* branch on first fractional variable */
		j = branch_first( T, next );
	} else if ( T->parm->br_tech == GLP_BR_LFV ) { /* branch on last fractional variable */
		j = branch_last( T, next );
	} else if ( T->parm->br_tech == GLP_BR_MFV ) { /* branch on most fractional variable */
		j = branch_mostf( T, next );
	} else if ( T->parm->br_tech == GLP_BR_DTH ) { /* branch using the heuristic by Dreebeck and Tomlin */
		j = branch_drtom( T, next );
	} else if ( T->parm->br_tech == GLP_BR_PCH ) { /* hybrid pseudocost heuristic */
...
static int branch_drtom( glp_tree* T, int* _next ) {
	...
	for ( j = 1; j <= n; j++ ) { /* if j-th column is not marked as fractional, skip it */
		...
		for ( kase = -1; kase <= +1; kase += 2 ) { /* if kase < 0, the new upper bound of x[j] is introduced;
			...
			/* Tomlin noticed that if the variable x[k] is of integer
			   ...
			   the magnitude */
			if ( k > m && glp_get_col_kind( mip, k - m ) != GLP_CV ) { /* x[k] is structural integer variable */
		...
		if ( degrad < fabs( dz_dn ) || degrad < fabs( dz_up ) ) {
			jj = j;
			if ( fabs( dz_dn ) < fabs( dz_up ) ) { /* select down branch to be solved next */
				next = GLP_DN_BRNCH;
				degrad = fabs( dz_up );
			} else { /* select up branch to be solved next */
				next = GLP_UP_BRNCH;
				degrad = fabs( dz_dn );
			}
...
void* ios_pcost_init( glp_tree* tree ) { /* initialize working data used on pseudocost branching */
	struct csa* csa;
	int n = tree->n, j;
...
static double eval_degrad( glp_prob* P, int j, double bnd ) { /* compute degradation of the objective on fixing x[j] at given
	...
	if ( ret == 0 || ret == GLP_EITLIM ) {
		if ( glp_get_prim_stat( lp ) == GLP_NOFEAS ) { /* resulting LP has no primal feasible solution */
			degrad = DBL_MAX;
		} else if ( glp_get_dual_stat( lp ) == GLP_FEAS ) { /* resulting basis is optimal or at least dual feasible,
			...
			if ( degrad < 1e-6 * ( 1.0 + 0.001 * fabs( P->obj_val ) ) )
				degrad = 0.0;
		} else { /* the final basis reported by the simplex solver is dual
					infeasible, so we cannot determine a non-trivial lower
					bound to degradation */
			degrad = 0.0;
		}
	} else { /* the simplex solver failed */
		degrad = 0.0;
	}
...
void ios_pcost_update( glp_tree* tree ) { /* update history information for pseudocost branching */
	...
	if ( dx < 0.0 ) { /* the current subproblem is down-branch */
		csa->dn_cnt[j]++;
		csa->dn_sum[j] += psi;
	} else /* dx > 0.0 */
...
static double eval_psi( glp_tree* T, int j, int brnch ) { /* compute estimation of pseudocost of variable x[j] for down-
															 or up-branch */
	struct csa* csa = T->pcost;
	double beta, degrad, psi;
	xassert( csa != NULL );
	xassert( 1 <= j && j <= T->n );
	if ( brnch == GLP_DN_BRNCH ) {	 /* down-branch */
		if ( csa->dn_cnt[j] == 0 ) { /* initialize down pseudocost */
	...
	} else if ( brnch == GLP_UP_BRNCH ) { /* up-branch */
		if ( csa->up_cnt[j] == 0 ) {	  /* initialize up pseudocost */
			beta = T->mip->col[j]->prim;
...
static void progress( glp_tree* T ) { /* display progress of pseudocost initialization */
	struct csa* csa = T->pcost;
	int j, nv = 0, ni = 0;
	for ( j = 1; j <= T->n; j++ ) {
...
int ios_pcost_branch( glp_tree* T, int* _next ) { /* choose branching variable with pseudocost branching */
#if 0											  /* 10/VI-2013 */
      glp_long t = xtime();
#else
	double t = xtime();
	...
	for ( j = 1; j <= T->n; j++ ) {
		...
		if ( psi == DBL_MAX ) { /* down-branch has no primal feasible solution */
		...
		if ( psi == DBL_MAX ) { /* up-branch has no primal feasible solution */
	...
	if ( dmax == 0.0 ) { /* no degradation is indicated; choose a variable having most
							fractional value */
