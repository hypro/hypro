...
static void set_d_eps( mpq_t x, double val ) { /* convert double val to rational x obtaining a more adequate
...
#if 1							 /* 30/VII-2008 */
	if ( val == floor( val ) ) { /* if val is integral, do not approximate */
		mpq_set_d( x, val );
		goto done;
	}
#endif
...
static void load_data( SSX* ssx, glp_prob* lp ) { /* load LP problem data into simplex solver workspace */
	int m = ssx->m;
	int n = ssx->n;
...
static int load_basis( SSX* ssx, glp_prob* lp ) { /* load current LP basis into simplex solver workspace */
	int m = ssx->m;
	int n = ssx->n;
	int* type = ssx->type;
	int* stat = ssx->stat;
...
if ( parm->msg_lev >= GLP_MSG_ALL ) {
	...
	xprintf(
		  "(Consider installing GNU MP to attain a much better perf"
		  "ormance.)\n" );
#endif
#if 1 /* 25/XI-2017 */
}
