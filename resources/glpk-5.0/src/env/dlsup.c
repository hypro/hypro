...
void* xdlopen( const char* module ) { /* open dynamically linked library */
	void* h = NULL;
	if ( lt_dlinit() != 0 ) {
...
void* xdlsym( void* h, const char* symbol ) { /* obtain address of symbol from dynamically linked library */
	void* ptr;
	xassert( h != NULL );
...
void xdlclose( void* h ) { /* close dynamically linked library */
	xassert( h != NULL );
	if ( lt_dlclose( h ) != 0 )
		xerror( "xdlclose: %s\n", lt_dlerror() );
	if ( lt_dlexit() != 0 )
