...
static void* dma( const char* func, void* ptr, size_t size ) {
	ENV* env = get_env_ptr();
	MBD* mbd;
	if ( ptr == NULL ) { /* new memory block will be allocated */
		mbd = NULL;
	} else { /* allocated memory block will be reallocated or freed */
...
void* glp_realloc( void* ptr, int n, int size ) { /* reallocate memory block */
	if ( ptr == NULL )
