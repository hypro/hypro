...
struct glp_file { /* sequential stream descriptor */
	char* base;
	/* pointer to buffer */
	int size;
...
int glp_read( glp_file* f, void* buf, int nnn ) {
	...
	for ( nrd = 0; nrd < nnn; nrd += cnt ) {
		if ( f->cnt == 0 ) { /* buffer is empty; fill it */
			if ( f->flag & IONULL )
				cnt = 0;
...
static int do_flush( glp_file* f ) {
	...
	if ( f->cnt > 0 ) {
		if ( f->flag & IONULL )
			...
			if ( (int)fwrite( f->base, 1, f->cnt, (FILE*)( f->file ) ) != f->cnt ) {
				f->flag |= IOERR;
#if 0 /* 29/I-2017 */
               put_err_msg(strerror(errno));
#else
...
int glp_write( glp_file* f, const void* buf, int nnn ) {
	...
	for ( nwr = 0; nwr < nnn; nwr += cnt ) {
		...
		f->ptr += cnt;
		f->cnt += cnt;
		if ( f->cnt == f->size ) { /* buffer is full; flush it */
			if ( do_flush( f ) != 0 )
				return EOF;
		}
	}
