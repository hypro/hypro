...
static void* db_iodbc_open_int( TABDCA* dca, int mode, const char** sqllines );
static void* db_mysql_open_int( TABDCA* dca, int mode, const char** sqllines );
...
static char** args_concat( TABDCA* dca ) {
	...
	for ( j = 3; j <= narg; j++ ) {
		...
		/* add length of space separating parts or 0x00 at end of SQL
		   statement */
		lentot++;
		if ( arg[len - 1] == ';' || j == narg ) { /* Join arguments for a single SQL statement */
...
static void* db_iodbc_open_int( TABDCA* dca, int mode, const char** sqllines ) {
	...
	if ( SQL_SUCCEEDED( ret ) ) { /* output information about data base connection */
	...
	if ( sql->mode == 'R' ) {
		...
		for ( i = 1; i <= total; i++ ) { /* return a set of attributes for a column */
	...
	for ( i = 1; i <= sql->nresultcols; i++ ) {
		if ( sql->ref[i] > 0 ) {
			...
			if ( len != SQL_NULL_DATA ) {
				if ( sql->isnumeric[i] ) {  mpl_tab_set_num(dca, sql->ref[i],
#if 0 /* 12/I-2014 */
...
int db_iodbc_write( TABDCA* dca, void* link ) {
   ...
   for (k = 1, part = strtok (template, "?"); (part != NULL);
	...
	if ( part != NULL )
		strcat( query, part );
	if ( dl_SQLExecDirect( sql->hstmt, (SQLCHAR*)query, SQL_NTS ) != SQL_SUCCESS ) {
   ...
   for (i = 1, keyword = strtok (arg, "="); (keyword != NULL);
	...
	if ( sql->mode == 'R' ) {
		...
		if ( sql->res ) {
			...
			for ( i = 1; i <= total; i++ ) {
				for ( j = sql->nf; j >= 1; j-- ) {
					if ( strcmp( mpl_tab_get_name( dca, j ), fields[i - 1].name ) == 0 )
						break;
				}
				sql->ref[i] = j;
			}
