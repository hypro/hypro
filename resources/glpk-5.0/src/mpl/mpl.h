...
struct SYMBOL { /* symbol (numeric or abstract quantity) */
	double num;
...
struct TUPLE { /* component of n-tuple; the n-tuple itself is associated with
...
#define set_cross _glp_mpl_set_cross
	...
	struct ELEMVAR { /* elemental variable; formally it is a "value" assigned to
	...
	struct FORMULA { /* term of linear form c * x, where c is a coefficient, x is an
...
#define reduce_terms _glp_mpl_reduce_terms
	...
	struct ELEMCON { /* elemental constraint; formally it is a "value" assigned to
	...
	union VALUE { /* generic value, which can be assigned to object member or be a
...
#define delete_value _glp_mpl_delete_value
	...
	struct ARRAY { /* multi-dimensional array, a set of members indexed over simple
...
#define delete_array _glp_mpl_delete_array
	...
	struct DOMAIN { /* domain (a simple or compound set); syntactically domain looks
	...
	struct DOMAIN_BLOCK { /* domain block; syntactically domain blocks look like 'i in I',
	...
	struct DOMAIN_SLOT { /* domain slot; it specifies an individual indexing position and
...
#define clean_domain _glp_mpl_clean_domain
	...
	struct WITHIN { /* restricting superset list entry */
		CODE* code;
	...
	struct GADGET { /* plain set used to initialize the array of sets with data */
		SET* set;
...
#define clean_set _glp_mpl_clean_set
	...
	struct PARAMETER { /* model parameter */
		char* name;
		/* symbolic name; cannot be NULL */
		char* alias;
		/* alias; NULL means alias is not specified */
	...
	struct CONDITION { /* restricting condition list entry */
		int rho;
...
#define clean_parameter _glp_mpl_clean_parameter
	...
	struct VARIABLE { /* model variable */
		char* name;
		/* symbolic name; cannot be NULL */
		char* alias;
		/* alias; NULL means alias is not specified */
...
#define clean_variable _glp_mpl_clean_variable
	...
	struct CONSTRAINT { /* model constraint or objective */
		char* name;
		/* symbolic name; cannot be NULL */
		char* alias;
		/* alias; NULL means alias is not specified */
...
#define clean_constraint _glp_mpl_clean_constraint
	...
	struct TABARG { /* table argument list entry */
		CODE* code;
	...
	struct TABFLD { /* table field list entry */
		char* name;
	...
	struct TABIN { /* table input list entry */
		PARAMETER* par;
	...
	struct TABOUT { /* table output list entry */
		CODE* code;
	...
	struct TABDCA { /* table driver communication area */
		int id;
...
#define mpl_tab_drv_close _glp_mpl_tab_drv_close
	...
	union OPERANDS { /* operands that participate in pseudo-code operation (choice of
...
#define DOT_DUAL 0x05	/* .dual    (dual value) */
	...
	struct ARG_LIST { /* operands list entry */
		CODE* x;
	...
	struct CODE { /* pseudo-code (internal form of expressions) */
		int op;
		/* operation code: */
...
#define clean_code _glp_mpl_clean_code
	...
	struct DISPLAY { /* display statement */
		DOMAIN* domain;
		/* subscript domain; NULL means domain is not used */
		DISPLAY1* list;
	...
	struct DISPLAY1 { /* display list entry */
		int type;
		/* item type:
		   A_INDEX      - dummy index
		   A_SET        - model set
	...
	struct PRINTF { /* printf statement */
		DOMAIN* domain;
		/* subscript domain; NULL means domain is not used */
		CODE* fmt;
	...
	struct PRINTF1 { /* printf list entry */
		CODE* code;
	...
	struct STATEMENT { /* model statement */
		int line;
