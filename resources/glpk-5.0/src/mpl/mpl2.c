...
int is_literal( MPL* mpl, char* literal ) {
	return is_symbol( mpl ) && strcmp( mpl->image, literal ) == 0;
}
...
SLICE* read_slice( MPL* mpl,
	...
	slice = create_slice( mpl );
	for ( ;; ) { /* the current token must be a symbol or asterisk */
		if ( is_symbol( mpl ) )
...
void simple_format( MPL* mpl,
	...
	for ( temp = slice; temp != NULL; temp = temp->next ) {
		if ( temp->sym == NULL ) { /* substitution is needed; read symbol */
			if ( !is_symbol( mpl ) ) {
				...
				if ( lack == 1 )
					...
					error( mpl,
						   ...
						   "with %s",
						   lack, format_symbol( mpl, with ) );
			}
			sym = read_symbol( mpl );
			if ( with == NULL ) with = sym;
		} else { /* copy symbol from the slice */
...
void matrix_format( MPL* mpl,
	...
	while ( mpl->token != T_ASSIGN ) { /* read column symbol and append it to the column list */
		if ( !is_symbol( mpl ) )
	...
	while ( is_symbol( mpl ) ) { /* read row symbol (if the matrix has no columns, row symbols
									are just ignored) */
		row = read_symbol( mpl );
			...
			if ( is_literal( mpl, "+" ) )
				...
				if ( lack == 1 )
					error( mpl,
						   "one item missing in data group beginning "
						   "with %s",
						   format_symbol( mpl, row ) );
				else
					error( mpl,
						   "%d items missing in data group beginning "
						   "with %s",
						   lack, format_symbol( mpl, row ) );
			}
			/* construct complete n-tuple */
			tuple = create_tuple( mpl );
			for ( temp = slice; temp != NULL; temp = temp->next ) {
				if ( temp->sym == NULL ) { /* substitution is needed */
					switch ( ++which ) {
						case 1:
...
void set_data( MPL* mpl ) {
	...
	/* read optional subscript list, which identifies member of the
	   set to be read */
	tuple = create_tuple( mpl );
	if ( mpl->token == T_LBRACKET ) { /* subscript list is specified */
		if ( set->dim == 0 )
		...
		get_token( mpl /* ] */ );
	} else { /* subscript list is not specified */
		if ( set->dim != 0 )
	...
	for ( ;; ) { /* skip optional comma */
		if ( mpl->token == T_COMMA ) get_token( mpl /* , */ );
		/* process assignment element */
		if ( mpl->token == T_ASSIGN ) { /* assignment ligature is non-significant element */
			get_token( mpl /* := */ );
		} else if ( mpl->token == T_LEFT ) { /* left parenthesis begins either new slice or "transpose"
												indicator */
			int is_tr;
			get_token( mpl /* ( */ );
		...
		} else if ( is_symbol( mpl ) ) { /* number or symbol begins data in the simple format */
			simple_format( mpl, set, memb, slice );
		} else if ( mpl->token == T_COLON ) { /* colon begins data in the matrix format */
			if ( slice_arity( mpl, slice ) != 2 )
			err1:
				error( mpl,
					   "slice currently used must specify 2 asterisk"
					   "s, not %d",
					   slice_arity( mpl, slice ) );
			get_token( mpl /* : */ );
		...
		left : { /* left parenthesis begins the "transpose" indicator, which
			...
			err2:
				error( mpl, "transpose indicator (tr) incomplete" );
			...
			else if ( mpl->token == T_SEMICOLON ) { /* semicolon terminates the data block */
				get_token( mpl /* ; */ );
				break;
			}
		else
...
void tabular_format( MPL* mpl,
	...
	while ( is_symbol( mpl ) ) { /* read row symbol (if the table has no columns, these symbols
									are just ignored) */
		row = read_symbol( mpl );
...
void tabbing_format( MPL* mpl,
	...
	if ( is_symbol( mpl ) ) {
		...
		if ( next_token == T_COLON ) { /* select the set to saturate it with data */
			set = select_set( mpl, mpl->image );
	...
	list = create_slice( mpl );
	while ( mpl->token != T_ASSIGN ) { /* there must be symbolic name of parameter */
		if ( !is_symbol( mpl ) )
	...
	while ( is_symbol( mpl ) ) { /* read subscript list */
		tuple = create_tuple( mpl );
		for ( j = 1; j <= dim; j++ ) { /* read j-th subscript */
			if ( !is_symbol( mpl ) ) {
				...
				error( mpl,
					   "%d items missing in data group beginning wit"
					   "h %s",
					   lack, format_symbol( mpl, tuple->sym ) );
			}
		...
		if ( mpl->token == T_COMMA ) get_token( mpl /* , */ );
		/* read values accordingly to the column list */
		for ( col = list; col != NULL; col = col->next ) { /* if the token is single point, no value is provided */
			if ( is_literal( mpl, "." ) ) {
				get_token( mpl /* . */ );
				continue;
			}
			...
			read_value( mpl, (PARAMETER*)col->sym, copy_tuple( mpl, tuple ) );
...
void parameter_data( MPL* mpl ) {
	...
	for ( ;; ) { /* skip optional comma */
		...
		if ( mpl->token == T_ASSIGN ) { /* assignment ligature is non-significant element */
			get_token( mpl /* := */ );
		} else if ( mpl->token == T_LBRACKET ) { /* left bracket begins new slice; delete the current slice
													and read new one */
			delete_slice( mpl, slice );
			slice = read_slice( mpl, par->name, par->dim );
			/* each new slice resets the "transpose" indicator */
			tr = 0;
		} else if ( is_symbol( mpl ) ) { /* number or symbol begins data in the plain format */
			plain_format( mpl, par, slice );
		} else if ( mpl->token == T_COLON ) { /* colon begins data in the tabular format */
			if ( par->dim == 0 )
			err1:
				error( mpl, "%s not a subscripted parameter",
					   par->name );
			if ( slice_arity( mpl, slice ) != 2 )
			err2:
				error( mpl,
		...
		} else if ( mpl->token == T_LEFT ) { /* left parenthesis begins the "transpose" indicator, which
