...
struct csv { /* comma-separated values file */
	int mode;
	/* 'R' = reading; 'W' = writing */
	char* fname;
	/* name of csv file */
...
static void read_char( struct csv* csv ) { /* read character from csv data file */
	int c;
	xassert( csv->c != EOF );
	if ( csv->c == '\n' ) csv->count++;
loop:
	c = fgetc( csv->fp );
	if ( ferror( csv->fp ) ) {  xprintf("%s:%d: read error - %s\n", csv->fname, csv->count,
#if 0 /* 29/I-2017 */
            strerror(errno));
#else
				 xstrerr( errno ) );
...
static void read_field( struct csv* csv ) { /* read field from csv data file */
	...
	if ( csv->c == '\n' ) {
		...
		if ( csv->c == ',' )
		err1 : {
			xprintf( "%s:%d: empty field not allowed\n", csv->fname,
					 csv->count );
			longjmp( csv->jump, 0 );
		}
			if ( csv->c == '\n' ) {
	...
	if ( csv->c == ',' )
		read_char( csv );
	/* read field */
	if ( csv->c == '\'' || csv->c == '"' ) { /* read a field enclosed in quotes */
		int quote = csv->c, len = 0;
		csv->what = CSV_STR;
		/* skip opening quote */
		read_char( csv );
		/* read field characters within quotes */
		for ( ;; ) { /* check for closing quote and read it */
			if ( csv->c == quote ) {
				read_char( csv );
				if ( csv->c == quote )
					;
		...
		if ( len == 0 ) goto err1;
		csv->field[len] = '\0';
	} else { /* read a field not enclosed in quotes */
		int len = 0;
		double temp;
		csv->what = CSV_NUM;
...
static struct csv* csv_open_file( TABDCA* dca, int mode ) { /* open csv data file */
	struct csv* csv;
	...
	if ( mode == 'R' ) { /* open the file for reading */
		int k;
		csv->fp = fopen( csv->fname, "r" );
		if ( csv->fp == NULL ) {  xprintf("csv_driver: unable to open %s - %s\n",
#if 0 /* 29/I-2017 */
		...
		csv->ref[0] = k;
	} else if ( mode == 'W' ) { /* open the file for writing */
		int k, nf;
		csv->fp = fopen( csv->fname, "w" );
		if ( csv->fp == NULL ) {  xprintf("csv_driver: unable to create %s - %s\n",
#if 0 /* 29/I-2017 */
               csv->fname, strerror(errno));
#else
					 csv->fname, xstrerr( errno ) );
...
static int csv_read_record( TABDCA* dca, struct csv* csv ) { /* read next record from csv data file */
	int k, ret = 0;
	xassert( csv->mode == 'R' );
	if ( setjmp( csv->jump ) ) {
		ret = 1;
	...
	if ( csv->ref[0] > 0 )
		...
		for ( k = 1; k <= csv->nf; k++ ) {
			read_field( csv );
			if ( csv->what == CSV_EOF ) { /* end-of-file reached */
				xassert( k == 1 );
				ret = -1;
				goto done;
			} else if ( csv->what == CSV_EOR ) { /* end-of-record reached */
				int lack = csv->nf - k + 1;
				if ( lack == 1 )
					...
					xprintf( "%s:%d: %d fields missing\n", csv->fname,
							 csv->count, lack );
				longjmp( csv->jump, 0 );
			} else if ( csv->what == CSV_NUM ) { /* floating-point number */
				if ( csv->ref[k] > 0 ) {
					double num;
					xassert( str2num( csv->field, &num ) == 0 );
					mpl_tab_set_num( dca, csv->ref[k], num );
				}
			} else if ( csv->what == CSV_STR ) { /* character string */
				if ( csv->ref[k] > 0 )
...
static int csv_write_record( TABDCA* dca, struct csv* csv ) { /* write next record to csv data file */
	int k, nf, ret = 0;
	const char* c;
	xassert( csv->mode == 'W' );
	nf = mpl_tab_num_flds( dca );
	...
	csv->count++;
	if ( ferror( csv->fp ) ) {  xprintf("%s:%d: write error - %s\n", csv->fname, csv->count,
#if 0 /* 29/I-2017 */
            strerror(errno));
#else
				 xstrerr( errno ) );
...
static int csv_close_file( TABDCA* dca, struct csv* csv ) { /* close csv data file */
	int ret = 0;
	xassert( dca == dca );
	if ( csv->mode == 'W' ) {
		fflush( csv->fp );
...
static int read_byte( struct dbf* dbf ) { /* read byte from xBASE data file */
	int b;
	b = fgetc( dbf->fp );
	if ( ferror( dbf->fp ) ) {  xprintf("%s:0x%X: read error - %s\n", dbf->fname,
#if 0 /* 29/I-2017 */
...
static void read_header( TABDCA* dca, struct dbf* dbf ) { /* read xBASE data file header */
	int b, j, k, recl;
	char name[10 + 1];
	/* (ignored) */
	for ( j = 1; j <= 10; j++ )
...
static void parse_third_arg( TABDCA* dca, struct dbf* dbf ) { /* parse xBASE file format (third argument) */
	int j, k, temp;
	const char* arg;
	dbf->nf = mpl_tab_num_flds( dca );
	arg = mpl_tab_get_arg( dca, 3 ), j = 0;
	for ( k = 1; k <= dbf->nf; k++ ) { /* parse specification of k-th field */
		if ( arg[j] == '\0' ) {
		...
		err : {
			xprintf( "xBASE driver: field %s: invalid field format\n",
					 mpl_tab_get_name( dca, k ) );
			longjmp( dbf->jump, 0 );
		}
			/* parse field length */
...
static void write_byte( struct dbf* dbf, int b ) { /* write byte to xBASE data file */
	fputc( b, dbf->fp );
	dbf->offset++;
	return;
}

static void write_header( TABDCA* dca, struct dbf* dbf ) { /* write xBASE data file header */
	int j, k, temp;
	const char* name;
	/* version number */
	...
	for ( j = 1; j <= 20; j++ )
		write_byte( dbf, 0x00 );
	/* field descriptor array */
	for ( k = 1; k <= dbf->nf; k++ ) { /* field name (terminated by 0x00) */
...
static struct dbf* dbf_open_file( TABDCA* dca, int mode ) { /* open xBASE data file */
	struct dbf* dbf;
	/* create control structure */
	...
	if ( mode == 'R' ) { /* open the file for reading */
		...
		if ( dbf->fp == NULL ) {  xprintf("xBASE driver: unable to open %s - %s\n",
#if 0 /* 29/I-2017 */
		...
		if ( dbf->fp == NULL ) {  xprintf("xBASE driver: unable to create %s - %s\n",
#if 0 /* 29/I-2017 */
               dbf->fname, strerror(errno));
#else
					 dbf->fname, xstrerr( errno ) );
...
static int dbf_read_record( TABDCA* dca, struct dbf* dbf ) { /* read next record from xBASE data file */
	int b, j, k, ret = 0;
	...
	/* read fields */
	for ( k = 1; k <= dbf->nf; k++ ) { /* read k-th field */
		for ( j = 0; j < dbf->len[k]; j++ )
			buf[j] = (char)read_byte( dbf );
		buf[dbf->len[k]] = '\0';
		/* set field value */
		if ( dbf->type[k] == 'C' ) { /* character field */
			if ( dbf->ref[k] > 0 )
				mpl_tab_set_str( dca, dbf->ref[k], strtrim( buf ) );
		} else if ( dbf->type[k] == 'N' ) { /* numeric field */
			if ( dbf->ref[k] > 0 ) {
				double num;
				strspx( buf );
...
static int dbf_write_record( TABDCA* dca, struct dbf* dbf ) { /* write next record to xBASE data file */
	int j, k, ret = 0;
	char buf[255 + 1];
	xassert( dbf->mode == 'W' );
	if ( setjmp( dbf->jump ) ) {
...
static int dbf_close_file( TABDCA* dca, struct dbf* dbf ) { /* close xBASE data file */
	int ret = 0;
	xassert( dca == dca );
	if ( dbf->mode == 'W' ) {
		if ( setjmp( dbf->jump ) ) {
		...
		/* number of records */
		dbf->offset = 4;
		if ( fseek( dbf->fp, dbf->offset, SEEK_SET ) ) {  xprintf("%s:0x%X: seek error - %s\n", dbf->fname,
#if 0 /* 29/I-2017 */
               dbf->offset, strerror(errno));
#else
					 dbf->offset, xstrerr( errno ) );
		...
		fflush( dbf->fp );
		if ( ferror( dbf->fp ) ) {  xprintf("%s:0x%X: write error - %s\n", dbf->fname,
#if 0 /* 29/I-2017 */
               dbf->offset, strerror(errno));
#else
					 dbf->offset, xstrerr( errno ) );
