...
double fp_idiv( MPL* mpl, double x, double y ) {
	...
	if ( fabs( y ) < 1.0 && fabs( x ) > ( 0.999 * DBL_MAX ) * fabs( y ) )
		error( mpl, "%.*g div %.*g; floating-point overflow",
			   DBL_DIG, x, DBL_DIG, y );
	x /= y;
	return x > 0.0 ? floor( x ) : x < 0.0 ? ceil( x )
										  : 0.0;
}
...
double fp_irand224( MPL* mpl ) {
	return (double)rng_unif_rand( mpl->rand, two_to_the_24 );
}
...
double fp_uniform01( MPL* mpl ) {
	return (double)rng_next_rand( mpl->rand ) / (double)two_to_the_31;
}
...
SYMBOL* create_symbol_str( MPL* mpl,
						   STRING* str /* destroyed */
) {
	SYMBOL* sym;
	xassert( str != NULL );
...
MEMBER* check_then_add( MPL* mpl,
						ELEMSET* set, /* modified */
						TUPLE* tuple  /* destroyed */
) {
	if ( find_tuple( mpl, set, tuple ) != NULL )
		error( mpl, "duplicate tuple %s detected", format_tuple( mpl, '(', tuple ) );
	...
	for ( memx = X->head; memx != NULL; memx = memx->next ) {
		for ( memy = Y->head; memy != NULL; memy = memy->next ) {
			...
			for ( temp = memy->tuple; temp != NULL; temp = temp->next )
				tuple = expand_tuple( mpl, tuple, copy_symbol( mpl, temp->sym ) );
...
FORMULA* single_variable( MPL* mpl,
...
FORMULA* remove_constant( MPL* mpl,
						  FORMULA* form, /* destroyed */
	...
	*coef = 0.0;
	while ( form != NULL ) {
		temp = form;
		form = form->next;
		if ( temp->var == NULL ) { /* constant term */
...
static int compare_member_tuples( void* info, const void* key1,
								  const void* key2 ) { /* this is an auxiliary routine used to compare keys, which are
														  n-tuples assigned to array members */
	return compare_tuples( (MPL*)info, (TUPLE*)key1, (TUPLE*)key2 );
}

MEMBER* find_member( MPL* mpl,
	...
	if ( array->tree == NULL ) { /* the search tree doesn't exist; use the linear search */
		for ( memb = array->head; memb != NULL; memb = memb->next )
			if ( compare_tuples( mpl, memb->tuple, tuple ) == 0 ) break;
	} else { /* the search tree exists; use the binary search */
		AVLNODE* node;
...
void assign_dummy_index( MPL* mpl,
	...
	if ( slot->value != NULL ) { /* if the current value and the new one are identical, actual
	...
	for ( leaf = slot->list; leaf != NULL; leaf = leaf->arg.index.next ) {
		...
		for ( code = leaf; code != NULL; code = code->up ) {
			if ( code->valid ) { /* invalidate and delete resultant value */
				code->valid = 0;
...
void update_dummy_indices( MPL* mpl,
...
int enter_domain_block( MPL* mpl,
						DOMAIN_BLOCK* block, /* not changed */
						TUPLE* tuple,		 /* not changed */
...
struct eval_domain_info { /* working info used by the routine eval_within_domain */
	DOMAIN* domain;
...
static void eval_domain_func( MPL* mpl, void* _my_info ) { /* this routine recursively enters into the domain scope and then
															  calls the routine func */
	struct eval_domain_info* my_info = _my_info;
	if ( my_info->block != NULL ) { /* the current domain block to be entered exists */
		DOMAIN_BLOCK* block;
		DOMAIN_SLOT* slot;
		...
		for ( slot = block->list; slot != NULL; slot = slot->next ) { /* create component that corresponds to the current slot */
			if ( tuple == NULL )
				tuple = temp = dmp_get_atom( mpl->tuples, sizeof( TUPLE ) );
			else
				temp = ( temp->next = dmp_get_atom( mpl->tuples, sizeof( TUPLE ) ) );
			if ( slot->code == NULL ) { /* dummy index is free; take reference to symbol, which
			...
			} else { /* dummy index is non-free; compute symbolic value to be
		...
		for ( slot = block->list; slot != NULL; slot = slot->next ) {
			...
			temp = tuple;
			tuple = tuple->next;
			if ( slot->code != NULL ) { /* dummy index is non-free; delete symbolic value */
				delete_symbol( mpl, temp->sym );
			}
			/* delete component that corresponds to the current slot */
			dmp_free_atom( mpl->tuples, temp, sizeof( TUPLE ) );
		}
	} else { /* there are no more domain blocks, i.e. we have reached the
				domain scope */
		xassert( my_info->tuple == NULL );
		/* check optional predicate specified for the domain */
		if ( my_info->domain->code != NULL && !eval_logical( mpl,
															 my_info->domain->code ) ) { /* the predicate is false */
			my_info->failure = 2;
		} else { /* the predicate is true; do the job */
...
int eval_within_domain( MPL* mpl,
						DOMAIN* domain,										   /* not changed */
						TUPLE* tuple,										   /* not changed */
						void* info, void ( *func )( MPL* mpl, void* info ) ) { /* this routine performs evaluation within domain scope */
...
struct loop_domain_info { /* working info used by the routine loop_within_domain */
	DOMAIN* domain;
	/* domain, which has to be entered */
	DOMAIN_BLOCK* block;
	/* domain block, which is currently processed */
...
static void loop_domain_func( MPL* mpl, void* _my_info ) { /* this routine enumerates all n-tuples in the basic set of the
	...
	if ( my_info->block != NULL ) { /* the current domain block to be entered exists */
		...
		for ( slot = block->list; slot != NULL; slot = slot->next ) {
			if ( slot->code != NULL )
				bound = expand_tuple( mpl, bound, eval_symbolic( mpl, slot->code ) );
		}
		/* start enumeration */
		xassert( block->code != NULL );
		if ( block->code->op == O_DOTS ) { /* the basic set is "arithmetic", in which case it doesn't
			...
			for ( j = 1; j <= n && my_info->looping; j++ ) { /* construct dummy 1-tuple for the current member */
		...
		} else { /* the basic set is of general kind, in which case it needs
			...
			for ( memb = set->head; memb != NULL && my_info->looping;
				  memb = memb->next ) { /* all components of the current n-tuple that correspond
				...
				for ( slot = block->list; slot != NULL; slot = slot->next ) {
					xassert( temp1 != NULL );
					if ( slot->code != NULL ) { /* non-free dummy index */
						xassert( temp2 != NULL );
						if ( compare_symbols( mpl, temp1->sym, temp2->sym ) != 0 ) { /* the n-tuple is not in the basic set */
							goto skip;
						}
						temp2 = temp2->next;
					}
...
void loop_within_domain( MPL* mpl,
						 DOMAIN* domain,									   /* not changed */
						 void* info, int ( *func )( MPL* mpl, void* info ) ) { /* this routine performs iterations within domain scope */
...
void out_of_domain( MPL* mpl,
					char* name,	 /* not changed */
					TUPLE* tuple /* not changed */
) {
	xassert( name != NULL );
	xassert( tuple != NULL );
	error( mpl, "%s%s out of domain", name, format_tuple( mpl, '[', tuple ) );
	/* no return */
}
...
TUPLE* get_domain_tuple( MPL* mpl,
						 DOMAIN* domain /* not changed */
) {
	DOMAIN_BLOCK* block;
	DOMAIN_SLOT* slot;
		...
		for ( block = domain->list; block != NULL; block = block->next ) {
			for ( slot = block->list; slot != NULL; slot = slot->next ) {
				if ( slot->code == NULL ) {
					...
					tuple = expand_tuple( mpl, tuple, copy_symbol( mpl, slot->value ) );
				}
			}
		}
	}
...
void clean_domain( MPL* mpl, DOMAIN* domain ) {
	...
	for ( block = domain->list; block != NULL; block = block->next ) { /* clean all domain slots */
		for ( slot = block->list; slot != NULL; slot = slot->next ) {  /* clean pseudo-code for computing bound value */
		...
		for ( memb = refer->head; memb != NULL; memb = memb->next ) {
			if ( !is_member( mpl, within->code, memb->tuple ) ) {
				...
				error( mpl,
					   ...
					   "set; see (%d)",
					   set->name, format_tuple( mpl, '[', tuple ), buf, eqno );
			}
		}
	}
	return;
...
ELEMSET* take_member_set /* returns reference, not value */
	...
	if ( memb != NULL ) { /* member exists, so just take the reference */
		refer = memb->value.set;
	} else if ( set->assign != NULL ) { /* compute value using assignment expression */
	...
	} else if ( set->option != NULL ) { /* compute default elemental set */
		refer = eval_elemset( mpl, set->option );
		goto add;
	} else { /* no value (elemental set) is provided */
		error( mpl, "no value for %s%s", set->name, format_tuple( mpl, '[', tuple ) );
	}
	return refer;
}
...
struct eval_set_info { /* working info used by the routine eval_member_set */
	SET* set;
	/* model set */
	TUPLE* tuple;
...
static void eval_set_func( MPL* mpl, void* _info ) { /* this is auxiliary routine to work within domain scope */
	struct eval_set_info* info = _info;
	if ( info->memb != NULL ) { /* checking call; check elemental set being assigned */
		check_elem_set( mpl, info->set, info->memb->tuple,
						info->memb->value.set );
	} else { /* normal call; evaluate member, which has given n-tuple */
...
static void saturate_set( MPL* mpl, SET* set ) {
	...
	for ( elem = data->head; elem != NULL; elem = elem->next ) { /* create a copy of n-tuple */
		...
		memb = find_member( mpl, set->array, tuple );
		if ( memb == NULL ) { /* not found; add new member to the set and assign it empty
								 elemental set */
			memb = add_member( mpl, set->array, tuple );
			memb->value.set = create_elemset( mpl, set->dimen );
		} else { /* found; free subscript list */
			delete_tuple( mpl, tuple );
		}
...
ELEMSET* eval_member_set /* returns reference, not value */
	...
	info->tuple = tuple;
#if 1											   /* 12/XII-2008 */
	if ( set->gadget != NULL && set->data == 0 ) { /* initialize the set with data from a plain set */
		saturate_set( mpl, set );
	}
#endif
	if ( set->data == 1 ) { /* check data, which are provided in the data section, but not
							   checked yet */
...
static int whole_set_func( MPL* mpl, void* info ) { /* this is auxiliary routine to work within domain scope */
	SET* set = (SET*)info;
...
double take_member_num( MPL* mpl,
						PARAMETER* par, /* not changed */
						TUPLE* tuple	/* not changed */
) {
	MEMBER* memb;
	...
	if ( memb != NULL ) { /* member exists, so just take its value */
		value = memb->value.num;
	} else if ( par->assign != NULL ) { /* compute value using assignment expression */
	...
	add: /* check that the value satisfies to all restrictions, assign
	...
	} else if ( par->option != NULL ) { /* compute default value */
		value = eval_numeric( mpl, par->option );
		goto add;
	} else if ( par->defval != NULL ) { /* take default value provided in the data section */
		...
		error( mpl, "no value for %s%s", par->name, format_tuple( mpl, '[', tuple ) );
	}
	return value;
}
...
struct eval_num_info { /* working info used by the routine eval_member_num */
	PARAMETER* par;
	/* model parameter  */
	TUPLE* tuple;
...
static void eval_num_func( MPL* mpl, void* _info ) { /* this is auxiliary routine to work within domain scope */
	struct eval_num_info* info = _info;
	if ( info->memb != NULL ) { /* checking call; check numeric value being assigned */
...
double eval_member_num( MPL* mpl,
						PARAMETER* par, /* not changed */
						TUPLE* tuple	/* not changed */
	...
	info->par = par;
	info->tuple = tuple;
	if ( par->data == 1 ) { /* check data, which are provided in the data section, but not
							   checked yet */
		/* save pointer to the last array member; note that during the
		   check new members may be added beyond the last member due to
		   references to the same parameter from default expression as
...
void check_value_sym( MPL* mpl,
	...
	for ( in = par->in, eqno = 1; in != NULL; in = in->next, eqno++ ) {
		...
		xassert( in->code != NULL );
		xassert( in->code->dim == 1 );
		dummy = expand_tuple( mpl, create_tuple( mpl ), copy_symbol( mpl, value ) );
		if ( !is_member( mpl, in->code, dummy ) )
...
struct eval_sym_info { /* working info used by the routine eval_member_sym */
	PARAMETER* par;
	/* model parameter */
	TUPLE* tuple;
	/* n-tuple, which defines parameter member */
...
static void eval_sym_func( MPL* mpl, void* _info ) { /* this is auxiliary routine to work within domain scope */
	struct eval_sym_info* info = _info;
	if ( info->memb != NULL ) { /* checking call; check symbolic value being assigned */
...
static int whole_par_func( MPL* mpl, void* info ) { /* this is auxiliary routine to work within domain scope */
...
ELEMVAR* take_member_var /* returns reference */
	...
	} else { /* member is referenced for the first time and therefore does
...
struct eval_var_info { /* working info used by the routine eval_member_var */
	VARIABLE* var;
	/* model variable */
	TUPLE* tuple;
...
static void eval_var_func( MPL* mpl, void* _info ) { /* this is auxiliary routine to work within domain scope */
...
static int whole_var_func( MPL* mpl, void* info ) { /* this is auxiliary routine to work within domain scope */
...
ELEMCON* take_member_con /* returns reference */
	...
	if ( memb != NULL ) { /* member exists, so just take the reference */
		...
		if ( con->lbnd == NULL && con->ubnd == NULL ) { /* objective has no bounds */
			double temp;
		...
		} else if ( con->lbnd != NULL && con->ubnd == NULL ) { /* constraint a * x + b >= c * y + d is transformed to the
			...
			refer->lbnd = 0.0;
			refer->ubnd = -temp;
		} else if ( con->lbnd == con->ubnd ) { /* constraint a * x + b = c * y + d is transformed to the
												  standard form a * x - c * y = d - b */
			double temp;
			xassert( con->type == A_CONSTRAINT );
			refer->form = linear_comb( mpl,
			...
			refer->form = remove_constant( mpl, refer->form, &temp );
			refer->lbnd = refer->ubnd = -temp;
		} else { /* ranged constraint c <= a * x + b <= d is transformed to
...
struct eval_con_info { /* working info used by the routine eval_member_con */
	CONSTRAINT* con;
	/* model constraint */
	TUPLE* tuple;
...
static void eval_con_func( MPL* mpl, void* _info ) { /* this is auxiliary routine to work within domain scope */
...
static int whole_con_func( MPL* mpl, void* info ) { /* this is auxiliary routine to work within domain scope */
...
struct iter_num_info { /* working info used by the routine iter_num_func */
	CODE* code;
...
static int iter_num_func( MPL* mpl, void* _info ) { /* this is auxiliary routine used to perform iterated operation
...
struct iter_log_info { /* working info used by the routine iter_log_func */
	CODE* code;
	/* pseudo-code for iterated operation to be performed */
	int value;
	/* resultant value */
...
static int iter_log_func( MPL* mpl, void* _info ) { /* this is auxiliary routine used to perform iterated operation
	...
	switch ( code->op ) {
		case O_TUPLE:
			/* make n-tuple */
			{
				...
				for ( e = code->arg.list; e != NULL; e = e->next )
					value = expand_tuple( mpl, value, eval_symbolic( mpl, e->x ) );
			}
			break;
		case O_CVTTUP:
			/* convert to 1-tuple */
...
struct iter_set_info { /* working info used by the routine iter_set_func */
	CODE* code;
	/* pseudo-code for iterated operation to be performed */
	ELEMSET* value;
	/* resultant value */
...
static int iter_set_func( MPL* mpl, void* _info ) { /* this is auxiliary routine used to perform iterated operation
	...
	switch ( info->code->op ) {
		...
		case O_BUILD:
			...
			add_tuple( mpl, info->value, get_domain_tuple( mpl, info->code->arg.loop.domain ) );
			break;
		default:
			xassert( info != info );
	}
	...
	switch ( code->op ) {
		...
		case O_DOTS:
			...
			value = create_arelset( mpl,
									eval_numeric( mpl, code->arg.arg.x ),
									eval_numeric( mpl, code->arg.arg.y ),
									code->arg.arg.z == NULL ? 1.0 : eval_numeric( mpl, code->arg.arg.z ) );
			break;
		case O_FORK:
			/* if-then-else */
			if ( eval_logical( mpl, code->arg.arg.x ) )
...
static void null_func( MPL* mpl, void* info ) { /* this is dummy routine used to enter the domain scope */
	xassert( mpl == mpl );
	xassert( info == NULL );
	return;
}
	...
	switch ( code->op ) {
		case O_MEMSET:
			   ...
			   is assigned to member of model set */
			{
				...
				for ( e = code->arg.set.list; e != NULL; e = e->next )
					temp = expand_tuple( mpl, temp, eval_symbolic( mpl, e->x ) );
...
struct iter_form_info { /* working info used by the routine iter_form_func */
	CODE* code;
	/* pseudo-code for iterated operation to be performed */
	FORMULA* value;
	/* resultant value */
...
static int iter_form_func( MPL* mpl, void* _info ) { /* this is auxiliary routine used to perform iterated operation
...
const char* mpl_tab_get_arg( TABDCA* dca, int k ) { /* returns pointer to k-th argument */
...
const char* mpl_tab_get_name( TABDCA* dca, int k ) { /* returns pointer to name of k-th field */
...
double mpl_tab_get_num( TABDCA* dca, int k ) { /* returns numeric value of k-th field */
	xassert( 1 <= k && k <= dca->nf );
	xassert( dca->type[k] == 'N' );
	return dca->num[k];
}

const char* mpl_tab_get_str( TABDCA* dca, int k ) { /* returns pointer to string value of k-th field */
	xassert( 1 <= k && k <= dca->nf );
...
void mpl_tab_set_num( TABDCA* dca, int k, double num ) { /* assign numeric value to k-th field */
	xassert( 1 <= k && k <= dca->nf );
	xassert( dca->type[k] == '?' );
	dca->type[k] = 'N';
	dca->num[k] = num;
...
void mpl_tab_set_str( TABDCA* dca, int k, const char* str ) { /* assign string value to k-th field */
	xassert( 1 <= k && k <= dca->nf );
	xassert( dca->type[k] == '?' );
...
void execute_table( MPL* mpl, TABLE* tab ) { /* execute table statement */
	TABARG* arg;
	TABFLD* fld;
	TABIN* in;
	TABOUT* out;
...
read_table:
	...
	for ( ;; ) {
		...
		for ( fld = tab->u.in.fld; fld != NULL; fld = fld->next ) {
			...
			switch ( dca->type[k] ) {
				case 'N':
					tup = expand_tuple( mpl, tup, create_symbol_num( mpl, dca->num[k] ) );
					break;
				case 'S':
					xassert( strlen( dca->str[k] ) <= MAX_LENGTH );
					tup = expand_tuple( mpl, tup, create_symbol_str( mpl, create_string( mpl, dca->str[k] ) ) );
					break;
				default:
					xassert( dca != dca );
			}
...
void free_dca( MPL* mpl ) { /* free table driver communucation area */
	TABDCA* dca = mpl->dca;
	int k;
	if ( dca != NULL ) {
		if ( dca->link != NULL )
...
void clean_table( MPL* mpl, TABLE* tab ) { /* clean table statement */
	TABARG* arg;
	TABOUT* out;
	/* clean string list */
	for ( arg = tab->arg; arg != NULL; arg = arg->next )
...
static int check_func( MPL* mpl, void* info ) { /* this is auxiliary routine to work within domain scope */
	CHECK* chk = (CHECK*)info;
	if ( !eval_logical( mpl, chk->code ) )
		error( mpl, "check%s failed", format_tuple( mpl, '[', get_domain_tuple( mpl, chk->domain ) ) );
	return 0;
}
...
void clean_check( MPL* mpl, CHECK* chk ) { /* clean subscript domain */
...
static void display_set( MPL* mpl, SET* set, MEMBER* memb ) { /* display member of model set */
...
static void display_par( MPL* mpl, PARAMETER* par, MEMBER* memb ) { /* display member of model parameter */
	switch ( par->type ) {
		case A_NUMERIC:
		case A_INTEGER:
		case A_BINARY:
...
static void display_var( MPL* mpl, VARIABLE* var, MEMBER* memb,
						 int suff ) { /* display member of model variable */
	if ( suff == DOT_NONE || suff == DOT_VAL )
		...
		write_text( mpl, "%s%s.status = %d\n", var->name, format_tuple( mpl, '[', memb->tuple ), memb->value.var->stat );
...
static void display_con( MPL* mpl, CONSTRAINT* con, MEMBER* memb,
						 int suff ) { /* display member of model constraint */
	if ( suff == DOT_NONE || suff == DOT_VAL )
		write_text( mpl, "%s%s.val = %.*g\n", con->name,
					format_tuple( mpl, '[', memb->tuple ), DBL_DIG,
					memb->value.con->prim );
		...
		write_text( mpl, "%s%s.status = %d\n", con->name, format_tuple( mpl, '[', memb->tuple ), memb->value.con->stat );
	else if ( suff == DOT_DUAL )
		write_text( mpl, "%s%s.dual = %.*g\n", con->name,
					format_tuple( mpl, '[', memb->tuple ), DBL_DIG,
					memb->value.con->dual );
...
static void display_memb( MPL* mpl, CODE* code ) { /* display member specified by pseudo-code */
	MEMBER memb;
	ARG_LIST* e;
	xassert( code->op == O_MEMNUM || code->op == O_MEMSYM || code->op == O_MEMSET || code->op == O_MEMVAR || code->op == O_MEMCON );
	memb.tuple = create_tuple( mpl );
	for ( e = code->arg.par.list; e != NULL; e = e->next )
		memb.tuple = expand_tuple( mpl, memb.tuple, eval_symbolic( mpl, e->x ) );
	switch ( code->op ) {
		case O_MEMNUM:
		...
		case O_MEMVAR:
			memb.value.var = eval_member_var( mpl, code->arg.var.var,
											  memb.tuple );
			display_var( mpl, code->arg.var.var, &memb, code->arg.var.suff );
			break;
		case O_MEMCON:
			memb.value.con = eval_member_con( mpl, code->arg.con.con,
											  memb.tuple );
			display_con( mpl, code->arg.con.con, &memb, code->arg.con.suff );
			break;
		default:
			xassert( code != code );
	}
...
static void display_code( MPL* mpl, CODE* code ) { /* display value of expression */
	switch ( code->type ) {
		case A_NUMERIC:
			/* numeric value */
			{
...
static int display_func( MPL* mpl, void* info ) { /* this is auxiliary routine to work within domain scope */
	DISPLAY* dpy = (DISPLAY*)info;
	DISPLAY1* entry;
	for ( entry = dpy->list; entry != NULL; entry = entry->next ) {
		if ( entry->type == A_INDEX ) { /* dummy index */
			DOMAIN_SLOT* slot = entry->u.slot;
			write_text( mpl, "%s = %s\n", slot->name,
						format_symbol( mpl, slot->value ) );
		} else if ( entry->type == A_SET ) { /* model set */
			SET* set = entry->u.set;
			MEMBER* memb;
			if ( set->assign != NULL ) { /* the set has assignment expression; evaluate all its
											members over entire domain */
				eval_whole_set( mpl, set );
			} else {										   /* the set has no assignment expression; refer to its
		...
		} else if ( entry->type == A_PARAMETER ) { /* model parameter */
			PARAMETER* par = entry->u.par;
			MEMBER* memb;
			if ( par->assign != NULL ) { /* the parameter has an assignment expression; evaluate
											all its member over entire domain */
				eval_whole_par( mpl, par );
			} else { /* the parameter has no assignment expression; refer to
		...
		} else if ( entry->type == A_VARIABLE ) { /* model variable */
		...
		} else if ( entry->type == A_CONSTRAINT ) { /* model constraint */
		...
		} else if ( entry->type == A_EXPRESSION ) { /* expression */
...
void clean_display( MPL* mpl, DISPLAY* dpy ) {
	...
	/* clean display list */
	for ( d = dpy->list; d != NULL; d = d->next ) { /* clean pseudo-code for computing expression */
...
static int printf_func( MPL* mpl, void* info ) { /* this is auxiliary routine to work within domain scope */
	...
	for ( c = fmt; *c != '\0'; c++ ) {
		if ( *c == '%' ) { /* scan format specifier */
			...
			save = *( c + 1 ), *( c + 1 ) = '\0';
			if ( *c == 'd' || *c == 'i' || *c == 'e' || *c == 'E' ||
				 *c == 'f' || *c == 'F' || *c == 'g' || *c == 'G' ) { /* the specifier requires numeric value */
				double value;
				xassert( entry != NULL );
			...
			} else if ( *c == 's' ) { /* the specifier requires symbolic value */
				char value[MAX_LENGTH + 1];
				switch ( entry->code->type ) {
					case A_NUMERIC:
						sprintf( value, "%.*g", DBL_DIG, eval_numeric( mpl, entry->code ) );
						break;
					case A_LOGICAL:
						if ( eval_logical( mpl, entry->code ) )
							strcpy( value, "T" );
			...
			*( c + 1 ) = save;
			entry = entry->next;
		} else if ( *c == '\\' ) { /* write some control character */
			c++;
			if ( *c == 't' )
				print_char( mpl, '\t' );
			else if ( *c == 'n' )
				print_char( mpl, '\n' );
#if 1								 /* 28/X-2010 */
			else if ( *c == '\0' ) { /* format string ends with backslash */
				error( mpl,
				...
				print_char( mpl, *c );
		} else { /* write character without formatting */
			print_char( mpl, *c );
		}
	}
	return 0;
...
#else
void execute_printf( MPL* mpl, PRINTF* prt ) {
	if ( prt->fname == NULL ) { /* switch to the standard output */
		if ( mpl->prt_fp != NULL ) {
			glp_close( mpl->prt_fp ), mpl->prt_fp = NULL;
			xfree( mpl->prt_file ), mpl->prt_file = NULL;
		}
	} else { /* evaluate file name string */
		SYMBOL* sym;
...
void clean_printf( MPL* mpl, PRINTF* prt ) {
	...
	/* clean printf list */
	for ( p = prt->list; p != NULL; p = p->next ) { /* clean pseudo-code for computing value to be printed */
...
static int for_func( MPL* mpl, void* info ) { /* this is auxiliary routine to work within domain scope */
	FOR* fur = (FOR*)info;
	STATEMENT *stmt, *save;
	save = mpl->stmt;
...
void clean_statement( MPL* mpl, STATEMENT* stmt ) {
	switch ( stmt->type ) {
		case A_SET:
			clean_set( mpl, stmt->u.set );
			break;
		case A_PARAMETER:
			clean_parameter( mpl, stmt->u.par );
			break;
		case A_VARIABLE:
			clean_variable( mpl, stmt->u.var );
			break;
		case A_CONSTRAINT:
			clean_constraint( mpl, stmt->u.con );
			break;
#if 1 /* 11/II-2008 */
		case A_TABLE:
			clean_table( mpl, stmt->u.tab );
			break;
#endif
		case A_SOLVE:
			break;
		case A_CHECK:
			clean_check( mpl, stmt->u.chk );
			break;
		case A_DISPLAY:
			clean_display( mpl, stmt->u.dpy );
			break;
		case A_PRINTF:
			clean_printf( mpl, stmt->u.prt );
			break;
		case A_FOR:
			clean_for( mpl, stmt->u.fur );
			break;
		default:
			xassert( stmt != stmt );
	}
