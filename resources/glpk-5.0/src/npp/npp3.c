...
int npp_empty_row( NPP* npp, NPPROW* p ) { /* process empty row */
	double eps = 1e-3;
...
int npp_empty_col( NPP* npp, NPPCOL* q ) { /* process empty column */
	...
	info->q = q->j;
	/* fix the column */
	if ( q->lb == -DBL_MAX && q->ub == +DBL_MAX ) { /* free column */
		info->stat = GLP_NF;
		q->lb = q->ub = 0.0;
	...
	else if ( q->lb != q->ub ) { /* double-bounded column */
		if ( q->coef >= +DBL_EPSILON ) goto lo;
		if ( q->coef <= -DBL_EPSILON ) goto up;
		if ( fabs( q->lb ) <= fabs( q->ub ) )
			goto lo;
		else
			goto up;
	} else { /* fixed column */
...
static int rcv_empty_col( NPP* npp, void* _info ) { /* recover empty column */
...
int npp_implied_value( NPP* npp, NPPCOL* q, double s ) { /* process implied column value */
	double eps, nint;
	xassert( npp == npp );
...
struct eq_singlet { /* row singleton (equality constraint) */
	int p;
...
int npp_eq_singlet( NPP* npp, NPPROW* p ) { /* process row singleton (equality constraint) */
...
static int rcv_eq_singlet( NPP* npp, void* _info ) { /* recover row singleton (equality constraint) */
	struct eq_singlet* info = _info;
	NPPLFE* lfe;
	double temp;
	if ( npp->sol == GLP_SOL ) { /* column q must be already recovered as GLP_NS */
	...
	if ( npp->sol != GLP_MIP ) { /* compute multiplier for row p with formula (3) */
		temp = info->c;
...
int npp_implied_lower( NPP* npp, NPPCOL* q, double l ) { /* process implied column lower bound */
	int ret;
	double eps, nint;
	xassert( npp == npp );
	/* column must not be fixed */
...
struct ineq_singlet { /* row singleton (inequality constraint) */
	int p;
	/* row reference number */
	int q;
	/* column reference number */
...
int npp_ineq_singlet( NPP* npp, NPPROW* p ) { /* process row singleton (inequality constraint) */
	...
	if ( uu == +DBL_MAX )
		ub_changed = 0;
	else if ( lb_changed == 3 ) { /* column was fixed on its upper bound due to l'[q] = u[q] */
...
static int rcv_ineq_singlet( NPP* npp, void* _info ) { /* recover row singleton (inequality constraint) */
	...
	if ( npp->sol == GLP_SOL ) {				/* recover basic solution */
		if ( npp->c_stat[info->q] == GLP_BS ) { /* column q is basic, so row p is inactive */
			npp->r_stat[info->p] = GLP_BS;
			npp->r_pi[info->p] = 0.0;
		} else if ( npp->c_stat[info->q] == GLP_NL )
		nl : {						  /* column q is non-basic with lower bound active */
			if ( info->lb_changed ) { /* it is implied bound, so actually row p is active
										 while column q is basic */
				npp->r_stat[info->p] =
					  (char)( info->apq > 0.0 ? GLP_NL : GLP_NU );
				npp->c_stat[info->q] = GLP_BS;
				npp->r_pi[info->p] = lambda / info->apq;
			} else { /* it is original bound, so row p is inactive */
				npp->r_stat[info->p] = GLP_BS;
				npp->r_pi[info->p] = 0.0;
			}
		}
				...
				if ( info->ub_changed ) { /* it is implied bound, so actually row p is active
											 while column q is basic */
					npp->r_stat[info->p] =
		...
		else if ( npp->c_stat[info->q] == GLP_NS ) { /* column q is non-basic and fixed; note, however, that in
														in the original problem it is non-fixed */
			if ( lambda > +1e-7 ) {
				if ( info->apq > 0.0 && info->lb != -DBL_MAX ||
					 info->apq < 0.0 && info->ub != +DBL_MAX ||
					 !info->lb_changed ) { /* either corresponding bound of row p exists or
			...
			if ( lambda < -1e-7 ) {
				if ( info->apq > 0.0 && info->ub != +DBL_MAX ||
					 ...
					 !info->ub_changed ) { /* either corresponding bound of row p exists or
			...
			if ( info->lb != -DBL_MAX && info->ub == +DBL_MAX ) { /* row lower bound exists, but upper bound doesn't */
				npp->r_stat[info->p] = GLP_NL;
			} else if ( info->lb == -DBL_MAX && info->ub != +DBL_MAX ) { /* row upper bound exists, but lower bound doesn't */
				npp->r_stat[info->p] = GLP_NU;
			} else if ( info->lb != -DBL_MAX && info->ub != +DBL_MAX ) { /* both row lower and upper bounds exist */
	...
	if ( npp->sol == GLP_IPT ) { /* recover interior-point solution */
		if ( lambda > +DBL_EPSILON && info->lb_changed ||
			 lambda < -DBL_EPSILON && info->ub_changed ) { /* actually row p has corresponding active bound */
			npp->r_pi[info->p] = lambda / info->apq;
		} else { /* either bounds of column q are both inactive or its
...
struct implied_slack { /* column singleton (implied slack variable) */
	int p;
	/* row reference number */
	int q;
	/* column reference number */
...
void npp_implied_slack( NPP* npp, NPPCOL* q ) { /* process column singleton (implied slack variable) */
...
static int rcv_implied_slack( NPP* npp, void* _info ) { /* recover column singleton (implied slack variable) */
	struct implied_slack* info = _info;
	NPPLFE* lfe;
	double temp;
	if ( npp->sol == GLP_SOL ) { /* assign statuses to row p and column q */
	...
	if ( npp->sol != GLP_MIP ) { /* compute multiplier for row p */
...
struct implied_free { /* column singleton (implied free variable) */
	int p;
	/* row reference number */
	char stat;
	/* row status:
...
int npp_implied_free( NPP* npp, NPPCOL* q ) { /* process column singleton (implied free variable) */
	...
	if ( pi > +DBL_EPSILON ) { /* lower bound L[p] must be active */
		if ( p->lb != -DBL_MAX )
		nl : {
			info->stat = GLP_NL;
			p->ub = p->lb;
	...
	} else if ( pi < -DBL_EPSILON ) { /* upper bound U[p] must be active */
		if ( p->ub != +DBL_MAX )
		nu : {
			info->stat = GLP_NU;
			p->lb = p->ub;
	...
	} else { /* any bound (either L[p] or U[p]) can be made active  */
		if ( p->ub == +DBL_MAX ) {
			xassert( p->lb != -DBL_MAX );
			goto nl;
		}
...
static int rcv_implied_free( NPP* npp, void* _info ) { /* recover column singleton (implied free variable) */
...
struct eq_doublet { /* row doubleton (equality constraint) */
	int p;
	/* row reference number */
	double apq;
	/* constraint coefficient a[p,q] */
...
NPPCOL* npp_eq_doublet( NPP* npp, NPPROW* p ) { /* process row doubleton (equality constraint) */
	...
	/* choose column to be eliminated */
	{
		...
		if ( fabs( a2->val ) < 0.001 * fabs( a1->val ) ) { /* only first column can be eliminated, because second one
															  has too small constraint coefficient */
			apq = a1, apr = a2;
		} else if ( fabs( a1->val ) < 0.001 * fabs( a2->val ) ) { /* only second column can be eliminated, because first one
																	 has too small constraint coefficient */
			apq = a2, apr = a1;
		} else {																/* both columns are appropriate; choose that one which is
																				   shorter to minimize fill-in */
			if ( npp_col_nnz( npp, a1->col ) <= npp_col_nnz( npp, a2->col ) ) { /* first column is shorter */
				apq = a1, apr = a2;
			} else { /* second column is shorter */
				apq = a2, apr = a1;
			}
		}
	}
...
static int rcv_eq_doublet( NPP* npp, void* _info ) { /* recover row doubleton (equality constraint) */
	...
	if ( npp->sol != GLP_MIP ) { /* compute value of multiplier for row p; see (14) */
...
struct forcing_col { /* column fixed on its bound by forcing row */
	int j;
	/* column reference number */
	char stat;
...
struct forcing_row { /* forcing row */
	int p;
	/* row reference number */
	char stat;
...
int npp_forcing_row( NPP* npp, NPPROW* p, int at ) { /* process forcing row */
	...
	info->p = p->i;
	if ( p->lb == p->ub ) { /* equality constraint */
		info->stat = GLP_NS;
	} else if ( at == 0 ) { /* inequality constraint; case L[p] = U'[p] */
		info->stat = GLP_NL;
		xassert( p->lb != -DBL_MAX );
	} else /* at == 1 */
	...
	for ( apj = p->ptr; apj != NULL; apj = apj->r_next ) { /* column j has non-zero coefficient in the forcing row */
		j = apj->col;
		...
		/* fix column j */
		if ( at == 0 && apj->val < 0.0 || at != 0 && apj->val > 0.0 ) { /* at its lower bound */
			if ( npp->sol != GLP_MIP )
				col->stat = GLP_NL;
			xassert( j->lb != -DBL_MAX );
			j->ub = j->lb;
		} else { /* at its upper bound */
			if ( npp->sol != GLP_MIP )
				col->stat = GLP_NU;
...
static int rcv_forcing_row( NPP* npp, void* _info ) { /* recover forcing row */
	...
	for ( col = info->ptr; col != NULL; col = col->next ) {
		d = col->c; /* d[j] */
		temp = fabs( d / col->a );
		if ( col->stat == GLP_NL ) { /* column j has active lower bound */
			if ( d < 0.0 && big < temp )
				piv = col, big = temp;
		} else if ( col->stat == GLP_NU ) { /* column j has active upper bound */
			if ( d > 0.0 && big < temp )
				piv = col, big = temp;
		} else {
			npp_error();
	...
	if ( piv != NULL ) { /* correct solution; row p becomes active constraint while
...
int npp_analyze_row( NPP* npp, NPPROW* p ) { /* perform general row analysis */
	NPPAIJ* aij;
	int ret = 0x00;
	double l, u, eps;
	xassert( npp == npp );
	...
	if ( p->ub != +DBL_MAX ) {
		...
		if ( p->ub + eps < u ) { /* check if the upper bound is forcing */
			if ( p->ub - eps >= l )
				ret |= 0x10;
			else
				ret |= 0x20;
...
struct inactive_bound { /* row inactive bound */
	int p;
	/* row reference number */
	char stat;
...
void npp_inactive_bound( NPP* npp, NPPROW* p, int which ) { /* remove row lower/upper inactive bound */
	struct inactive_bound* info;
	if ( npp->sol == GLP_SOL ) { /* create transformation stack entry */
		info = npp_push_tse( npp,
...
static int rcv_inactive_bound( NPP* npp, void* _info ) { /* recover row status */
...
void npp_implied_bounds( NPP* npp, NPPROW* p ) {
	...
	if ( p->lb != -DBL_MAX ) {
		...
		if ( apk == NULL ) { /* temp = L[p] - U'[p] */
			for ( apj = p->ptr; apj != NULL; apj = apj->r_next ) {
				if ( apj->val >= +eps ) { /* l'[j] := u[j] + (L[p] - U'[p]) / a[p,j] */
					apj->col->ll.ll = apj->col->ub + temp / apj->val;
				} else if ( apj->val <= -eps ) { /* u'[j] := l[j] + (L[p] - U'[p]) / a[p,j] */
					apj->col->uu.uu = apj->col->lb + temp / apj->val;
				}
			}
		} else {					  /* temp = L[p,k] */
			if ( apk->val >= +eps ) { /* l'[k] := L[p,k] / a[p,k] */
				apk->col->ll.ll = temp / apk->val;
			} else if ( apk->val <= -eps ) { /* u'[k] := L[p,k] / a[p,k] */
	...
	if ( p->ub != +DBL_MAX ) {
		...
		/* compute column implied bounds */
		if ( apk == NULL ) { /* temp = U[p] - L'[p] */
			for ( apj = p->ptr; apj != NULL; apj = apj->r_next ) {
				if ( apj->val >= +eps ) { /* u'[j] := l[j] + (U[p] - L'[p]) / a[p,j] */
					apj->col->uu.uu = apj->col->lb + temp / apj->val;
				} else if ( apj->val <= -eps ) { /* l'[j] := u[j] + (U[p] - L'[p]) / a[p,j] */
					apj->col->ll.ll = apj->col->ub + temp / apj->val;
				}
			}
		} else {					  /* temp = U[p,k] */
			if ( apk->val >= +eps ) { /* u'[k] := U[p,k] / a[p,k] */
				apk->col->uu.uu = temp / apk->val;
			} else if ( apk->val <= -eps ) { /* l'[k] := U[p,k] / a[p,k] */
				apk->col->ll.ll = temp / apk->val;
			}
		}
	skip2:;
