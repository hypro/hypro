...
struct free_row { /* free (unbounded) row */
	int p;
...
void npp_free_row( NPP* npp, NPPROW* p ) { /* process free (unbounded) row */
...
static int rcv_free_row( NPP* npp, void* _info ) { /* recover free (unbounded) row */
...
struct ineq_row { /* inequality constraint row */
	int p;
	/* row reference number */
	int s;
...
void npp_geq_row( NPP* npp, NPPROW* p ) { /* process row of 'not less than' type */
...
static int rcv_geq_row( NPP* npp, void* _info ) { /* recover row of 'not less than' type */
...
void npp_leq_row( NPP* npp, NPPROW* p ) { /* process row of 'not greater than' type */
	struct ineq_row* info;
	NPPCOL* s;
...
static int rcv_leq_row( NPP* npp, void* _info ) { /* recover row of 'not greater than' type */
	struct ineq_row* info = _info;
	if ( npp->sol == GLP_SOL ) {
		if ( npp->r_stat[info->p] == GLP_BS ) {
			if ( npp->c_stat[info->s] == GLP_BS ) {
...
struct free_col { /* free (unbounded) column */
	int q;
...
void npp_free_col( NPP* npp, NPPCOL* q ) { /* process free (unbounded) column */
...
static int rcv_free_col( NPP* npp, void* _info ) { /* recover free (unbounded) column */
...
void npp_lbnd_col( NPP* npp, NPPCOL* q ) { /* process column with (non-zero) lower bound */
	struct bnd_col* info;
	NPPROW* i;
	NPPAIJ* aij;
...
static int rcv_lbnd_col( NPP* npp, void* _info ) { /* recover column with (non-zero) lower bound */
...
void npp_ubnd_col( NPP* npp, NPPCOL* q ) { /* process column with upper bound */
	struct bnd_col* info;
	NPPROW* i;
	NPPAIJ* aij;
...
static int rcv_ubnd_col( NPP* npp, void* _info ) { /* recover column with upper bound */
	struct bnd_col* info = _info;
	if ( npp->sol == GLP_BS ) {
		if ( npp->c_stat[info->q] == GLP_BS )
			npp->c_stat[info->q] = GLP_BS;
...
struct dbnd_col { /* double-bounded column */
	int q;
...
void npp_dbnd_col( NPP* npp, NPPCOL* q ) { /* process non-negative column with upper bound */
...
static int rcv_dbnd_col( NPP* npp, void* _info ) { /* recover non-negative column with upper bound */
...
void npp_fixed_col( NPP* npp, NPPCOL* q ) { /* process fixed column */
...
static int rcv_fixed_col( NPP* npp, void* _info ) { /* recover fixed column */
...
struct make_equality { /* row with almost identical bounds */
	int p;
	/* row reference number */
};
...
int npp_make_equality( NPP* npp, NPPROW* p ) { /* process row with almost identical bounds */
...
int rcv_make_equality( NPP* npp, void* _info ) { /* recover row with almost identical bounds */
...
struct make_fixed { /* column with almost identical bounds */
	int q;
...
int npp_make_fixed( NPP* npp, NPPCOL* q ) { /* process column with almost identical bounds */
...
static int rcv_make_fixed( NPP* npp, void* _info ) { /* recover column with almost identical bounds */
	...
	if ( npp->sol == GLP_SOL ) {
		if ( npp->c_stat[info->q] == GLP_BS )
			npp->c_stat[info->q] = GLP_BS;
		else if ( npp->c_stat[info->q] == GLP_NS ) { /* compute multiplier for column q with formula (6) */
			lambda = info->c;
