...
void npp_sat_free_row( NPP* npp, NPPROW* p ) { /* the row should be free */
...
struct sat_fixed_col { /* fixed column */
	int q;
...
int npp_sat_is_cover_ineq( NPP* npp, NPPROW* row ) {
	xassert( npp == npp );
	if ( row->lb != -DBL_MAX && row->ub == +DBL_MAX ) {				   /* row is inequality of '>=' type */
		if ( npp_sat_is_bin_comb( npp, row ) ) {					   /* row is a binary combination */
			if ( row->lb == 1.0 - npp_sat_num_neg_coef( npp, row ) ) { /* row has the form (2) */
				return 1;
			}
		}
	} else if ( row->lb == -DBL_MAX && row->ub != +DBL_MAX ) {		   /* row is inequality of '<=' type */
		if ( npp_sat_is_bin_comb( npp, row ) ) {					   /* row is a binary combination */
			if ( row->ub == npp_sat_num_pos_coef( npp, row ) - 1.0 ) { /* row has the form (3) */
				return 2;
			}
		}
	}
...
int npp_sat_is_pack_ineq( NPP* npp, NPPROW* row ) {
	...
	if ( row->lb == -DBL_MAX && row->ub != +DBL_MAX ) {				   /* row is inequality of '<=' type */
		if ( npp_sat_is_bin_comb( npp, row ) ) {					   /* row is a binary combination */
			if ( row->ub == 1.0 - npp_sat_num_neg_coef( npp, row ) ) { /* row has the form (2) */
				return 1;
			}
		}
	} else if ( row->lb != -DBL_MAX && row->ub == +DBL_MAX ) {		   /* row is inequality of '>=' type */
		if ( npp_sat_is_bin_comb( npp, row ) ) {					   /* row is a binary combination */
			if ( row->lb == npp_sat_num_pos_coef( npp, row ) - 1.0 ) { /* row has the form (3) */
				return 2;
			}
		}
	}
...
int npp_sat_is_partn_eq( NPP* npp, NPPROW* row ) {
	xassert( npp == npp );
	if ( row->lb == row->ub ) {										   /* row is equality constraint */
		if ( npp_sat_is_bin_comb( npp, row ) ) {					   /* row is a binary combination */
			if ( row->lb == 1.0 - npp_sat_num_neg_coef( npp, row ) ) { /* row has the form (2) */
				return 1;
			}
...
void npp_sat_encode_pack( NPP* npp, NPPROW* row ) {
	...
	for ( aij = row->ptr; aij != NULL; aij = aij->r_next ) {		/* due to symmetry only one of inequalities t[j] + t[k] <= 1
																	   and t[k] <= t[j] <= 1 can be considered */
		for ( aik = aij->r_next; aik != NULL; aik = aik->r_next ) { /* create edge packing inequality (2) */
			rrr = npp_add_row( npp );
			rrr->lb = -DBL_MAX, rrr->ub = 1.0;
			npp_add_aij( npp, rrr, aij->col, aij->val );
			if ( aij->val < 0.0 )
...
void npp_sat_encode_sum2( NPP* npp, NPPLSE* set, NPPSED* sed ) {
	...
	for ( x = 0; x <= 1; x++ ) {
		for ( y = 0; y <= 1; y++ ) {
			for ( s = 0; s <= 1; s++ ) {
				if ( ( x + y ) % 2 != s ) { /* generate CNF clause to disable infeasible
											   combination */
					row = npp_add_row( npp );
	...
	for ( x = 0; x <= 1; x++ ) {
		for ( y = 0; y <= 1; y++ ) {
			for ( c = 0; c <= 1; c++ ) {
				if ( ( x + y ) / 2 != c ) { /* generate CNF clause to disable infeasible
											   combination */
					row = npp_add_row( npp );
					row->lb = 1.0, row->ub = +DBL_MAX;
					if ( x == sed->x.neg )
...
void npp_sat_encode_sum3( NPP* npp, NPPLSE* set, NPPSED* sed ) {
	...
	for ( x = 0; x <= 1; x++ ) {
		for ( y = 0; y <= 1; y++ ) {
			for ( z = 0; z <= 1; z++ ) {
				for ( s = 0; s <= 1; s++ ) {
					if ( ( x + y + z ) % 2 != s ) { /* generate CNF clause to disable infeasible
													   combination */
						row = npp_add_row( npp );
						row->lb = 1.0, row->ub = +DBL_MAX;
						if ( x == sed->x.neg )
	...
	for ( x = 0; x <= 1; x++ ) {
		for ( y = 0; y <= 1; y++ ) {
			for ( z = 0; z <= 1; z++ ) {
				for ( c = 0; c <= 1; c++ ) {
					if ( ( x + y + z ) / 2 != c ) { /* generate CNF clause to disable infeasible
													   combination */
						row = npp_add_row( npp );
						row->lb = 1.0, row->ub = +DBL_MAX;
						if ( x == sed->x.neg )
...
static NPPLSE* remove_lse( NPP* npp, NPPLSE* set, NPPCOL* col ) { /* remove specified literal from specified literal set */
...
int npp_sat_encode_sum_ax( NPP* npp, NPPROW* row, NPPLIT y[] ) {
	...
	for ( k = 1; k <= n; k++ ) { /* reduce J[k] and set y[k] */
		for ( ;; ) {
			if ( set[k] == NULL ) { /* J[k] is empty */
				/* set y[k] to constant false */
				y[k].col = NULL, y[k].neg = 0;
				break;
			}
			if ( set[k]->next == NULL ) { /* J[k] contains one literal */
				/* set y[k] to that literal */
				y[k] = set[k]->lit;
				dmp_free_atom( npp->pool, set[k], sizeof( NPPLSE ) );
				break;
			}
			if ( set[k]->next->next == NULL ) { /* J[k] contains two literals */
				/* apply transformation (7) */
				npp_sat_encode_sum2( npp, set[k], &sed );
			} else { /* J[k] contains at least three literals */
...
int npp_sat_normalize_clause( NPP* npp, int size, NPPLIT lit[] ) {
	...
	for ( k = 1; k <= size; k++ ) {
		for ( j = 1; j <= new_size; j++ ) {
			if ( lit[k].col == lit[j].col ) {	  /* lit[k] refers to the same variable as lit[j], which
													 is already included in the resulting clause */
				if ( lit[k].neg == lit[j].neg ) { /* ignore lit[k] due to the idempotent law */
					goto skip;
				} else { /* lit[k] is NOT lit[j]; the clause is equivalent to
							the value true */
					return -1;
				}
			}
...
int npp_sat_encode_geq( NPP* npp, int n, NPPLIT y[], int rhs ) {
	...
	for ( k = 1, temp = rhs; k <= n; k++, temp >>= 1 )
		b[k] = temp & 1;
	if ( temp != 0 ) { /* b >= 2**n; the constraint (1) is infeasible */
		return 1;
	}
	/* main transformation loop */
	for ( k = 1; k <= n; k++ ) { /* build the clause (5) for current k */
		size = 0;				 /* clause size = number of literals */
		/* add literal y[k] >= b[k] */
		if ( b[k] == 0 ) { /* b[k] = 0 -> the literal is true */
			goto skip;
		} else if ( y[k].col == NULL ) { /* y[k] = 0, b[k] = 1 -> the literal is false */
			xassert( y[k].neg == 0 );
		} else { /* add literal y[k] = 1 */
			lit[++size] = y[k];
		}
		for ( j = k + 1; j <= n; j++ ) { /* add literal y[j] != b[j] */
			if ( y[j].col == NULL ) {
				xassert( y[j].neg == 0 );
				if ( b[j] == 0 ) { /* y[j] = 0, b[j] = 0 -> the literal is false */
					continue;
				} else { /* y[j] = 0, b[j] = 1 -> the literal is true */
					goto skip;
				}
			} else {
				lit[++size] = y[j];
		...
		if ( size < 0 ) { /* the clause is equivalent to the value true */
			goto skip;
		}
		if ( size == 0 ) { /* the clause is equivalent to the value false; this means
...
int npp_sat_encode_leq( NPP* npp, int n, NPPLIT y[], int rhs ) {
	...
	for ( k = 1, temp = rhs; k <= n; k++, temp >>= 1 )
		b[k] = temp & 1;
	if ( temp != 0 ) { /* b >= 2**n; the constraint (1) is redundant */
		return 0;
	}
	/* main transformation loop */
	for ( k = 1; k <= n; k++ ) { /* build the clause (5) for current k */
		size = 0;				 /* clause size = number of literals */
		/* add literal y[k] <= b[k] */
		if ( b[k] == 1 ) { /* b[k] = 1 -> the literal is true */
			goto skip;
		} else if ( y[k].col == NULL ) { /* y[k] = 0, b[k] = 0 -> the literal is true */
			xassert( y[k].neg == 0 );
			goto skip;
		} else { /* add literal y[k] = 0 */
			lit[++size] = y[k];
			lit[size].neg = 1 - lit[size].neg;
		}
		for ( j = k + 1; j <= n; j++ ) { /* add literal y[j] != b[j] */
...
int npp_sat_encode_prob( NPP* npp ) {
	...
	for ( row = npp->r_tail; row != NULL; row = prev_row ) {
		...
		if ( ret != 0 ) { /* row is covering inequality */
			cover++;
		...
		if ( ret != 0 ) { /* row is partitioning equality */
			NPPROW* cov;
			NPPAIJ* aij;
			partn++;
		...
		if ( ret != 0 ) { /* row is packing inequality */
			NPPROW* rrr;
			...
			for ( ;; ) { /* determine the number of literals in the remaining
							inequality */
