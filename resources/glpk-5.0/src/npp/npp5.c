...
void npp_clean_prob( NPP* npp ) { /* perform initial LP/MIP processing */
	NPPROW *row, *next_row;
	NPPCOL *col, *next_col;
	int ret;
	xassert( npp == npp );
	...
	for ( row = npp->r_head; row != NULL; row = next_row ) {
		next_row = row->next;
		if ( row->lb == -DBL_MAX && row->ub == +DBL_MAX ) { /* process free row */
#ifdef GLP_DEBUG
			xprintf( "1" );
#endif
	...
	for ( row = npp->r_head; row != NULL; row = next_row ) {
		...
		if ( row->lb != -DBL_MAX && row->ub != +DBL_MAX &&
			...
			if ( ret == 0 )
				;
			else if ( ret == 1 ) { /* row was replaced by equality constraint */
#ifdef GLP_DEBUG
				xprintf( "2" );
#endif
			} else
	...
	for ( col = npp->c_head; col != NULL; col = next_col ) {
		next_col = col->next;
		if ( col->lb == col->ub ) { /* process fixed column */
#ifdef GLP_DEBUG
			xprintf( "3" );
#endif
	...
	for ( col = npp->c_head; col != NULL; col = next_col ) {
		...
		if ( col->lb != -DBL_MAX && col->ub != +DBL_MAX &&
			...
			if ( ret == 0 )
				;
			else if ( ret == 1 ) { /* column was replaced by fixed column; process it */
#ifdef GLP_DEBUG
				xprintf( "4" );
#endif
				npp_fixed_col( npp, col );
...
int npp_process_row( NPP* npp, NPPROW* row, int hard ) { /* perform basic row processing */
	NPPCOL* col;
	...
	if ( row->ptr == NULL ) { /* empty row */
		ret = npp_empty_row( npp, row );
		if ( ret == 0 ) { /* row was deleted */
#ifdef GLP_DEBUG
			xprintf( "A" );
#endif
			return 0;
		} else if ( ret == 1 ) { /* primal infeasibility */
			return GLP_ENOPFS;
		} else
			xassert( ret != ret );
	}
	if ( row->ptr->r_next == NULL ) { /* row singleton */
		col = row->ptr->col;
		if ( row->lb == row->ub ) { /* equality constraint */
			ret = npp_eq_singlet( npp, row );
			if ( ret == 0 ) { /* column was fixed, row was deleted */
#ifdef GLP_DEBUG
				xprintf( "B" );
#endif
			...
			if ( 0 <= ret && ret <= 3 ) { /* row was deleted */
#ifdef GLP_DEBUG
				xprintf( "C" );
#endif
				...
				if ( ret >= 2 ) { /* column bounds changed significantly or column was
									 fixed */
					/* activate rows affected by column */
					for ( aij = col->ptr; aij != NULL; aij = aij->c_next )
						npp_activate_row( npp, aij->row );
				}
				if ( ret == 3 ) { /* column was fixed; process it */
#ifdef GLP_DEBUG
					xprintf( "D" );
#endif
					npp_fixed_col( npp, col );
				...
				return 0;
			} else if ( ret == 4 ) { /* primal infeasibility */
				return GLP_ENOPFS;
			} else
				xassert( ret != ret );
		}
	...
	if ( ret == 0x33 ) { /* row bounds are inconsistent with column bounds */
		return GLP_ENOPFS;
	}
	if ( ( ret & 0x0F ) == 0x00 ) {	 /* row lower bound does not exist or redundant */
		if ( row->lb != -DBL_MAX ) { /* remove redundant row lower bound */
#ifdef GLP_DEBUG
			xprintf( "F" );
#endif
			npp_inactive_bound( npp, row, 0 );
		}
	} else if ( ( ret & 0x0F ) == 0x01 ) { /* row lower bound can be active */
										   /* see below */
	} else if ( ( ret & 0x0F ) == 0x02 ) { /* row lower bound is a forcing bound */
#ifdef GLP_DEBUG
		xprintf( "G" );
#endif
		/* process forcing row */
		...
		fixup : {												  /* columns were fixed, row was made free */
			for ( aij = row->ptr; aij != NULL; aij = next_aij ) { /* process column fixed by forcing row */
#ifdef GLP_DEBUG
				xprintf( "H" );
#endif
				col = aij->col;
		...
		xassert( ret != ret );
	if ( ( ret & 0xF0 ) == 0x00 ) {	 /* row upper bound does not exist or redundant */
		if ( row->ub != +DBL_MAX ) { /* remove redundant row upper bound */
#ifdef GLP_DEBUG
			xprintf( "I" );
#endif
			npp_inactive_bound( npp, row, 1 );
		}
	} else if ( ( ret & 0xF0 ) == 0x10 ) { /* row upper bound can be active */
										   /* see below */
	} else if ( ( ret & 0xF0 ) == 0x20 ) { /* row upper bound is a forcing bound */
#ifdef GLP_DEBUG
		xprintf( "J" );
#endif
		/* process forcing row */
		...
		xassert( ret != ret );
	if ( row->lb == -DBL_MAX && row->ub == +DBL_MAX ) { /* row became free due to redundant bounds removal */
#ifdef GLP_DEBUG
		xprintf( "K" );
#endif
	...
	if ( npp->sol == GLP_MIP && hard ) { /* improve current column bounds (optional) */
...
int npp_improve_bounds( NPP* npp, NPPROW* row, int flag ) { /* improve current column bounds */
	NPPCOL* col;
	NPPAIJ *aij, *next_aij, *aaa;
	...
	for ( aij = row->ptr; aij != NULL; aij = next_aij ) {
		col = aij->col;
		next_aij = aij->r_next;
		for ( kase = 0; kase <= 1; kase++ ) { /* save current column bounds */
			lb = col->lb, ub = col->ub;
			if ( kase == 0 ) { /* process implied column lower bound */
				if ( col->ll.ll == -DBL_MAX ) continue;
				ret = npp_implied_lower( npp, col, col->ll.ll );
			} else { /* process implied column upper bound */
				if ( col->uu.uu == +DBL_MAX ) continue;
				ret = npp_implied_upper( npp, col, col->uu.uu );
			}
			if ( ret == 0 || ret == 1 ) { /* current column bounds did not change or changed, but
											 not significantly; restore current column bounds */
				col->lb = lb, col->ub = ub;
			} else if ( ret == 2 || ret == 3 ) { /* current column bounds changed significantly or column
													was fixed */
#ifdef GLP_DEBUG
				xprintf( "L" );
#endif
			...
			} else if ( ret == 4 ) { /* primal/integer infeasibility */
				return -1;
			} else
				xassert( ret != ret );
		}
...
int npp_process_col( NPP* npp, NPPCOL* col ) { /* perform basic column processing */
	NPPROW* row;
	NPPAIJ* aij;
	int ret;
	...
	if ( col->ptr == NULL ) { /* empty column */
		ret = npp_empty_col( npp, col );
		if ( ret == 0 ) { /* column was fixed and deleted */
#ifdef GLP_DEBUG
			xprintf( "N" );
#endif
			return 0;
		} else if ( ret == 1 ) { /* dual infeasibility */
			return GLP_ENODFS;
		} else
			xassert( ret != ret );
	}
	if ( col->ptr->c_next == NULL ) { /* column singleton */
		row = col->ptr->row;
		if ( row->lb == row->ub ) { /* equality constraint */
			if ( !col->is_int )
			slack : { /* implied slack variable */
				...
				/* column was deleted */
				if ( row->lb == -DBL_MAX && row->ub == +DBL_MAX ) { /* row became free due to implied slack variable */
#ifdef GLP_DEBUG
					xprintf( "P" );
#endif
					...
					npp_free_row( npp, row );
					/* row was deleted */
				} else { /* row became inequality constraint; activate it
			...
			if ( !col->is_int ) {
				...
				if ( ret == 0 ) { /* implied free variable */
#ifdef GLP_DEBUG
					xprintf( "Q" );
#endif
					/* column bounds were removed, row was replaced by
					   equality constraint */
					goto slack;
				} else if ( ret == 1 ) { /* column is not implied free variable, because its
											lower and/or upper bounds can be active */
				} else if ( ret == 2 ) { /* dual infeasibility */
					return GLP_ENODFS;
				}
			}
		}
...
int npp_process_prob( NPP* npp, int hard ) { /* perform basic LP/MIP processing */
	NPPROW* row;
	NPPCOL* col;
	int processing, ret;
...
int npp_simplex( NPP* npp, const glp_smcp* parm ) { /* process LP prior to applying primal/dual simplex method */
	int ret;
...
int npp_integer( NPP* npp, const glp_iocp* parm ) { /* process MIP prior to applying branch-and-bound method */
	NPPROW *row, *prev_row;
	NPPCOL* col;
	NPPAIJ* aij;
	int count, ret;
