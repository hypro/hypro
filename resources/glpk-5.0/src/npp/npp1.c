...
NPP* npp_create_wksp( void ) { /* create LP/MIP preprocessor workspace */
	NPP* npp;
...
void npp_insert_row( NPP* npp, NPPROW* row, int where ) { /* insert row to the row list */
	if ( where == 0 ) {									  /* insert row to the beginning of the row list */
		row->prev = NULL;
		row->next = npp->r_head;
		if ( row->next == NULL )
			npp->r_tail = row;
			...
			row->next->prev = row;
		npp->r_head = row;
	} else { /* insert row to the end of the row list */
		row->prev = npp->r_tail;
		row->next = NULL;
		if ( row->prev == NULL )
			npp->r_head = row;
...
void npp_remove_row( NPP* npp, NPPROW* row ) { /* remove row from the row list */
	if ( row->prev == NULL )
		npp->r_head = row->next;
	else
...
void npp_activate_row( NPP* npp, NPPROW* row ) { /* make row active */
	if ( !row->temp ) {
		row->temp = 1;
...
void npp_deactivate_row( NPP* npp, NPPROW* row ) { /* make row inactive */
	if ( row->temp ) {
		row->temp = 0;
...
void npp_insert_col( NPP* npp, NPPCOL* col, int where ) { /* insert column to the column list */
	if ( where == 0 ) {									  /* insert column to the beginning of the column list */
		col->prev = NULL;
		col->next = npp->c_head;
		if ( col->next == NULL )
			npp->c_tail = col;
			...
			col->next->prev = col;
		npp->c_head = col;
	} else { /* insert column to the end of the column list */
		col->prev = npp->c_tail;
		col->next = NULL;
		if ( col->prev == NULL )
			npp->c_head = col;
...
void npp_remove_col( NPP* npp, NPPCOL* col ) { /* remove column from the column list */
	if ( col->prev == NULL )
		npp->c_head = col->next;
	else
...
void npp_activate_col( NPP* npp, NPPCOL* col ) { /* make column active */
	if ( !col->temp ) {
		col->temp = 1;
...
void npp_deactivate_col( NPP* npp, NPPCOL* col ) { /* make column inactive */
	if ( col->temp ) {
		col->temp = 0;
...
NPPROW* npp_add_row( NPP* npp ) { /* add new row to the current problem */
	NPPROW* row;
...
NPPCOL* npp_add_col( NPP* npp ) { /* add new column to the current problem */
	NPPCOL* col;
...
NPPAIJ* npp_add_aij( NPP* npp, NPPROW* row, NPPCOL* col, double val ) { /* add new element to the constraint matrix */
	NPPAIJ* aij;
...
int npp_row_nnz( NPP* npp, NPPROW* row ) { /* count number of non-zero coefficients in row */
	NPPAIJ* aij;
	int nnz;
	xassert( npp == npp );
	nnz = 0;
...
int npp_col_nnz( NPP* npp, NPPCOL* col ) { /* count number of non-zero coefficients in column */
	NPPAIJ* aij;
	int nnz;
	xassert( npp == npp );
	nnz = 0;
...
void* npp_push_tse( NPP* npp, int ( *func )( NPP* npp, void* info ),
					int size ) { /* push new entry to the transformation stack */
	NPPTSE* tse;
...
#if 1										  /* 23/XII-2009 */
void npp_erase_row( NPP* npp, NPPROW* row ) { /* erase row content to make it empty */
	NPPAIJ* aij;
	while ( row->ptr != NULL ) {
		aij = row->ptr;
		row->ptr = aij->r_next;
...
void npp_del_row( NPP* npp, NPPROW* row ) { /* remove row from the current problem */
#if 0										/* 23/XII-2009 */
      NPPAIJ *aij;
#endif
	if ( row->name != NULL )
...
void npp_del_col( NPP* npp, NPPCOL* col ) { /* remove column from the current problem */
	NPPAIJ* aij;
	if ( col->name != NULL )
...
void npp_del_aij( NPP* npp, NPPAIJ* aij ) { /* remove element from the constraint matrix */
	if ( aij->r_prev == NULL )
		aij->row->ptr = aij->r_next;
	else
		aij->r_prev->r_next = aij->r_next;
...
void npp_load_prob( NPP* npp, glp_prob* orig, int names, int sol,
					int scaling ) { /* load original problem into the preprocessor workspace */
	int m = orig->m;
	int n = orig->n;
	NPPROW** link;
	int i, j;
...
void npp_build_prob( NPP* npp, glp_prob* prob ) { /* build resultant (preprocessed) problem */
	NPPROW* row;
	NPPCOL* col;
	NPPAIJ* aij;
...
void npp_postprocess( NPP* npp, glp_prob* prob ) { /* postprocess solution from the resultant problem */
	GLPROW* row;
	GLPCOL* col;
	NPPTSE* tse;
	int i, j, k;
...
void npp_unload_sol( NPP* npp, glp_prob* orig ) { /* store solution to the original problem */
	GLPROW* row;
	GLPCOL* col;
	int i, j;
	double dir;
	...
	if ( npp->sol == GLP_SOL ) { /* store basic solution */
		orig->valid = 0;
		...
		for ( i = 1; i <= orig->m; i++ ) {
			...
			if ( !npp->scaling ) { /*row->prim = npp->r_prim[i];*/
				row->dual = dir * npp->r_pi[i];
			} else { /*row->prim = npp->r_prim[i] / row->rii;*/
...
#endif
	} else if ( npp->sol == GLP_IPT ) { /* store interior-point solution */
		orig->ipt_stat = npp->t_stat;
		orig->ipt_obj = orig->c0;
		for ( i = 1; i <= orig->m; i++ ) {
			row = orig->row[i];
			if ( !npp->scaling ) { /*row->pval = npp->r_prim[i];*/
				row->dval = dir * npp->r_pi[i];
			} else { /*row->pval = npp->r_prim[i] / row->rii;*/
...
#endif
	} else if ( npp->sol == GLP_MIP ) { /* store MIP solution */
		xassert( !npp->scaling );
...
void npp_delete_wksp( NPP* npp ) { /* delete LP/MIP preprocessor workspace */
	if ( npp->pool != NULL )
