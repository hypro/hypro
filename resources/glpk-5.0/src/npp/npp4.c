...
int npp_binarize_prob( NPP* npp ) { /* binarize MIP problem */
	...
	for ( col = npp->c_tail; col != NULL; col = col->prev ) { /* skip continuous variable */
		...
		if ( col->lb < -1e6 || col->ub > +1e6 ||
			 col->ub - col->lb > 4095.0 ) { /* unfortunately, not */
			nfails++;
			continue;
		}
		...
		for ( k = 1, temp = 2; k < n; k++, temp += temp ) { /* add new binary variable x[k] */
			bin = npp_add_col( npp );
			bin->is_int = 1;
...
static int rcv_binarize_prob( NPP* npp, void* _info ) { /* recovery binarized variable */
...
struct elem { /* linear form element a[j] x[j] */
	double aj;
...
static struct elem* copy_form( NPP* npp, NPPROW* row, double s ) { /* copy linear form */
	NPPAIJ* aij;
	struct elem *ptr, *e;
	ptr = NULL;
...
static void drop_form( NPP* npp, struct elem* ptr ) { /* drop linear form */
	struct elem* e;
	while ( ptr != NULL ) {
		e = ptr;
		ptr = e->next;
...
int npp_is_packing( NPP* npp, NPPROW* row ) { /* test if constraint is packing inequality */
	NPPCOL* col;
	NPPAIJ* aij;
	int b;
	xassert( npp == npp );
...
static int hidden_packing( NPP* npp, struct elem* ptr, double* _b ) { /* process inequality constraint: sum a[j] x[j] <= b;
	...
	if ( e == NULL ) {					  /* all coefficients a[j] are +1 or -1; check rhs b */
		if ( b == (double)( 1 - neg ) ) { /* it is packing inequality; no processing is needed */
			return 1;
		}
	}
...
int npp_hidden_packing( NPP* npp, NPPROW* row ) { /* identify hidden packing inequality */
	NPPROW* copy;
	NPPAIJ* aij;
	struct elem *ptr, *e;
	...
	for ( kase = 0; kase <= 1; kase++ ) {
		if ( kase == 0 ) { /* process row upper bound */
			if ( row->ub == +DBL_MAX ) continue;
			ptr = copy_form( npp, row, +1.0 );
			b = +row->ub;
		} else { /* process row lower bound */
		...
		xassert( 0 <= ret && ret <= 2 );
		if ( kase == 1 && ret == 1 || ret == 2 ) { /* the original inequality has been identified as hidden
													  packing inequality */
			count++;
#ifdef GLP_DEBUG
			...
			xprintf( ", <= %g\n", b );
#endif
			if ( row->lb == -DBL_MAX || row->ub == +DBL_MAX ) { /* the original row is single-sided inequality; no copy
																   is needed */
				copy = NULL;
			} else { /* the original row is double-sided inequality; we need
						to create its copy for other bound before replacing it
						with the equivalent inequality */
				copy = npp_add_row( npp );
				if ( kase == 0 ) { /* the copy is for lower bound */
					copy->lb = row->lb, copy->ub = +DBL_MAX;
				} else { /* the copy is for upper bound */
...
int npp_implied_packing( NPP* npp, NPPROW* row, int which,
	...
	for ( e = ptr; e != NULL; e = e->next ) {
		if ( !( e->xj->is_int && e->xj->lb == 0.0 && e->xj->ub == 1.0 ) ) { /* x[j] is non-binary variable */
			if ( e->aj > 0.0 ) {
	...
	eps = 1e-3 + 1e-6 * b;
	i = k = NULL;
	for ( e = ptr; e != NULL; e = e->next ) {	   /* note that alfa[j] = |a[j]| */
		if ( fabs( e->aj ) > 0.5 * ( b + eps ) ) { /* alfa[j] > (b + eps) / 2; include x[j] in set P, i.e. in
													  set Jp or Jn */
			var[++len] = e->xj;
			set[len] = (char)( e->aj > 0.0 ? 0 : 1 );
			/* alfa[i] = min alfa[j] over all j included in set P */
			if ( i == NULL || fabs( i->aj ) > fabs( e->aj ) ) i = e;
		} else if ( fabs( e->aj ) >= 1e-3 ) { /* alfa[k] = max alfa[j] over all j not included in set P;
...
static int hidden_covering( NPP* npp, struct elem* ptr, double* _b ) { /* process inequality constraint: sum a[j] x[j] >= b;
	...
	if ( e == NULL ) {					  /* all coefficients a[j] are +1 or -1; check rhs b */
		if ( b == (double)( 1 - neg ) ) { /* it is covering inequality; no processing is needed */
			return 1;
		}
	}
	/* substitute x[j] = 1 - x~[j] for all j in Jn to make all a[j]
...
int npp_hidden_covering( NPP* npp, NPPROW* row ) { /* identify hidden covering inequality */
	NPPROW* copy;
	NPPAIJ* aij;
	struct elem *ptr, *e;
	int kase, ret, count = 0;
	...
	xassert( row->lb < row->ub );
	for ( kase = 0; kase <= 1; kase++ ) {
		if ( kase == 0 ) { /* process row lower bound */
			if ( row->lb == -DBL_MAX ) continue;
			ptr = copy_form( npp, row, +1.0 );
			b = +row->lb;
		} else { /* process row upper bound */
			if ( row->ub == +DBL_MAX ) continue;
			ptr = copy_form( npp, row, -1.0 );
			b = -row->ub;
		}
		...
		if ( kase == 1 && ret == 1 || ret == 2 ) { /* the original inequality has been identified as hidden
			...
			if ( row->lb == -DBL_MAX || row->ub == +DBL_MAX ) { /* the original row is single-sided inequality; no copy
				...
				copy = NULL;
						...
						with the equivalent inequality */
				copy = npp_add_row( npp );
				if ( kase == 0 ) { /* the copy is for upper bound */
					copy->lb = -DBL_MAX, copy->ub = row->ub;
				} else { /* the copy is for lower bound */
					copy->lb = row->lb, copy->ub = +DBL_MAX;
				}
				/* copy original row coefficients */
				for ( aij = row->ptr; aij != NULL; aij = aij->r_next )
...
int npp_is_partitioning( NPP* npp, NPPROW* row ) { /* test if constraint is partitioning equality */
	NPPCOL* col;
	NPPAIJ* aij;
	int b;
	xassert( npp == npp );
...
static int reduce_ineq_coef( NPP* npp, struct elem* ptr, double* _b ) { /* process inequality constraint: sum a[j] x[j] >= b */
	...
	for ( e = ptr; e != NULL; e = e->next ) { /* skip non-binary variable */
		if ( !( e->xj->is_int && e->xj->lb == 0.0 && e->xj->ub == 1.0 ) )
			continue;
		if ( e->aj > 0.0 ) { /* compute inf t[k]; see (14) */
			inf_t = h;
			if ( b - e->aj < inf_t && inf_t < b ) { /* compute reduced coefficient a'[k]; see (7) */
				new_a = b - inf_t;
				if ( new_a >= +1e-3 &&
					 e->aj - new_a >= 0.01 * ( 1.0 + e->aj ) ) { /* accept a'[k] */
#ifdef GLP_DEBUG
					xprintf( "+" );
#endif
					e->aj = new_a;
		...
		{	   /* compute inf t[k]; see (14) */
			inf_t = h - e->aj;
			if ( b < inf_t && inf_t < b - e->aj ) { /* compute reduced coefficient a'[k]; see (11) */
				new_a = e->aj + ( inf_t - b );
				if ( new_a <= -1e-3 &&
					 new_a - e->aj >= 0.01 * ( 1.0 - e->aj ) ) { /* accept a'[k] */
#ifdef GLP_DEBUG
					xprintf( "-" );
#endif
					e->aj = new_a;
...
int npp_reduce_ineq_coef( NPP* npp, NPPROW* row ) { /* reduce inequality constraint coefficients */
	NPPROW* copy;
	NPPAIJ* aij;
	struct elem *ptr, *e;
	int kase, count[2];
	...
	for ( kase = 0; kase <= 1; kase++ ) {
		...
		if ( count[kase] > 0 ) {								/* the original inequality has been replaced by equivalent
