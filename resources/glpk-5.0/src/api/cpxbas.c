...
struct var { /* structural variable */
	int j;
	/* ordinal number */
	double q;
	/* penalty value */
...
static int CDECL fcmp( const void* ptr1, const void* ptr2 ) { /* this routine is passed to the qsort() function */
...
static int get_column( glp_prob* lp, int j, int ind[], double val[] ) { /* Bixby's algorithm assumes that the constraint matrix is scaled
...
static void cpx_basis( glp_prob* lp ) { /* main routine */
	...
	for ( i = 1; i <= m; i++ ) {
		...
		if ( type != GLP_FX ) { /* row i is either free or inequality constraint */
			glp_set_row_stat( lp, i, GLP_BS );
			I[i] = 1;
			r[i] = 1;
		} else { /* row i is equality constraint */
			I[i] = 0;
			r[i] = 0;
		}
		v[i] = +DBL_MAX;
