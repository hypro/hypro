...
static int overflow( int u, int v ) { /* check for integer overflow on computing u + v */
...
int glp_mincost_relax4( glp_graph* G, int v_rhs, int a_low, int a_cap,
						int a_cost, int crash, double* sol, int a_x, int a_rc ) { /* find minimum-cost flow with Bertsekas-Tseng relaxation method
																					 (RELAX-IV) */
	glp_vertex* v;
	glp_arc* a;
	...
	for ( i = 1; i <= n; i++ ) {
		...
		for ( a = v->out; a != NULL; a = a->t_next ) {
			...
			if ( a->tail->i == a->head->i ) { /* self-loops not allowed */
				ret = GLP_EDATA;
				goto done;
			}
	...
	ret = relax4( &csa );
	if ( ret != 0 ) { /* problem is found to be infeasible */
		xassert( 1 <= ret && ret <= 8 );
		ret = GLP_ENOPFS;
		goto done;
	}
