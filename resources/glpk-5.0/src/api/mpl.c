...
glp_tran* glp_mpl_alloc_wksp( void ) { /* allocate the MathProg translator workspace */
	glp_tran* tran;
	tran = mpl_initialize();
	return tran;
}

void glp_mpl_init_rand( glp_tran* tran, int seed ) { /* initialize pseudo-random number generator */
	if ( tran->phase != 0 )
...
int glp_mpl_read_model( glp_tran* tran, const char* fname, int skip ) { /* read and translate model section */
	int ret;
	if ( tran->phase != 0 )
...
int glp_mpl_read_data( glp_tran* tran, const char* fname ) { /* read and translate data section */
	int ret;
...
int glp_mpl_generate( glp_tran* tran, const char* fname ) { /* generate the model */
	int ret;
	if ( !( tran->phase == 1 || tran->phase == 2 ) )
...
void glp_mpl_build_prob( glp_tran* tran, glp_prob* prob ) { /* build LP/MIP problem instance from the model */
	...
	for ( i = 1; i <= m; i++ ) {
		...
		if ( kind == MPL_MIN || kind == MPL_MAX ) { /* set objective name */
...
int glp_mpl_postsolve( glp_tran* tran, glp_prob* prob, int sol ) { /* postsolve the model */
...
void glp_mpl_free_wksp( glp_tran* tran ) { /* free the MathProg translator workspace */
	mpl_terminate( tran );
	return;
}
