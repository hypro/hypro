...
static void check_parm( const char* func, const glp_cpxcp* parm ) { /* check control parameters */
	xassert( func != NULL );
	xassert( parm != NULL );
	return;
}
...
static void read_char( struct csa* csa ) { /* read next character from input file */
	int c;
	xassert( csa->c != EOF );
	if ( csa->c == '\n' ) csa->count++;
	c = glp_getc( csa->fp );
...
static void add_char( struct csa* csa ) { /* append current character to current token */
...
static int the_same( char* s1, char* s2 ) { /* compare two character strings ignoring case sensitivity */
	for ( ; *s1 != '\0'; s1++, s2++ ) {
...
static void scan_token( struct csa* csa ) { /* scan next token */
	int flag;
	csa->token = -1;
	csa->image[0] = '\0';
	csa->imlen = 0;
...
loop:
	...
	if ( csa->c == EOF )
		...
		goto loop;
	} else if ( csa->c == '\\' ) { /* comment; ignore everything until end-of-line */
		while ( csa->c != '\n' ) read_char( csa );
		goto loop;
	...
	name : { /* symbolic name */
		...
		else if ( isdigit( csa->c ) || csa->c == '.' ) { /* numeric constant */
			csa->token = T_NUMBER;
			/* scan integer part */
...
static int find_col( struct csa* csa, char* name ) { /* find column by its symbolic name */
	int j;
	j = glp_find_col( csa->P, name );
	if ( j == 0 ) { /* not found; create new column */
...
static void parse_objective( struct csa* csa ) { /* parse objective sense */
	int k, len;
	...
	if ( csa->token == T_NAME && csa->c == ':' ) { /* objective name is followed by a colon */
		glp_set_obj_name( csa->P, csa->image );
		scan_token( csa );
		xassert( csa->token == T_COLON );
		scan_token( csa );
	} else { /* objective name is not specified; use default */
...
loop: /* create new row (constraint) */
	...
	/* parse row name */
	if ( csa->token == T_NAME && csa->c == ':' ) { /* row name is followed by a colon */
		...
		xassert( csa->token == T_COLON );
		scan_token( csa );
	} else { /* row name is not specified; use default */
		char name[50];
...
static void set_lower_bound( struct csa* csa, int j, double lb ) { /* set lower bound of j-th variable */
...
static void set_upper_bound( struct csa* csa, int j, double ub ) { /* set upper bound of j-th variable */
...
loop: /* bound definition can start with a sign, numeric constant, or
	...
	if ( csa->token == T_PLUS || csa->token == T_MINUS ) { /* parse signed lower bound */
		lb_flag = 1;
	...
	} else if ( csa->token == T_NUMBER ) { /* parse unsigned lower bound */
		lb_flag = 1;
		lb = csa->value, scan_token( csa );
	} else { /* lower bound is not specified */
		lb_flag = 0;
	}
	...
	if ( csa->token == T_LE ) { /* parse upper bound */
		scan_token( csa );
		if ( csa->token == T_PLUS || csa->token == T_MINUS ) { /* parse signed upper bound */
			s = ( csa->token == T_PLUS ? +1.0 : -1.0 );
			scan_token( csa );
			if ( csa->token == T_NUMBER ) {
		...
		} else if ( csa->token == T_NUMBER ) { /* parse unsigned upper bound */
			set_upper_bound( csa, j, csa->value );
			scan_token( csa );
		} else
			error( csa, "missing upper bound\n" );
	} else if ( csa->token == T_GE ) { /* parse lower bound */
		if ( lb_flag ) {			   /* the context '... <= x >= ...' is invalid */
		...
		if ( csa->token == T_PLUS || csa->token == T_MINUS ) { /* parse signed lower bound */
			...
			error( csa, "missing lower bound\n" );
	} else if ( csa->token == T_EQ ) { /* parse fixed value */
		if ( lb_flag ) {			   /* the context '... <= x = ...' is invalid */
			error( csa, "invalid bound definition\n" );
		}
		scan_token( csa );
		if ( csa->token == T_PLUS || csa->token == T_MINUS ) { /* parse signed fixed value */
			s = ( csa->token == T_PLUS ? +1.0 : -1.0 );
			scan_token( csa );
			if ( csa->token == T_NUMBER ) {
				set_lower_bound( csa, j, s * csa->value );
		...
		} else if ( csa->token == T_NUMBER ) { /* parse unsigned fixed value */
			set_lower_bound( csa, j, csa->value );
			set_upper_bound( csa, j, csa->value );
			scan_token( csa );
		} else
			error( csa, "missing fixed value\n" );
	} else if ( the_same( csa->image, "free" ) ) { /* parse the keyword 'free' */
		if ( lb_flag ) {						   /* the context '... <= x free ...' is invalid */
			error( csa, "invalid bound definition\n" );
		}
		set_lower_bound( csa, j, -DBL_MAX );
		set_upper_bound( csa, j, +DBL_MAX );
		scan_token( csa );
	} else if ( !lb_flag ) { /* neither lower nor upper bounds are specified */
		error( csa, "invalid bound definition\n" );
	}
	goto loop;
done:
...
static void parse_integer( struct csa* csa ) {
	...
	while ( csa->token == T_NAME ) { /* find the corresponding column */
		j = find_col( csa, csa->image );
...
int glp_read_lp( glp_prob* P, const glp_cpxcp* parm, const char* fname ) { /* read problem data in CPLEX LP format */
	glp_cpxcp _parm;
...
static int check_name( char* name ) { /* check if specified name is valid for CPLEX LP format */
	if ( *name == '.' ) return 1;
...
static void adjust_name( char* name ) { /* attempt to adjust specified name to make it valid for CPLEX LP
										   format */
	for ( ; *name; name++ ) {
		if ( *name == ' ' )
			*name = '_';
...
static char* row_name( struct csa* csa, int i, char rname[255 + 1] ) { /* construct symbolic name of i-th row (constraint) */
	const char* name;
	if ( i == 0 )
...
static char* col_name( struct csa* csa, int j, char cname[255 + 1] ) { /* construct symbolic name of j-th column (variable) */
	const char* name;
...
fake : /* construct fake name depending on column's attributes */
{
	...
	if ( col->type == GLP_FX ) { /* fixed column */
		sprintf( cname, "s_%d", j );
	} else if ( col->kind == GLP_CV ) { /* continuous variable */
		sprintf( cname, "x_%d", j );
	} else if ( !( col->lb == 0 && col->ub == 1 ) ) { /* general (non-binary) integer variable */
...
int glp_write_lp( glp_prob* P, const glp_cpxcp* parm, const char* fname ) { /* write problem data in CPLEX LP format */
	glp_cpxcp _parm;
	struct csa _csa, *csa = &_csa;
	glp_file* fp;
	GLPROW* row;
	...
	for ( i = 1; i <= P->m; i++ ) {
		...
		if ( row->type == GLP_DB ) { /* double-bounded (ranged) constraint */
			sprintf( term, " - ~r_%d", i );
			if ( strlen( line ) + strlen( term ) > 72 )
				xfprintf( fp, "%s\n", line ), line[0] = '\0', count++;
			strcat( line, term );
		} else if ( row->ptr == NULL ) { /* empty constraint */
			sprintf( term, " 0 %s", col_name( csa, 1, name ) );
			strcat( line, term );
		}
		/* right hand-side */
	...
	for ( j = 1; j <= P->n; j++ ) {
		...
		if ( col->type == GLP_FR )
			...
			xfprintf( fp, " %.*g <= %s <= %.*g\n",
					  DBL_DIG, col->lb, name, DBL_DIG, col->ub ),
				  count++;
