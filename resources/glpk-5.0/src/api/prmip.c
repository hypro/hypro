...
int glp_print_mip( glp_prob* P, const char* fname ) { /* write MIP solution in printable format */
	glp_file* fp;
	GLPROW* row;
	GLPCOL* col;
	...
	t = glp_mip_status( P );
	xfprintf( fp, "%-12s%s\n", "Status:",
			  t == GLP_OPT ? "INTEGER OPTIMAL" : t == GLP_FEAS ? "INTEGER NON-OPTIMAL"
	...
	for ( j = 1; j <= P->n; j++ ) {
		...
		if ( col->name == NULL || strlen( col->name ) <= 12 )
			...
			xfprintf( fp, "%s\n%20s", col->name, "" );
		xfprintf( fp, "%s  ",
				  col->kind == GLP_CV ? " " : col->kind == GLP_IV ? "*"
																  : "?" );
		xfprintf( fp, "%13.6g ",
