...
static void check_parm( const char* func, const glp_mpscp* parm ) { /* check control parameters */
...
static void error( struct csa* csa, const char* fmt, ... ) { /* print error message and terminate processing */
	va_list arg;
...
static void warning( struct csa* csa, const char* fmt, ... ) { /* print warning message and continue processing */
	va_list arg;
...
static void read_char( struct csa* csa ) { /* read next character */
	int c;
	if ( csa->c == '\n' )
...
static int indicator( struct csa* csa, int name ) { /* skip comment records and read possible indicator record */
	int ret;
...
loop: /* read the very first character of the next record */
	...
	if ( csa->c == ' ' || csa->c == '\n' ) { /* data record */
		ret = 0;
	} else if ( csa->c == '*' ) { /* comment record */
		while ( csa->c != '\n' )
			read_char( csa );
		goto loop;
	} else { /* indicator record */
		int len = 0;
		while ( csa->c != ' ' && csa->c != '\n' && len < 12 ) {
...
static void read_field( struct csa* csa ) { /* read next field of the current data record */
	csa->fldno++;
	if ( csa->deck ) { /* fixed MPS format */
		int beg, end, pos;
...
static void patch_name( struct csa* csa, char* name ) { /* process embedded blanks in symbolic name */
	int blank = csa->parm->blank;
	if ( blank == '\0' ) { /* remove emedded blanks */
		strspx( name );
	} else { /* replace embedded blanks by specified character */
		for ( ; *name != '\0'; name++ )
...
static double read_number( struct csa* csa ) { /* read next field and convert it to floating-point number */
	double x;
	char* s;
	/* read next field */
	read_field( csa );
...
static void skip_field( struct csa* csa ) { /* read and skip next field (assumed to be blank) */
	read_field( csa );
	if ( csa->field[0] != '\0' )
...
static void read_name( struct csa* csa ) { /* read NAME indicator record */
...
static void read_rows( struct csa* csa ) { /* read ROWS section */
	int i, type;
loop:
	if ( indicator( csa, 0 ) ) goto done;
	/* field 1: row type */
...
static void read_columns( struct csa* csa ) { /* read COLUMNS section */
...
loop:
	...
	read_field( csa ), patch_name( csa, csa->field );
	if ( strcmp( csa->field, "'MARKER'" ) == 0 ) { /* process kind data record */
	...
	if ( name[0] == '\0' ) { /* the same column as in previous data record */
		if ( j == 0 )
			error( csa, "missing column name in field 2\n" );
	} else if ( j != 0 && strcmp( name, csa->P->col[j]->name ) == 0 ) { /* the same column as in previous data record */
		xassert( j != 0 );
	} else { /* store the current column */
		if ( j != 0 ) {
	...
	for ( f = 3; f <= 5; f += 2 ) { /* field 3 or 5: row name */
		if ( f == 3 ) {
			if ( csa->field[0] == '\0' )
				error( csa, "missing row name in field 3\n" );
		} else {
			read_field( csa ), patch_name( csa, csa->field );
			if ( csa->field[0] == '\0' ) { /* if field 5 is blank, field 6 also must be blank */
				skip_field( csa );
				continue;
			}
		}
...
static void read_rhs( struct csa* csa ) { /* read RHS section */
	int i, f, v, type;
	double rhs;
	char name[255 + 1], *flag;
...
loop:
	...
	read_field( csa ), patch_name( csa, csa->field );
	if ( csa->field[0] == '\0' ) { /* the same RHS vector as in previous data record */
		if ( v == 0 ) {
			warning( csa, "missing RHS vector name in field 2\n" );
			goto blnk;
		}
	} else if ( v != 0 && strcmp( csa->field, name ) == 0 ) { /* the same RHS vector as in previous data record */
		xassert( v != 0 );
	} else
	blnk : { /* new RHS vector */
		if ( v != 0 )
		...
		for ( f = 3; f <= 5; f += 2 ) { /* field 3 or 5: row name */
			...
			if ( csa->field[0] == '\0' ) {
				if ( f == 3 )
					error( csa, "missing row name in field 3\n" );
				else { /* if field 5 is blank, field 6 also must be blank */
					skip_field( csa );
					continue;
				}
			}
...
static void read_ranges( struct csa* csa ) { /* read RANGES section */
	int i, f, v, type;
	double rhs, rng;
	char name[255 + 1], *flag;
	/* allocate working array */
...
loop:
	...
	read_field( csa ), patch_name( csa, csa->field );
	if ( csa->field[0] == '\0' ) { /* the same RANGES vector as in previous data record */
		if ( v == 0 ) {
			warning( csa, "missing RANGES vector name in field 2\n" );
			goto blnk;
		}
	} else if ( v != 0 && strcmp( csa->field, name ) == 0 ) { /* the same RANGES vector as in previous data record */
		xassert( v != 0 );
	} else
...
static void read_bounds( struct csa* csa ) { /* read BOUNDS section */
	GLPCOL* col;
	int j, v, mask, data;
	double bnd, lb, ub;
...
loop:
	...
	read_field( csa ), patch_name( csa, csa->field );
	if ( csa->field[0] == '\0' ) { /* the same BOUNDS vector as in previous data record */
		if ( v == 0 ) {
			warning( csa, "missing BOUNDS vector name in field 2\n" );
			goto blnk;
		}
	} else if ( v != 0 && strcmp( csa->field, name ) == 0 ) { /* the same BOUNDS vector as in previous data record */
		xassert( v != 0 );
	} else
...
int glp_read_mps( glp_prob* P, int fmt, const glp_mpscp* parm,
				  const char* fname ) { /* read problem data in MPS format */
	glp_mpscp _parm;
	...
	if ( parm->obj_name == NULL || parm->obj_name[0] == '\0' ) { /* use the first row of N type */
		int i;
		for ( i = 1; i <= P->m; i++ ) {
	...
	} else { /* use a row with specified name */
		int i;
		for ( i = 1; i <= P->m; i++ ) {
...
static char* mps_name( struct csa* csa ) { /* make problem name */
	char* f;
	if ( csa->P->name == NULL )
		csa->field[0] = '\0';
	else if ( csa->deck ) {
...
static char* row_name( struct csa* csa, int i ) { /* make i-th row name */
	char* f;
...
static char* col_name( struct csa* csa, int j ) { /* make j-th column name */
	char* f;
...
static char* mps_numb( struct csa* csa, double val ) { /* format floating-point number */
	int dig;
	char* exp;
...
int glp_write_mps( glp_prob* P, int fmt, const glp_mpscp* parm,
				   const char* fname ) { /* write problem data in MPS format */
	glp_mpscp _parm;
	struct csa _csa, *csa = &_csa;
	glp_file* fp;
	...
	xfprintf( fp, "* %-*s%s\n", P->name == NULL ? 1 : 12, "Problem:",
			  P->name == NULL ? "" : P->name ),
		  recno++;
	xfprintf( fp, "* %-12s%s\n", "Class:", glp_get_num_int( P ) == 0 ? "LP" : "MIP" ), recno++;
	xfprintf( fp, "* %-12s%d\n", "Rows:", P->m ), recno++;
	if ( glp_get_num_int( P ) == 0 )
		xfprintf( fp, "* %-12s%d\n", "Columns:", P->n ), recno++;
	else
	...
	xfprintf( fp, "* %-12s%s\n", "Format:", csa->deck ? "Fixed MPS" : "Free MPS" ), recno++;
	xfprintf( fp, "*\n", recno++ );
	/* write NAME indicator record */
	xfprintf( fp, "NAME%*s%s\n",
			  P->name == NULL ? 0 : csa->deck ? 10
											  : 1,
			  "", mps_name( csa ) ),
		  recno++;
#if 1
	...
	for ( j = 1; j <= P->n; j++ ) {
		...
		kind = P->col[j]->kind;
		if ( kind == GLP_CV ) {
			if ( marker % 2 == 1 ) { /* close current integer block */
				marker++;
			...
			if ( marker % 2 == 0 ) { /* open new integer block */
				marker++;
			...
			xassert( kind != kind );
		if ( out_obj && P->col[j]->coef != 0.0 ) { /* make fake objective coefficient */
			aij = &cj;
			aij->row = NULL;
	...
	if ( marker % 2 == 1 ) { /* close last integer block */
		marker++;
		xfprintf( fp, "%*sM%07d%*s'MARKER'%*s'INTEND'\n",
				  csa->deck ? 4 : 1, "", marker, csa->deck ? 2 : 1, "",
				  csa->deck ? 17 : 1, "" ),
	...
	for ( i = 1; i <= P->m; i++ ) {
		if ( P->row[i]->type == GLP_DB ) {
			...
			xfprintf( fp, "%*s%*s",
					  csa->deck ? 2 : 1, "", csa->deck ? 12 : 1,
					  mps_numb( csa, P->row[i]->ub - P->row[i]->lb ) ),
				  count++;
			if ( one_col || count % 2 == 0 )
				xfprintf( fp, "\n" ), recno++;
		}
