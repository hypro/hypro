...
int glp_mincost_okalg( glp_graph* G, int v_rhs, int a_low, int a_cap,
					   int a_cost, double* sol, int a_x, int v_pi ) { /* find minimum-cost flow with out-of-kilter algorithm */
	glp_vertex* v;
	glp_arc* a;
	...
	for ( i = 1; i <= G->nv; i++ ) {
		...
		if ( temp > 0.0 ) { /* artificial arc from s to original source i */
			k++;
			tail[k] = s;
			head[k] = i;
			...
			cost[k] = 0;
			sum += (double)temp;
		} else if ( temp < 0.0 ) { /* artificial arc from original sink i to t */
			k++;
			tail[k] = i;
			head[k] = t;
