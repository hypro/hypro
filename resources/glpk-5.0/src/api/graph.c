...
void glp_set_vertex_name( glp_graph* G, int i, const char* name ) { /* assign (change) vertex name */
	glp_vertex* v;
	if ( !( 1 <= i && i <= G->nv ) )
		xerror(
...
void glp_del_vertices( glp_graph* G, int ndel, const int num[] ) {
	...
	if ( !( 1 <= ndel && ndel <= G->nv ) )
		xerror(
			  ...
			  "s\n",
			  ndel );
	for ( k = 1; k <= ndel; k++ ) { /* take the number of vertex to be deleted */
		i = num[k];
	...
	nv_new = 0;
	for ( i = 1; i <= G->nv; i++ ) { /* obtain pointer to i-th vertex */
		v = G->v[i];
		/* check if the vertex is marked */
		if ( v->i == 0 ) { /* it is marked, delete it */
			dmp_free_atom( G->pool, v, sizeof( glp_vertex ) );
		} else { /* it is not marked, keep it */
			v->i = ++nv_new;
			G->v[v->i] = v;
		}
	}
...
void glp_del_arc( glp_graph* G, glp_arc* a ) { /* some sanity checks */
	xassert( G->na > 0 );
...
void glp_create_v_index( glp_graph* G ) { /* create vertex name index */
	glp_vertex* v;
	int i;
	if ( G->index == NULL ) {
...
int glp_find_vertex( glp_graph* G, const char* name ) { /* find vertex by its name */
	AVLNODE* node;
	int i = 0;
	if ( G->index == NULL )
...
void glp_delete_v_index( glp_graph* G ) { /* delete vertex name index */
	int i;
	if ( G->index != NULL ) {
