...
int glp_weak_comp( glp_graph* G, int v_num ) {
	...
	nc = 0;
	while ( f != 0 ) { /* take an unlabelled vertex */
		i = f;
		...
		while ( pos1 <= pos2 ) { /* dequeue vertex i */
			i = list[pos1++];
			/* consider all arcs incoming to vertex i */
			for ( a = G->v[i]->in; a != NULL; a = a->h_next ) { /* vertex j is adjacent to vertex i */
				j = a->tail->i;
				if ( prev[j] >= 0 ) { /* vertex j is unlabelled */
			...
			for ( a = G->v[i]->out; a != NULL; a = a->t_next ) { /* vertex j is adjacent to vertex i */
				j = a->head->i;
				if ( prev[j] >= 0 ) { /* vertex j is unlabelled */
					/* remove it from the list of unlabelled vertices */
					if ( prev[j] == 0 )
