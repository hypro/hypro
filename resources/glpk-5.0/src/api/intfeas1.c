...
int glp_intfeas1( glp_prob* P, int use_bound, int obj_bound ) { /* solve integer feasibility problem */
	NPP* npp = NULL;
	glp_prob* mip = NULL;
	int* obj_ind = NULL;
	double* obj_val = NULL;
	...
	for ( j = 1; j <= P->n; j++ ) {
		GLPCOL* col = P->col[j];
         ...
         {  xprintf("glp_intfeas1: column %d: non-integer non-fixed var"
               "iable not allowed\n", j);
#else
		if ( !( ( col->kind == GLP_IV && col->lb == 0.0 && col->ub == 1.0 ) || col->type == GLP_FX ) ) {
			xprintf(
...
if ( !use_bound ) { /* skip check if no obj. bound is specified */
	goto skip;
}
#endif
temp = (int)P->c0;
...
xprintf(
	  ...
	  "ero%s\n",
	  P->m, P->m == 1 ? "" : "s", P->n, P->n == 1 ? "" : "s", P->nnz, P->nnz == 1 ? "" : "s" );
npp = npp_create_wksp();
