...
void glp_set_row_stat( glp_prob* lp, int i, int stat ) {
	...
	if ( row->stat == GLP_BS && stat != GLP_BS ||
		 row->stat != GLP_BS && stat == GLP_BS ) { /* invalidate the basis factorization */
		lp->valid = 0;
	}
	row->stat = stat;
	return;
...
void glp_set_col_stat( glp_prob* lp, int j, int stat ) {
	...
	if ( col->stat == GLP_BS && stat != GLP_BS ||
		 col->stat != GLP_BS && stat == GLP_BS ) { /* invalidate the basis factorization */
		lp->valid = 0;
	}
	col->stat = stat;
	return;
