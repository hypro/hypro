...
int glp_read_cnfsat( glp_prob* P, const char* fname ) { /* read CNF-SAT problem data in DIMACS format */
	DMX _csa, *csa = &_csa;
	...
	/* read clauses */
	for ( i = 1; i <= m; i++ ) { /* read i-th clause */
		len = 0, rhs = 1;
		for ( ;; ) { /* skip white-space characters */
			while ( csa->c == ' ' || csa->c == '\n' )
				read_char( csa );
			/* read term */
			read_field( csa );
