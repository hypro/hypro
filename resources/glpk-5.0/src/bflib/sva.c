...
int sva_alloc_vecs( SVA* sva, int nnn ) {
	...
	new_n = n + nnn;
	xassert( new_n > n );
	if ( n_max < new_n ) { /* enlarge the SVA arrays */
		while ( n_max < new_n ) {
			n_max += n_max;
			xassert( n_max > 0 );
		}
...
void sva_defrag_area( SVA* sva ) {
	 ...
	 * part of SVA */
	for ( k = sva->head; k != 0; k = next_k ) { /* save number of next vector in the list */
		next_k = next[k];
		/* determine length of k-th vector */
		len_k = len[k];
		if ( len_k == 0 ) { /* k-th vector is empty; remove it from the left part */
			ptr[k] = cap[k] = 0;
			prev[k] = next[k] = -1;
		} else { /* determine pointer to first location of k-th vector */
			ptr_k = ptr[k];
			xassert( m_ptr <= ptr_k );
...
void sva_enlarge_cap( SVA* sva, int k, int new_cap, int skip ) {
	...
	/* relocate the vector */
	if ( cap[k] == 0 ) { /* the vector is empty */
		xassert( ptr[k] == 0 );
		xassert( len[k] == 0 );
	} else { /* the vector has non-zero capacity */
		...
		if ( prev[k] == 0 )
			sva->head = next[k];
		else { /* preceding vector exists; increase its capacity */
			cap[prev[k]] += cap[k];
			next[prev[k]] = next[k];
		}
		if ( next[k] == 0 )
...
void sva_check_area( SVA* sva ) {
	...
	for ( k = 1; k <= n; k++ ) {
		if ( cap[k] < 0 ) { /* k-th vector is stored in the left part */
			cap[k] = -cap[k];
		} else if ( cap[k] == 0 ) { /* k-th vector has zero capacity */
			xassert( ptr[k] == 0 );
			xassert( len[k] == 0 );
		} else /* cap[k] > 0 */
