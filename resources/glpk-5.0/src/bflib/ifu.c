...
void ifu_expand( IFU* ifu, double c[/*1+n*/], double r[/*1+n*/],
				 double d ) { /* non-optimized version */
	int n_max = ifu->n_max;
	int n = ifu->n;
	double* f_ = ifu->f;
	double* u_ = ifu->u;
...
#define u( i, j ) u_[(i)*n_max + ( j )]
	 ...
	 * matrix U */
	for ( k = 0; k < n; k++ ) {						   /* if |u[k,k]| < |u[n,k]|, interchange k-th and n-th rows to
														* provide |u[k,k]| >= |u[n,k]| for numeric stability */
		if ( fabs( u( k, k ) ) < fabs( u( n, k ) ) ) { /* interchange k-th and n-th rows of matrix U */
			for ( j = k; j <= n; j++ )
		...
		if ( fabs( u( k, k ) ) < tol ) { /* u[k,k] is too small in magnitude */
			return 1;
		}
	...
	if ( fabs( u( n, n ) ) < tol ) { /* u[n,n] is too small in magnitude */
		return 2;
	}
#undef f
#undef u
...
static void givens( double a, double b, double* c, double* s ) { /* non-optimized version */
	double t;
	if ( b == 0.0 )
		( *c ) = 1.0, ( *s ) = 0.0;
...
#define u( i, j ) u_[(i)*n_max + ( j )]
	...
	/* eliminate spike (non-zero subdiagonal elements) in last row of
	 * matrix U */
	for ( k = 0; k < n; k++ ) {										/* check if elements u[k,k] and u[n,k] are eligible */
		if ( fabs( u( k, k ) ) < tol && fabs( u( n, k ) ) < tol ) { /* both u[k,k] and u[n,k] are too small in magnitude */
			return 1;
		}
		/* if u[n,k] = 0, elimination is not needed */
		if ( u( n, k ) == 0.0 )
...
void ifu_a_solve( IFU* ifu, double x[/*1+n*/], double w[/*1+n*/] ) { /* non-optimized version */
	int n_max = ifu->n_max;
	int n = ifu->n;
	double* f_ = ifu->f;
	double* u_ = ifu->u;
...
#define u( i, j ) u_[(i)*n_max + ( j )]
	...
	for ( i = 0; i < n; i++ ) { /* y[i] := (i-th row of F) * b */
		t = 0.0;
		for ( j = 0; j < n; j++ )
			t += f( i, j ) * w[j];
		x[i] = t;
...
void ifu_at_solve( IFU* ifu, double x[/*1+n*/], double w[/*1+n*/] ) { /* non-optimized version */
	int n_max = ifu->n_max;
	int n = ifu->n;
	double* f_ = ifu->f;
	double* u_ = ifu->u;
...
#define u( i, j ) u_[(i)*n_max + ( j )]
	...
	/* x := F'* y */
	for ( j = 0; j < n; j++ ) { /* x[j] := (j-th column of F) * y */
		t = 0.0;
		for ( i = 0; i < n; i++ )
			t += f( i, j ) * x[i];
		w[j] = t;
