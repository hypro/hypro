...
int btf_store_a_cols( BTF* btf, int ( *col )( void* info, int j, int ind[], double val[] ), void* info, int ind[], double val[] ) {
	int n = btf->n;
	SVA* sva = btf->sva;
	int* sv_ind = sva->ind;
...
int btf_make_blocks( BTF* btf ) {
	...
	if ( rank < n ) { /* A is structurally singular (rank is its symbolic rank) */
		goto done;
	}
...
void btf_check_blocks( BTF* btf ) {
	...
	for ( k = 1; k <= num; k++ ) { /* determine size of k-th block */
		size = beg[k + 1] - beg[k];
		xassert( size >= 1 );
		...
		for ( jj = beg[k]; jj < beg[k + 1]; jj++ ) {
			...
			for ( ; ptr < end; ptr++ ) { /* determine row index of a[i,j] */
				i = sv_ind[ptr];
				/* i-th row of A = ii-th row of A~ */
				ii = pp_ind[i];
				/* a~[ii,jj] should not be below k-th block */
				xassert( ii < beg[k + 1] );
				if ( ii == jj ) { /* non-zero diagonal element of A~ encountered */
					diag = 1;
				}
			}
			xassert( diag );
...
void btf_a_solve( BTF* btf, double b[/*1+n*/], double x[/*1+n*/],
	...
	double t;
	for ( k = num; k >= 1; k-- ) { /* determine order of diagonal block A~[k,k] */
		luf.n = beg[k + 1] - ( beg_k = beg[k] );
		if ( luf.n == 1 ) { /* trivial case */
...
void btf_at_solve( BTF* btf, double b[/*1+n*/], double x[/*1+n*/],
	...
	int i, j, jj, k, beg_k, flag;
	double t;
	for ( k = 1; k <= num; k++ ) { /* determine order of diagonal block A~[k,k] */
		luf.n = beg[k + 1] - ( beg_k = beg[k] );
		if ( luf.n == 1 ) { /* trivial case */
			/* solve system A~'[k,k] * X[k] = B[k] */
			t = x[pp_inv[beg_k]] =
...
double btf_estimate_norm( BTF* btf, double w1[/*1+n*/], double w2[/*1+n*/], double w3[/*1+n*/], double w4[/*1+n*/] ) {
	int n = btf->n;
	double* e = w1;
	double* y = w2;
	double* z = w1;
