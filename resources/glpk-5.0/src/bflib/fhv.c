...
int fhv_ft_update( FHV* fhv, int q, int aq_len, const int aq_ind[],
	...
	for ( q_end = ( q_ptr = vc_ptr[q] ) + vc_len[q];
		  q_ptr < q_end; q_ptr++ ) { /* get row index of v[i,q] */
		i = sv_ind[q_ptr];
	 ...
	 * u[s,t] != 0 */
	t = ( vpq == 0.0 ? 0 : s );
	for ( k = 1; k <= len; k++ ) { /* get row index of v[i,q] */
		i = ind[k];
		/* put v[i,q] to i-th row */
		if ( vr_cap[i] == vr_len[i] ) { /* reserve extra locations in i-th row to reduce further
	...
	if ( s >= t ) { /* no spike; matrix U is already upper triangular */
		/* store its diagonal element u[s,s] = v[p,q] */
		vr_piv[p] = vpq;
		if ( s > t ) { /* matrix U is structurally singular, because its diagonal
	...
	if ( p_ptr == p_end ) { /* no spike; matrix U is already upper triangular */
	...
	work[q] = vpq;
	for ( p_end = ( p_ptr = vr_ptr[p] ) + vr_len[p];
		  p_ptr < p_end; p_ptr++ ) { /* get column index of v[p,j] and store this element to the
									  * working array */
	...
	for ( k = s; k < t; k++ ) { /* diagonal element u[k,k] = v[i,j] is used as pivot */
	 ...
	 * in magnitude) */
	if ( nnz > 0 ) {
		if ( fhv->nfs == fhv->nfs_max ) { /* maximal number of row-like factors has been reached */
			return 4;
		}
		k = ++( fhv->nfs );
		hh_ind[k] = p;
	...
	len = 0;
	for ( k = t + 1; k <= n; k++ ) { /* j-th column of V = k-th column of U */
		j = qq_ind[k];
		...
		if ( vc_cap[j] == vc_len[j] ) { /* reserve extra locations in j-th column to reduce further
	...
	/*--------------------------------------------------------------*/
	if ( nnz > 0 ) { /* copy p-th (non-trivial) row of row-like factor H[k] (except
