...
void scf_r_prod( SCF* scf, double y[/*1+nn*/], double a, const double x[/*1+n0*/] ) {
	int nn = scf->nn;
	SVA* sva = scf->sva;
	int* sv_ind = sva->ind;
	...
	int i, ptr, end;
	double t;
	for ( i = 1; i <= nn; i++ ) { /* t := (i-th row of R) * x */
		t = 0.0;
...
void scf_rt_prod( SCF* scf, double y[/*1+n0*/], double a, const double x[/*1+nn*/] ) {
	int nn = scf->nn;
	SVA* sva = scf->sva;
	int* sv_ind = sva->ind;
	double* sv_val = sva->val;
...
void scf_s_prod( SCF* scf, double y[/*1+n0*/], double a, const double x[/*1+nn*/] ) {
	int nn = scf->nn;
	SVA* sva = scf->sva;
	int* sv_ind = sva->ind;
	double* sv_val = sva->val;
...
void scf_st_prod( SCF* scf, double y[/*1+nn*/], double a, const double x[/*1+n0*/] ) {
	int nn = scf->nn;
	SVA* sva = scf->sva;
	int* sv_ind = sva->ind;
	double* sv_val = sva->val;
	...
	int j, ptr, end;
	double t;
	for ( j = 1; j <= nn; j++ ) { /* t := (j-th column of S) * x */
		t = 0.0;
		for ( end = ( ptr = ss_ptr[j] ) + ss_len[j]; ptr < end; ptr++ )
			t += sv_val[ptr] * x[sv_ind[ptr]];
		/* y[j] := y[j] + alpha * t */
...
int scf_update_aug( SCF* scf, double b[/*1+n0*/], double d[/*1+n0*/],
	...
	int k, ret;
	double *v, *w, *x, *y, z;
	if ( scf->nn == scf->nn_max ) { /* updating limit has been reached */
		return 1;
	}
	/* v := inv(R0) * b */
	...
	if ( ret != 0 ) { /* updating IFU-factorization failed */
		return 2;
	}
