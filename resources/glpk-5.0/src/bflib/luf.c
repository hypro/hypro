...
int luf_store_v_cols( LUF* luf, int ( *col )( void* info, int j, int ind[], double val[] ), void* info, int ind[], double val[] ) {
	int n = luf->n;
	SVA* sva = luf->sva;
	int* sv_ind = sva->ind;
...
void luf_check_all( LUF* luf, int k ) {
	...
	for ( j = 1; j <= n; j++ ) { /* j-th column of F = jj-th column of L */
		jj = pp_ind[j];
		if ( jj < k ) {
			j_ptr = fc_ptr[j];
	...
	for ( i = 1; i <= n; i++ ) { /* i-th row of V = ii-th row of U */
		ii = pp_ind[i];
		i_ptr = vr_ptr[i];
	...
	for ( j = 1; j <= n; j++ ) { /* j-th column of V = jj-th column of U */
		jj = qq_inv[j];
		if ( jj < k )
			xassert( vc_len[j] == 0 );
		else /* jj >= k */
...
void luf_f_solve( LUF* luf, double x[/*1+n*/] ) {
	...
	int j, k, ptr, end;
	double x_j;
	for ( k = 1; k <= n; k++ ) { /* k-th column of L = j-th column of F */
		j = pp_inv[k];
...
void luf_ft_solve( LUF* luf, double x[/*1+n*/] ) {
	...
	int i, k, ptr, end;
	double x_i;
	for ( k = n; k >= 1; k-- ) { /* k-th column of L' = i-th row of F */
		i = pp_inv[k];
...
void luf_v_solve( LUF* luf, double b[/*1+n*/], double x[/*1+n*/] ) {
	...
	int i, j, k, ptr, end;
	double x_j;
	for ( k = n; k >= 1; k-- ) { /* k-th row of U = i-th row of V */
...
void luf_vt_solve( LUF* luf, double b[/*1+n*/], double x[/*1+n*/] ) {
	...
	int i, j, k, ptr, end;
	double x_i;
	for ( k = 1; k <= n; k++ ) { /* k-th row of U' = j-th column of V */
...
double luf_estimate_norm( LUF* luf, double w1[/*1+n*/], double w2[/*1+n*/] ) {
	int n = luf->n;
	double* e = w1;
	double* y = w2;
	double* z = w1;
