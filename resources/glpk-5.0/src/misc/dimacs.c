...
void dmx_error( DMX* csa, const char* fmt, ... ) { /* print error message and terminate processing */
	va_list arg;
...
void dmx_warning( DMX* csa, const char* fmt, ... ) { /* print warning message and continue processing */
	va_list arg;
...
void dmx_read_char( DMX* csa ) { /* read character from input text file */
	int c;
	if ( csa->c == '\n' ) csa->count++;
	c = glp_getc( csa->fp );
	if ( c < 0 ) {
...
void dmx_read_designator( DMX* csa ) { /* read one-character line designator */
	xassert( csa->c == '\n' );
	dmx_read_char( csa );
	for ( ;; ) { /* skip preceding white-space characters */
		while ( csa->c == ' ' )
			dmx_read_char( csa );
		if ( csa->c == '\n' ) { /* ignore empty line */
			if ( !csa->empty ) {
			...
			dmx_read_char( csa );
		} else if ( csa->c == 'c' ) { /* skip comment line */
			while ( csa->c != '\n' )
				dmx_read_char( csa );
			dmx_read_char( csa );
		} else { /* hmm... looks like a line designator */
...
void dmx_read_field( DMX* csa ) { /* read data field */
	int len = 0;
...
void dmx_end_of_line( DMX* csa ) { /* skip white-space characters until end of line */
	while ( csa->c == ' ' )
		dmx_read_char( csa );
	if ( csa->c != '\n' )
...
void dmx_check_int( DMX* csa, double num ) { /* print a warning if non-integer data are detected */
