...
struct dsa { /* working area used by routine hbm_read_mat */
	const char* fname;
...
static int read_card( struct dsa* dsa ) {
	...
	if ( len > 80 )
	err : {
		xerror( "%s:%d: card image too long\n",
				dsa->fname, dsa->seqn );
		return 1;
	}
...
static int parse_fmt( struct dsa* dsa, char* fmt ) {
	...
	if ( fmt[0] != '(' )
	fail : {
		xprintf( "hbm_read_mat: format '%s' not recognised\n", fmt );
		return 1;
	}
		k = 1;
...
HBM* hbm_read_mat( const char* fname ) {
	...
	if ( dsa->fp == NULL ) {  xprintf("hbm_read_mat: unable to open '%s' - %s\n",
#if 0 /* 29/I-2017 */
	...
	if ( hbm->rhstyp[0] == 'F' ) { /* dense format */
	...
	} else if ( hbm->rhstyp[0] == 'M' && hbm->mxtype[2] == 'A' ) { /* sparse format */
		/* read pointers */
		...
		if ( read_real_array( dsa, "rhsval", hbm->rhsfmt, hbm->nrhsix,
							  hbm->rhsval ) ) goto fail;
	} else if ( hbm->rhstyp[0] == 'M' && hbm->mxtype[2] == 'E' ) { /* elemental format */
		hbm->rhsval = xcalloc( 1 + hbm->nrhsvl, sizeof( double ) );
		if ( read_real_array( dsa, "rhsval", hbm->rhsfmt, hbm->nrhsvl,
							  hbm->rhsval ) ) goto fail;
	} else {
