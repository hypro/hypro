...
int wclique1( int n, const double w[],
	...
	for ( l = 1; l <= n; l++ ) { /* take vertex i */
		...
		if ( c_wght + d_wght < best + 1e-5 * ( 1.0 + fabs( best ) ) ) { /* skip constructing the current clique */
			goto next;
		}
		...
		while ( d_size > 0 ) {												/* check an upper bound to the final clique weight */
			if ( c_wght + d_wght < best + 1e-5 * ( 1.0 + fabs( best ) ) ) { /* skip constructing the current clique */
				goto next;
			}
			...
			for ( k = 1; k <= kk; k++ ) {
				j = d_list[k];
				if ( d_flag[j] == 1 ) { /* remove vertex j from D */
					d_flag[j] = 0;
					d_wght -= w[j];
				} else if ( d_flag[j] == 2 ) { /* keep vertex j in D */
					d_list[++d_size] = j;
					d_flag[j] = 1;
				} else
