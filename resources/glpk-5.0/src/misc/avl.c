...
struct AVL { /* AVL tree (Adelson-Velsky & Landis binary search tree) */
	DMP* pool;
...
struct AVLNODE { /* node of AVL tree */
	const void* key;
...
int avl_strcmp( void* info, const void* key1, const void* key2 ) { /* compare character string keys */
	xassert( info == info );
...
AVLNODE* avl_insert_node( AVL* tree, const void* key ) { /* insert new node into AVL tree */
	AVLNODE *p, *q, *r;
	short int flag;
	...
	/* go upstairs to the root and correct all subtrees affected by
	   insertion */
	while ( p != NULL ) {
		if ( flag == 0 ) { /* the height of the left subtree of [p] is increased */
			if ( p->bal > 0 ) {
				p->bal = 0;
				break;
			}
			...
			flag = p->flag;
			p = p->up;
		} else { /* the height of the right subtree of [p] is increased */
			if ( p->bal < 0 ) {
				p->bal = 0;
				break;
			}
...
void avl_set_node_type( AVLNODE* node, int type ) { /* assign the type field of specified node */
	node->type = type;
	return;
}

void avl_set_node_link( AVLNODE* node, void* link ) { /* assign the link field of specified node */
	node->link = link;
	return;
}

AVLNODE* avl_find_node( AVL* tree, const void* key ) { /* find node in AVL tree */
	AVLNODE* p;
	int c;
	p = tree->root;
	while ( p != NULL ) {
...
int avl_get_node_type( AVLNODE* node ) { /* retrieve the type field of specified node */
	return node->type;
}

void* avl_get_node_link( AVLNODE* node ) { /* retrieve the link field of specified node */
	return node->link;
}

static AVLNODE* find_next_node( AVL* tree, AVLNODE* node ) { /* find next node in AVL tree */
	AVLNODE *p, *q;
	if ( tree->root == NULL ) return NULL;
	p = node;
	q = ( p == NULL ? tree->root : p->right );
	if ( q == NULL ) { /* go upstairs from the left subtree */
		for ( ;; ) {
			q = p->up;
			if ( q == NULL ) break;
			if ( p->flag == 0 ) break;
	...
	} else { /* go downstairs into the right subtree */
		for ( ;; ) {
			p = q->left;
			if ( p == NULL ) break;
			q = p;
...
void avl_delete_node( AVL* tree, AVLNODE* node ) { /* delete specified node from AVL tree */
...
skip: /* now the specified node [p] has at least one empty subtree;
	...
	/* go upstairs to the root and correct all subtrees affected by
	   deletion */
	while ( f != NULL ) {
		if ( flag == 0 ) { /* the height of the left subtree of [f] is decreased */
			if ( f->bal == 0 ) {
				f->bal = +1;
				break;
			}
			...
			flag = f->flag;
			f = f->up;
		} else { /* the height of the right subtree of [f] is decreased */
			if ( f->bal == 0 ) {
				f->bal = -1;
				break;
			}
...
static AVLNODE* rotate_subtree( AVL* tree, AVLNODE* node ) { /* restore balance of AVL subtree */
	AVLNODE *f, *p, *q, *r, *x, *y;
	xassert( node != NULL );
	p = node;
	if ( p->bal < 0 ) { /* perform negative (left) rotation */
		f = p->up;
		q = p->left;
		r = q->right;
		if ( q->bal <= 0 ) { /* perform single negative rotation */
			if ( f == NULL )
				tree->root = q;
			else if ( p->flag == 0 )
				f->left = q;
			...
			node = q;
		} else { /* perform double negative rotation */
			x = r->left;
			y = r->right;
			if ( f == NULL )
				tree->root = r;
	...
	} else { /* perform positive (right) rotation */
		f = p->up;
		q = p->right;
		r = q->left;
		if ( q->bal >= 0 ) { /* perform single positive rotation */
			if ( f == NULL )
				tree->root = q;
			else if ( p->flag == 0 )
				f->left = q;
			...
			node = q;
		} else { /* perform double positive rotation */
			x = r->left;
			y = r->right;
			if ( f == NULL )
				tree->root = r;
...
void avl_delete_tree( AVL* tree ) { /* delete AVL tree */
