...
static void put_word( FILE* fp, int w ) { /* big endian */
	put_byte( fp, w );
	put_byte( fp, w >> 8 );
	return;
}

static void put_dword( FILE* fp, int d ) { /* big endian */
	put_word( fp, d );
	put_word( fp, d >> 16 );
	return;
}
...
int rgr_write_bmp16( const char* fname, int m, int n, const char map[] ) {
	...
	fp = fopen( fname, "wb" );
	if ( fp == NULL ) {  xprintf("rgr_write_bmp16: unable to create '%s' - %s\n",
#if 0 /* 29/I-2017 */
	...
	fflush( fp );
	if ( ferror( fp ) ) {  xprintf("rgr_write_bmp16: write error on '%s' - %s\n",
#if 0 /* 29/I-2017 */
            fname, strerror(errno));
#else
				 fname, xstrerr( errno ) );
