...
int kellerman( int n, int ( *func )( void* info, int i, int ind[] ),
	...
	for ( i = 1; i <= n; i++ ) { /* W must be empty */
		...
		if ( W->size == 0 ) { /* set k := k + 1 and create new clique C[k] = { i } */
		...
		k = H->nv - n;
		for ( m = 1; m <= k; m++ ) { /* do while V != W; since here V is within W, we can use
		...
		for ( t = 1; t <= V->size; t++ ) {
			...
			if ( W->pos[j] != 0 ) { /* remove vertex j from W */
				if ( W->pos[j] != W->size ) {
		...
		while ( W->size > 0 ) { /* find clique C[m], 1 <= m <= k, which shares maximal
								 * number of vertices with W; to break ties choose clique
								 * having smallest number m */
			m = 0, best = -1;
			k = H->nv - n;
			for ( t = 1; t <= k; t++ ) { /* compute cardinality of intersection of W and C[t] */
				card = 0;
			...
			for ( a = H->v[n + m]->in; a != NULL; a = a->h_next ) {
				j = a->tail->i;
				if ( W->pos[j] != 0 ) { /* vertex j is in both W and C[m]; include it in new
										 * clique C[k] */
					glp_add_arc( H, j, n + k );
