...
loop: /* compute constraint and objective at current x */
	...
	if ( z_best < z ) { /* better solution has been found */
...
static struct ks* reduce( const int n, const int a[/*1+n*/], int b,
	...
	if ( ks->b < 0 ) { /* instance is infeasible */
		free_ks( ks );
		return NULL;
	}
	...
	for ( j = 1; j <= n; j++ ) {
		if ( ks->a[j] == 0 ) {
			if ( ks->c[j] <= 0 ) { /* fix x[j] at 0 */
				ks->x[j] ^= 0x10;
			} else { /* fix x[j] at 1 */
				ks->x[j] ^= 0x11;
				ks->c0 += ks->c[j];
			}
		} else if ( ks->a[j] > ks->b || ks->c[j] <= 0 ) { /* fix x[j] at 0 */
			ks->x[j] ^= 0x10;
		} else { /* include x[j] in reduced instance */
			ks->n++;
			ks->a[ks->n] = ks->a[j];
			ks->c[ks->n] = ks->c[j];
		}
...
static int mt1a( int n, const int a[], int b, const int c[], char x[] ) { /* interface routine to MT1 */
	struct mt* mt;
...
int ks_mt1( int n, const int a[/*1+n*/], int b, const int c[/*1+n*/],
	...
	ks = reduce( n, a, b, c );
	if ( ks == NULL ) { /* original instance is infeasible */
		return INT_MIN;
	}
...
static int greedy( int n, const int a[], int b, const int c[], char x[] ) { /* core routine for normalized 0-1 knapsack instance */
	struct mt* mt;
	int j, s, z;
	xassert( n >= 2 );
	/* reorder items to provide c[j] / a[j] >= a[j+1] / a[j+1] */
