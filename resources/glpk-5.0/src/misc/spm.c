...
void spm_delete_mat( SPM* A ) { /* delete sparse matrix */
...
int spm_drop_zeros( SPM* A, double eps ) {
	...
	for ( i = 1; i <= A->m; i++ ) {
		for ( e = A->row[i]; e != NULL; e = next ) {
			next = e->r_next;
			if ( e->val == 0.0 || fabs( e->val ) < eps ) { /* remove element from the row list */
				if ( e->r_prev == NULL )
					A->row[e->i] = e->r_next;
				else
...
SPM* spm_read_mat( const char* fname ) {
	...
	for ( k = 1; k <= nnz; k++ ) { /* row index, column index, element value */
		i = pds_scan_int( pds );
		if ( !( 1 <= i && i <= m ) )
...
int spm_write_mat( const SPM* A, const char* fname ) {
	...
	for ( i = 1; i <= A->m; i++ ) {
		...
		for ( e = A->row[i]; e != NULL; e = e->r_next ) { /* row index, column index, element value */
...
SPM* spm_add_sym( const SPM* A, const SPM* B ) { /* add two sparse matrices (symbolic phase) */
	SPM* C;
	int i, j, *flag;
	xassert( A->m == B->m );
	xassert( A->n == B->n );
	...
	for ( i = 1; i <= C->m; i++ ) {
		...
		for ( e = A->row[i]; e != NULL; e = e->r_next ) { /* (note that i-th row of A may have duplicate elements) */
			j = e->j;
			if ( !flag[j] ) {
		...
		for ( e = B->row[i]; e != NULL; e = e->r_next ) { /* (note that i-th row of B may have duplicate elements) */
			j = e->j;
			if ( !flag[j] ) {
				spm_new_elem( C, i, j, 0.0 );
				flag[j] = 1;
...
void spm_add_num( SPM* C, double alfa, const SPM* A, double beta,
				  const SPM* B ) { /* add two sparse matrices (numeric phase) */
	int i, j;
	double* work;
...
SPM* spm_add_mat( double alfa, const SPM* A, double beta, const SPM* B ) { /* add two sparse matrices (driver routine) */
	SPM* C;
	C = spm_add_sym( A, B );
...
SPM* spm_mul_sym( const SPM* A, const SPM* B ) { /* multiply two sparse matrices (symbolic phase) */
	int i, j, k, *flag;
	SPM* C;
	xassert( A->n == B->m );
	/* create resultant matrix */
	...
	for ( i = 1; i <= C->m; i++ ) {
		...
		for ( e = A->row[i]; e != NULL; e = e->r_next ) {
			...
			for ( ee = B->row[k]; ee != NULL; ee = ee->r_next ) {
				...
				if ( !flag[j] ) { /* c[i,j] does not exist, so create it */
					spm_new_elem( C, i, j, 0.0 );
					flag[j] = 1;
				}
			}
...
void spm_mul_num( SPM* C, const SPM* A, const SPM* B ) { /* multiply two sparse matrices (numeric phase) */
	int i, j;
	double* work;
	/* allocate and clear the working array */
...
SPM* spm_mul_mat( const SPM* A, const SPM* B ) { /* multiply two sparse matrices (driver routine) */
	SPM* C;
	C = spm_mul_sym( A, B );
	spm_mul_num( C, A, B );
	return C;
...
PER* spm_create_per( int n ) { /* create permutation matrix */
	PER* P;
	int k;
	xassert( n >= 0 );
...
void spm_check_per( PER* P ) { /* check permutation matrix for correctness */
	int i, j;
	xassert( P->n >= 0 );
	for ( i = 1; i <= P->n; i++ ) {
		j = P->row[i];
...
void spm_delete_per( PER* P ) { /* delete permutation matrix */
	xfree( P->row );
	xfree( P->col );
	xfree( P );
	return;
