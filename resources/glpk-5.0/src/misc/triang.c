...
int triang( int m, int n, int ( *mat )( void* info, int k, int ind[], double val[] ), void* info, double tol, int rn[], int cn[] ) {
	 ...
	 * column in the list */
	head = 0;
	for ( len = 0; len <= m; len++ ) { /* walk thru list of columns of length len */
	...
	ns = 0;
	for ( i = 1; i <= m; i++ ) { /* get i-th row */
		len = cnt[i] = mat( info, +i, rind, rval );
		xassert( 0 <= len && len <= n );
		if ( len == 1 ) { /* a[i,j] is row singleton */
			j = rind[1];
			xassert( 1 <= j && j <= n );
			if ( flag[j] != 2 ) { /* include j-th column in singleton list */
				flag[j] = 2;
				list[++ns] = j;
			}
		}
	...
	while ( head != 0 ) {
		if ( ns == 0 ) { /* singleton list is empty */
		...
		for ( k = 1; k <= len; k++ ) {
			i = cind[k];
			xassert( 1 <= i && i <= m );
			if ( cnt[i] == 1 ) { /* a[i,j] is row singleton */
		...
		if ( fabs( cval[kk] ) < tol * big[j] ) { /* all row singletons are too small in magnitude; drop j-th
												  * column */
			goto drop;
		}
	...
	drop: /* remove j-th column from the active submatrix */
		...
		for ( k = 1; k <= len; k++ ) {
			...
			xassert( cnt[i] > 0 );
			cnt[i]--;
			if ( cnt[i] == 1 ) { /* new singleton appeared in i-th row; determine number
				...
				jj = rind[ks];
				if ( flag[jj] != 2 ) { /* include jj-th column in the singleton list */
					flag[jj] = 2;
					list[++ns] = jj;
				}
			}
