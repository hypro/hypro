...
struct DMP { /* dynamic memory pool */
	void* avail[32];
...
struct prefix { /* atom prefix (for debugging only) */
	DMP* pool;
	/* dynamic memory pool */
	int size;
...
void* dmp_get_atom( DMP* pool, int size ) {
	...
	k = ( need >> 3 ) - 1;
	/* obtain free atom */
	if ( pool->avail[k] == NULL ) { /* corresponding list of free atoms is empty */
		/* if debug mode is on, add atom prefix size */
		if ( dmp_debug )
			need += prefix_size;
		if ( pool->used + need > DMP_BLK_SIZE ) { /* allocate new memory block */
		...
		pool->used += need;
	} else { /* obtain atom from corresponding list of free atoms */
		atom = pool->avail[k];
