...
int mc21a( int n, const int icn[], const int ip[], const int lenr[],
	...
	for ( jord = 1; jord <= n; jord++ ) {
		j = jord;
		pr[j] = -1;
		for ( k = 1; k <= jord; k++ ) { /* Look for a cheap assignment. */
			in1 = arp[j];
			if ( in1 >= 0 ) {
				in2 = ip[j] + lenr[j] - 1;
				in1 = in2 - in1;
			...
			for ( kk = 1; kk <= jord; kk++ ) {
				...
				if ( in1 >= 0 ) {
					...
					for ( ii = in1; ii <= in2; ii++ ) {
						i = icn[ii];
						if ( cv[i] != jord ) { /* Column i has not yet been accessed during
												* this pass. */
							j1 = j;
							j = iperm[i];
							cv[i] = jord;
...
int main( void ) { /* test program for the routine mc21a */
...
void ranmat( int m, int n, int icn[], int iptr[], int nnnp1, int* knum,
			 int iw[] ) { /* subroutine to generate random matrix */
...
double fa01as( int i ) { /* random number generator */
