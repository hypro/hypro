...
mpz_t _mpz_init( void ) { /* initialize x and set its value to 0 */
	mpz_t x;
...
void mpz_clear( mpz_t x ) { /* free the space occupied by x */
	mpz_set_si( x, 0 );
	xassert( x->ptr == NULL );
...
void mpz_set( mpz_t z, mpz_t x ) { /* set the value of z from x */
...
void mpz_set_si( mpz_t x, int val ) { /* set the value of x to val */
	struct mpz_seg* e;
	...
	/* assign new value */
	if ( val == 0x80000000 ) { /* long format is needed */
		x->val = -1;
		x->ptr = e = gmp_get_atom( sizeof( struct mpz_seg ) );
		memset( e->d, 0, 12 );
		e->d[1] = 0x8000;
		e->next = NULL;
	} else { /* short format is enough */
		x->val = val;
	}
	return;
}

double mpz_get_d( mpz_t x ) { /* convert x to a double, truncating if necessary */
	struct mpz_seg* e;
	int j;
	double val, deg;
	if ( x->ptr == NULL )
...
double mpz_get_d_2exp( int* exp, mpz_t x ) { /* convert x to a double, truncating if necessary (i.e. rounding
...
void mpz_swap( mpz_t x, mpz_t y ) { /* swap the values x and y efficiently */
	int val;
	void* ptr;
	val = x->val, ptr = x->ptr;
...
static void normalize( mpz_t x ) { /* normalize integer x that includes removing non-significant
...
void mpz_add( mpz_t z, mpz_t x, mpz_t y ) { /* set z to x + y */
	...
	sz = sx;
	ez = es = NULL;
	if ( sx > 0 && sy > 0 || sx < 0 && sy < 0 ) { /* [x] and [y] have identical signs -- addition */
		t = 0;
		...
		if ( t ) { /* overflow -- one extra digit is needed */
			ee = gmp_get_atom( sizeof( struct mpz_seg ) );
			ee->d[0] = 1;
	...
	} else { /* [x] and [y] have different signs -- subtraction */
		t = 1;
		for ( ; ex || ey; ex = ex->next, ey = ey->next ) {
			if ( ex == NULL )
				ex = &zero;
		...
		if ( !t ) { /* |[x]| < |[y]| -- result in complement coding */
			sz = -sz;
			t = 1;
...
void mpz_sub( mpz_t z, mpz_t x, mpz_t y ) { /* set z to x - y */
	if ( x == y )
		mpz_set_si( z, 0 );
	else {
		y->val = -y->val;
...
void mpz_mul( mpz_t z, mpz_t x, mpz_t y ) { /* set z to x * y */
...
void mpz_neg( mpz_t z, mpz_t x ) { /* set z to 0 - x */
	mpz_set( z, x );
	z->val = -z->val;
	return;
}

void mpz_abs( mpz_t z, mpz_t x ) { /* set z to the absolute value of x */
	mpz_set( z, x );
	if ( z->val < 0 )
		z->val = -z->val;
	return;
...
void mpz_div( mpz_t q, mpz_t r, mpz_t x, mpz_t y ) { /* divide x by y, forming quotient q and/or remainder r
...
void mpz_gcd( mpz_t z, mpz_t x, mpz_t y ) { /* set z to the greatest common divisor of x and y */
...
int mpz_sgn( mpz_t x ) { /* return +1 if x > 0, 0 if x = 0, and -1 if x < 0 */
	int s;
	s = ( x->val > 0 ? +1 : x->val < 0 ? -1
									   : 0 );
	return s;
}

int mpz_out_str( void* _fp, int base, mpz_t x ) { /* output x on stream fp, as a string in given base; the base
...
mpq_t _mpq_init( void ) { /* initialize x, and set its value to 0/1 */
	mpq_t x;
...
void mpq_clear( mpq_t x ) { /* free the space occupied by x */
	mpz_set_si( &x->p, 0 );
...
void mpq_canonicalize( mpq_t x ) { /* remove any factors that are common to the numerator and
...
void mpq_set( mpq_t z, mpq_t x ) { /* set the value of z from x */
	if ( z != x ) {
		mpz_set( &z->p, &x->p );
		mpz_set( &z->q, &x->q );
	}
...
void mpq_set_si( mpq_t x, int p, unsigned int q ) { /* set the value of x to p/q */
	if ( q == 0 )
...
double mpq_get_d( mpq_t x ) { /* convert x to a double, truncating if necessary */
	int np, nq;
	double p, q;
...
void mpq_set_d( mpq_t x, double val ) { /* set x to val; there is no rounding, the conversion is exact */
	int s, n, d, j;
	double f;
	mpz_t temp;
...
void mpq_add( mpq_t z, mpq_t x, mpq_t y ) { /* set z to x + y */
	mpz_t p, q;
	mpz_init( p );
	mpz_init( q );
	mpz_mul( p, &x->p, &y->q );
...
void mpq_sub( mpq_t z, mpq_t x, mpq_t y ) { /* set z to x - y */
	mpz_t p, q;
	mpz_init( p );
	mpz_init( q );
	mpz_mul( p, &x->p, &y->q );
...
void mpq_mul( mpq_t z, mpq_t x, mpq_t y ) { /* set z to x * y */
	mpz_mul( &z->p, &x->p, &y->p );
	mpz_mul( &z->q, &x->q, &y->q );
	mpq_canonicalize( z );
	return;
...
void mpq_div( mpq_t z, mpq_t x, mpq_t y ) { /* set z to x / y */
	mpz_t p, q;
	if ( mpq_sgn( y ) == 0 )
...
void mpq_neg( mpq_t z, mpq_t x ) { /* set z to 0 - x */
	mpq_set( z, x );
	mpz_neg( &z->p, &z->p );
	return;
}

void mpq_abs( mpq_t z, mpq_t x ) { /* set z to the absolute value of x */
	mpq_set( z, x );
	mpz_abs( &z->p, &z->p );
...
int mpq_cmp( mpq_t x, mpq_t y ) { /* compare x and y; return a positive value if x > y, zero if
...
int mpq_sgn( mpq_t x ) { /* return +1 if x > 0, 0 if x = 0, and -1 if x < 0 */
	int s;
	s = mpz_sgn( &x->p );
	xassert( mpz_sgn( &x->q ) > 0 );
	return s;
...
int mpq_out_str( void* _fp, int base, mpq_t x ) { /* output x on stream fp, as a string in given base; the base
