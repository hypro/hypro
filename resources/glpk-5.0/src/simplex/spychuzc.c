...
int spy_chuzc_std( SPXLP* lp, const double d[/*1+n-m*/],
	...
	for ( j = 1; j <= n - m; j++ ) {
		...
		if ( l[k] == u[k] )
			continue;
		alfa = s * trow[j];
		if ( alfa >= +tol_piv && !flag[j] ) { /* xN[j] is either free or has its lower bound active, so
		...
		} else if ( alfa <= -tol_piv && ( l[k] == -DBL_MAX || flag[j] ) ) { /* xN[j] is either free or has its upper bound active, so
																			 * lambdaN[j] = d[j] <= 0 increases up to zero */
			delta = tol + tol1 * ( c[k] >= 0.0 ? +c[k] : -c[k] );
			/* determine theta on which lambdaN[j] reaches zero */
			teta = ( d[j] > -delta ? 0.0 : d[j] / alfa );
		} else { /* lambdaN[j] cannot reach zero on increasing theta */
			continue;
		}
...
int spy_chuzc_harris( SPXLP* lp, const double d[/*1+n-m*/],
	...
	/*--------------------------------------------------------------*/
	if ( teta_min == DBL_MAX ) { /* theta may increase unlimitedly */
		q = 0;
		goto done;
	}
	/* nothing is chosen so far */
	...
	for ( j = 1; j <= n - m; j++ ) {
		...
		if ( alfa >= +tol_piv && !flag[j] ) { /* xN[j] is either free or has its lower bound active, so
			...
			if ( l[k] == -DBL_MAX && teta_max > teta )
				teta_max = teta;
		} else { /* lambdaN[j] cannot reach zero on increasing teta */
			continue;
		}
		/* add lambdaN[j] to the list */
		nnn++;
...
int spy_ls_select_bp( SPXLP* lp, const double trow[/*1+n-m*/],
					  int nbp, SPYBP bp[/*1+n-m*/], int num, double* slope, double teta_lim ) {
	int m = lp->m;
	int n = lp->n;
	double* l = lp->l;
	double* u = lp->u;
	...
	for ( t = num + 1; t <= num1; t++ ) { /* calculate the dual objective change relative to its value
