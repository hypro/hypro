...
void spx_build_at( SPXLP* lp, SPXAT* at ) {
	...
	for ( k = n; k >= 1; k-- ) { /* copy elements from k-th column to corresponding rows */
		ptr = A_ptr[k];
		end = A_ptr[k + 1];
		for ( ; ptr < end; ptr++ ) {
...
void spx_at_prod( SPXLP* lp, SPXAT* at, double y[/*1+n*/], double s,
	...
	double t;
	for ( i = 1; i <= m; i++ ) {
		if ( x[i] != 0.0 ) { /* y := y + s * (i-th row of A) * x[i] */
			t = s * x[i];
			ptr = AT_ptr[i];
			end = AT_ptr[i + 1];
			for ( ; ptr < end; ptr++ )
...
void spx_eval_trow1( SPXLP* lp, SPXAT* at, const double rho[/*1+m*/],
	...
	if ( cnt1 < cnt2 ) { /* as inner products */
		int* A_ptr = lp->A_ptr;
		int* A_ind = lp->A_ind;
		double* A_val = lp->A_val;
		int* head = lp->head;
