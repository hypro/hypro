...
static void set_art_bounds( struct csa* csa ) {
	...
	for ( k = 1; k <= n; k++ ) {
		if ( csa->orig_l[k] == -DBL_MAX && csa->orig_u[k] == +DBL_MAX ) { /* force free variables to enter the basis */
			l[k] = -1e3, u[k] = +1e3;
...
static int check_feas( struct csa* csa, double tol, double tol1,
	...
	for ( j = 1; j <= n - m; j++ ) {
		...
		if ( d[j] > +eps ) {					 /* xN[j] should have its lower bound active */
			if ( l[k] == -DBL_MAX || flag[j] ) { /* but it either has no lower bound or its lower bound
												  * is inactive */
				if ( l[k] == -DBL_MAX ) {		 /* cannot recover, since xN[j] has no lower bound */
					ret = j;
					break;
				}
		...
		} else if ( d[j] < -eps ) {		  /* xN[j] should have its upper bound active */
			if ( !flag[j] ) {			  /* but it either has no upper bound or its upper bound
										   * is inactive */
				if ( u[k] == +DBL_MAX ) { /* cannot recover, since xN[j] has no upper bound */
					ret = j;
					break;
				}
				/* recovering is possible */
	...
	if ( recov && ret ) { /* invalidate values of basic variables, since active bounds
...
#if 1 /* 30/III-2016 */
static void spy_eval_r( SPXLP* lp, const double beta[/*1+m*/], double tol,
						double tol1, FVS* r ) { /* this routine computes the vector of primal infeasibilities:
												 *
	...
	for ( i = 1; i <= m; i++ ) {
		...
		if ( beta[i] < lk ) { /* determine absolute tolerance eps1[i] */
			eps = tol + tol1 * ( lk >= 0.0 ? +lk : -lk );
			if ( beta[i] < lk - eps ) { /* lower bound is violated */
				ind[++nnz] = i;
				vec[i] = lk - beta[i];
			}
		} else if ( beta[i] > uk ) { /* determine absolute tolerance eps2[i] */
			eps = tol + tol1 * ( uk >= 0.0 ? +uk : -uk );
			if ( beta[i] > uk + eps ) { /* upper bound is violated */
				ind[++nnz] = i;
				vec[i] = uk - beta[i];
			}
		}
...
more: /* initial number of eligible basic variables */
	...
	try : /* choose basic variable xB[p] */
		...
		;
	try++;
	if ( se == NULL ) { /* dual Dantzig's rule */
		p = spy_chuzr_std( lp, beta, nnn, list );
	} else { /* dual projected steepest edge */
	...
	if ( csa->r_test == GLP_RT_FLIP && try <= 2 ) { /* long-step ratio test */
#if 0 /* 23/III-2016 */
		...
		while ( num < nbp ) { /* select and process a new portion of break-points */
			...
			if ( slope < 0.0 ) { /* the dual objective starts decreasing */
				break;
			}
...
#endif
#if 1 /* FIXME */
	if ( ret ) { /* invalidate basic solution components */
#if 0 /* 28/III-2016 */
...
static void play_coef( struct csa* csa, int all ) {
	...
	for ( j = 1; j <= n - m; j++ ) {
		if ( all || trow[j] != 0.0 ) {							 /* d[j] has changed in the adjacent basis */
			k = head[m + j];									 /* x[k] = xN[j] */
			if ( l[k] == u[k] ) {								 /* xN[j] is fixed variable */
																 /* d[j] may have any sign */
			} else if ( l[k] == -DBL_MAX && u[k] == +DBL_MAX ) { /* xN[j] is free (unbounded) variable */
			...
			} else if ( !flag[j] ) { /* xN[j] has its lower bound active */
				xassert( l[k] != -DBL_MAX );
				...
				if ( d[j] < +eps )
					c[k] -= d[j] - eps, d[j] = +eps;
			} else { /* xN[j] has its upper bound active */
				xassert( u[k] != +DBL_MAX );
	...
	if ( !spec &&
      ...
      switch (csa->phase)
      {
		case 1:
			...
			for ( j = 1; j <= n - m; j++ ) {
				k = head[m + j];	/* x[k] = xN[j] */
				if ( d[j] > 0.0 ) { /* xN[j] should have lower bound */
					if ( l[k] == -DBL_MAX ) {
						sum += d[j];
						if ( d[j] > +1e-7 )
							nnn++;
					}
				} else if ( d[j] < 0.0 ) { /* xN[j] should have upper bound */
					if ( u[k] == +DBL_MAX ) {
						sum -= d[j];
						if ( d[j] < -1e-7 )
							nnn++;
		...
		case 2:
			...
			for ( j = 1; j <= n - m; j++ ) {
				k = head[m + j];	/* x[k] = xN[j] */
				if ( d[j] > 0.0 ) { /* xN[j] should have its lower bound active */
					if ( l[k] == -DBL_MAX || flag[j] )
						sum += d[j];
				} else if ( d[j] < 0.0 ) { /* xN[j] should have its upper bound active */
					if ( l[k] != u[k] && !flag[j] )
						sum -= d[j];
				}
			}
...
#if 1 /* 31/III-2016 */
static void spy_update_r( SPXLP* lp, int p, int q, const double beta[/*1+m*/],
						  const FVS* tcol, double tol, double tol1, FVS* r ) { /* update vector r of primal infeasibilities */
...
static int dual_simplex( struct csa* csa ) { /* dual simplex method main logic routine */
	SPXLP* lp = csa->lp;
	int m = lp->m;
	int n = lp->n;
	double* l = lp->l;
...
loop: /* main loop starts here */
	...
	if ( !lp->valid ) {
		...
		if ( ret != 0 ) {
			if ( msg_lev >= GLP_MSG_ERR )
				xprintf(
					  "Error: unable to factorize the basis matrix (%d"
					  ")\n",
					  ret );
		...
		if ( cond > 1.0 / DBL_EPSILON ) {
			if ( msg_lev >= GLP_MSG_ERR )
				xprintf(
					  "Error: basis matrix is singular to working prec"
					  "ision (cond = %.3g)\n",
					  cond );
			csa->p_stat = csa->d_stat = GLP_UNDEF;
			ret = GLP_EFAIL;
		...
		if ( cond > 0.001 / DBL_EPSILON ) {
			if ( msg_lev >= GLP_MSG_ERR )
				xprintf(
					  "Warning: basis matrix is ill-conditioned (cond "
					  "= %.3g)\n",
					  cond );
		}
	...
	if ( !csa->d_st ) {
		...
		if ( !csa->phase ) {
			...
			if ( j > 0 ) { /* initial basic solution is dual infeasible and cannot
							* be recovered */
				/* start to search for dual feasible solution */
				set_art_bounds( csa );
				csa->phase = 1;
			} else { /* initial basic solution is either dual feasible or its
...
#if 1 /* 11/VII-2017 */
		if ( perturb <= 0 ) {
			if ( check_feas( csa, tol_dj, tol_dj1, 0 ) ) { /* dual feasibility is broken due to excessive round-off
															* errors */
				if ( perturb < 0 ) {
					if ( msg_lev >= GLP_MSG_ALL )
						xprintf(
							  "Perturbing LP to avoid instability [%d].."
							  ".\n",
							  csa->it_cnt );
					perturb = 1;
					goto loop;
				...
				if ( msg_lev >= GLP_MSG_ERR )
					xprintf(
						  "Warning: numerical instability (dual simplex"
						  ", phase %s)\n",
						  csa->phase == 1 ? "I" : "II" );
				instab++;
				if ( csa->dualp && instab >= 10 ) { /* do not continue the search; report failure */
					if ( msg_lev >= GLP_MSG_ERR )
						xprintf(
							  "Warning: dual simplex failed due to exces"
				...
				j = check_feas( csa, 0.97 * tol_dj, 0.97 * tol_dj1, 1 );
				if ( j > 0 ) { /* dual feasibility cannot be recovered (this may
	...
	if ( csa->phase == 2 && csa->obj_lim != DBL_MAX && spx_eval_obj( lp, beta ) >= csa->obj_lim ) {
...
#endif
	{
		...
		switch ( csa->phase ) {
			case 1:
				...
				set_orig_bounds( csa );
				check_flags( csa );
				if ( check_feas( csa, tol_dj, tol_dj1, 0 ) == 0 ) { /* dual feasible solution found; switch to phase II */
					csa->phase = 2;
					xassert( !csa->beta_st );
					goto loop;
				}
...
#else
	if ( spx_update_d_s( lp, d, csa->p, csa->q, &csa->trow, &csa->tcol ) <= 1e-9 )
#endif
	{ /* successful updating */
		csa->d_st = 2;
	}
...
int spy_dual( glp_prob* P, const glp_smcp* parm ) { /* driver to the dual simplex method */
