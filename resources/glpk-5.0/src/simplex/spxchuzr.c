...
int spx_chuzr_std( SPXLP* lp, int phase, const double beta[/*1+m*/],
	...
	k = head[m + q];							  /* x[k] = xN[q] */
	if ( l[k] == -DBL_MAX || u[k] == +DBL_MAX ) { /* xN[q] has no opposite bound */
		p = 0, *p_flag = 0, teta_min = DBL_MAX, biga = 0.0;
	} else { /* xN[q] have both lower and upper bounds */
	...
	for ( i = 1; i <= m; i++ ) {
		...
		alfa = s * tcol[i];
		if ( alfa <= -tol_piv ) { /* xB[i] decreases */
			/* determine actual lower bound of xB[i] */
			if ( phase == 1 && c[k] < 0.0 ) { /* xB[i] has no actual lower bound */
				continue;
			} else if ( phase == 1 && c[k] > 0.0 ) { /* actual lower bound of xB[i] is its upper bound */
				lk = u[k];
				xassert( lk != +DBL_MAX );
				i_flag = 1;
			} else { /* actual lower bound of xB[i] is its original bound */
				lk = l[k];
				if ( lk == -DBL_MAX )
					continue;
				i_flag = 0;
			...
			if ( beta[i] <= lk + delta )
				...
				teta = ( lk - beta[i] ) / alfa;
		} else if ( alfa >= +tol_piv ) { /* xB[i] increases */
			/* determine actual upper bound of xB[i] */
			if ( phase == 1 && c[k] < 0.0 ) { /* actual upper bound of xB[i] is its lower bound */
				uk = l[k];
				xassert( uk != -DBL_MAX );
				i_flag = 0;
			} else if ( phase == 1 && c[k] > 0.0 ) { /* xB[i] has no actual upper bound */
				continue;
			} else { /* actual upper bound of xB[i] is its original bound */
				uk = u[k];
				if ( uk == +DBL_MAX )
					continue;
				i_flag = 1;
			...
			if ( beta[i] >= uk - delta )
				...
				teta = ( uk - beta[i] ) / alfa;
		} else { /* xB[i] does not depend on teta */
			continue;
		}
...
int spx_chuzr_harris( SPXLP* lp, int phase, const double beta[/*1+m*/],
	...
	/*--------------------------------------------------------------*/
	k = head[m + q];							  /* x[k] = xN[q] */
	if ( l[k] != -DBL_MAX && u[k] != +DBL_MAX ) { /* xN[q] has both lower and upper bounds */
		if ( fabs( l[k] - u[k] ) <= teta_min ) {  /* and reaches its opposite bound */
			p = -1, *p_flag = 0;
			goto done;
		}
	}
	if ( teta_min == DBL_MAX ) { /* teta may increase unlimitedly */
		p = 0, *p_flag = 0;
		goto done;
	}
...
int spx_ls_eval_bp( SPXLP* lp, const double beta[/*1+m*/],
	...
	for ( i = 1; i <= m; i++ ) {
		...
		/* determine alfa such that (delta xB[i]) = alfa * teta */
		alfa = s * tcol[i];
		if ( alfa >= +tol_piv ) {	 /* xB[i] increases on increasing teta */
			if ( l[k] == u[k] ) {	 /* xB[i] is fixed at lB[i] = uB[i] */
				if ( c[k] <= 0.0 ) { /* increasing xB[i] can cross its fixed value lB[i],
				...
				if ( l[k] != -DBL_MAX && c[k] < 0.0 ) { /* increasing xB[i] can cross its lower bound lB[i],
														 * because currently xB[i] < lB[i] */
					nbp++;
					bp[nbp].i = +i;
					bp[nbp].teta = ( l[k] - beta[i] ) / alfa;
				...
				if ( u[k] != +DBL_MAX && c[k] <= 0.0 ) { /* increasing xB[i] can cross its upper bound uB[i],
		...
		} else if ( alfa <= -tol_piv ) { /* xB[i] decreases on increasing teta */
			if ( l[k] == u[k] ) {		 /* xB[i] is fixed at lB[i] = uB[i] */
				if ( c[k] >= 0.0 ) {	 /* decreasing xB[i] can cross its fixed value lB[i],
										  * because currently xB[i] >= lB[i] */
					nbp++;
					bp[nbp].i = +i;
					bp[nbp].teta = ( l[k] - beta[i] ) / alfa;
				...
				if ( l[k] != -DBL_MAX && c[k] >= 0.0 ) { /* decreasing xB[i] can cross its lower bound lB[i],
														  * because currently xB[i] does not violate it */
					nbp++;
					bp[nbp].i = +i;
					bp[nbp].teta = ( l[k] - beta[i] ) / alfa;
				...
				if ( u[k] != +DBL_MAX && c[k] > 0.0 ) { /* decreasing xB[i] can cross its upper bound uB[i],
		...
		} else { /* xB[i] does not depend on teta within a tolerance */
			continue;
		}
...
int spx_ls_select_bp( SPXLP* lp, const double tcol[/*1+m*/],
					  int nbp, SPXBP bp[/*1+m+m+1*/], int num, double* slope, double teta_lim ) {
	int m = lp->m;
	int i, t, num1;
	double teta, dz;
	...
	num1 = num;
	for ( t = num + 1; t <= nbp; t++ ) {
		if ( bp[t].teta <= teta_lim ) { /* move break point to the beginning of the new portion */
			num1++;
	 ...
	 * selected */
	for ( t = num + 1; t <= num1; t++ ) { /* calculate the penalty function change relative to its value
