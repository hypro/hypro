#include <lib/datastructures/HybridAutomaton/State.h>

namespace hydra
{

void State::addTimeToClocks(Number t) {
...
State State::aggregate(const State& in) const {
...
std::pair<bool,State> State::intersect(const State& in) const {
...
State State::applyTimeStep(const hypro::matrix_t<Number>& trafoMatrix, const hypro::vector_t<Number>& trafoVector, Number timeStepSize ) const {
	State res(*this);
