	...
	std::string toFlowstarFormat(const HybridAutomaton<Number>& in, const ReachabilitySettings& settings = ReachabilitySettings() ) {
		...
		if(!in.getLocations().empty()) {
			...
			for(const auto& s : in.getInitialStates() ) {
				res << "\n\t\t" << s.first->getName();
				res << "\n\t\t{";
				auto tmpConstraintSet = boost::get<ConstraintSet<Number>>(boost::apply_visitor(genericConversionVisitor<typename State_t<Number>::repVariant, Number, ConstraintSet<Number>>(representation_name::constraint_set), s.second.getSet()));
