#include "lib/datastructures/HybridAutomaton/Location.h"

namespace hydra
{
Location::Location(unsigned _id) : mFlow(), mExternalInput(), mTransitions(), mInvariant(), mId(_id)
{
}

Location::Location(unsigned _id, const hydra::Location& _loc)
...
Location::Location(unsigned _id, const hypro::matrix_t<Number>& _mat) : mFlow(_mat), mId(_id)
{
}

Location::Location(unsigned _id, const hypro::matrix_t<Number>& _mat, const hydra::Location::transitionSet& _trans, const Condition& _inv)
...
std::ostream& operator<<(std::ostream& _ostr, const Location& _l)
{
