...
namespace hypro {
	...
	template<typename Number>
	void EventTimingContainer<Number>::initialize(mpq_class timeHorizon) {
	...
	void EventTimingContainer<Number>::merge(const EventTimingContainer<Number>& snapshot) {
		...
		for(const auto& transitionTimingPair : snapshot.getTransitionTimings()) {
			...
			if(transitionIt == mTransitionEvents.end()) {
				...
				HierarchicalIntervalVector<CONTAINMENT,mpq_class> tmp = transitionIt->second;
	...
	template<typename Number>
	bool EventTimingContainer<Number>::hasTransitionEvent(const carl::Interval<mpq_class>& timeInterval, Transition<Number>* transition) const {
	...
	template<typename Number>
	bool EventTimingContainer<Number>::hasBadStateEvent(const carl::Interval<mpq_class>& timeInterval, CONTAINMENT type) const {
	...
	void EventTimingContainer<Number>::insertTransition(Transition<Number>* transition, const carl::Interval<mpq_class>& timeInterval, CONTAINMENT type) {
		...
		if(mTransitionEvents.find(transition) == mTransitionEvents.end()) {
			...
			HierarchicalIntervalVector<CONTAINMENT,mpq_class> tmp(std::vector<CONTAINMENT>({CONTAINMENT::BOT, CONTAINMENT::PARTIAL, CONTAINMENT::FULL, CONTAINMENT::NO, CONTAINMENT::YES}));
	...
	template<typename Number>
	const std::map<Transition<Number>*, HierarchicalIntervalVector<CONTAINMENT,mpq_class>>& EventTimingContainer<Number>::getTransitionTimings() const {
	...
	template<typename Number>
	const HierarchicalIntervalVector<CONTAINMENT,mpq_class>& EventTimingContainer<Number>::getInvariantTimings() const {
	...
	template<typename Number>
	const HierarchicalIntervalVector<CONTAINMENT,mpq_class>& EventTimingContainer<Number>::getBadStateTimings() const {
