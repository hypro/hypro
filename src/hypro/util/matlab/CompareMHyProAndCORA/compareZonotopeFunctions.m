function compareZonotopeFunctions()

% This file contains the comparison of zonotope functions for mhypro and
% cora.
...
% range_max = 100;
% 
% generators1 = randi( [range_min range_max], dim1 , dim2 );
% center1 = randi( [range_min range_max], dim1 , 1 );
...
mh_z1 = MHyProZonotope(center1, generators1);
cora_z1 = zonotope([center1, generators1]);

mh_c = mh_z1.center();
cora_c = cora_z1.center();
assert(isequal(mh_c, cora_c));

mh_g = mh_z1.generators();
cora_g = cora_z1.generators();
assert(isequal(mh_g, cora_g));

mh_z2 = MHyProZonotope(center2, generators2);
cora_z2 = zonotope([center2, generators2]);
...
mh_ms = mh_z1 + mh_z2;
stop_mh = toc;
disp(['MHyPro needed: ', num2str(stop_mh), 's']);

tic;
cora_ms = cora_z1 + cora_z2;
stop_cora = toc;
disp(['CORA needed: ', num2str(stop_cora), 's']);
...
disp('---------------------------------------');
tic;
mh_center = mh_ms.center();
stop = toc;
disp(['MHyPro needed ' , num2str(stop), 's']);

tic;
cora_center = cora_ms.center();
stop = toc;
...
tic;
mh_generators = mh_ms.generators();
stop = toc;
disp(['MHyPro needed ' , num2str(stop), 's']);

tic;
cora_generators = cora_ms.generators();
stop = toc;
disp(['CORA needed ' , num2str(stop), 's']);

assert(isequal(mh_center, cora_center));
assert(isequal(mh_generators, cora_generators));
...
tic;
mh_z_united = mh_z1.unite(mh_z2);
stop = toc;
disp(['MHyPro needed ' , num2str(stop), 's']);

tic;
cora_z_encl = enclose(cora_z1, cora_z2);
stop = toc;
disp(['CORA needed ' , num2str(stop), 's']);

mh_c = mh_z_united.center();
mh_g = mh_z_united.generators();
cora_c = cora_z_encl.center();
cora_g = cora_z_encl.generators();
...
disp(' ');
disp('---------------------------------------');
disp('Satisfies halfspace/constraint');
disp('---------------------------------------');

C = [-107 -182 814 549 -771 67 -791 120 -422 -459;...
...
mh_z3 = MHyProZonotope(center3, generators3);
cora_z3 = zonotope([center3, generators3]);

C = randi( [-1000 1000], 100);
d = randi( [-1000 1000], 100 , 1 );

tic;
[containment, ~] = mh_z3.satisfiesHalfspaces(C,d);
stop = toc;
disp(['MHyPro needed ' , num2str(stop), 's']);

tic;
out = cora_z3.constrSat(C,d);
stop = toc;
disp(['CORA needed ' , num2str(stop), 's']);
...
gen_point = [1 0 1 0 1; 0 1 0 2 2; 0 1 0 -2 2; 2 0 2 0 2;0 0 0 0 2];
cen_point = [2;0; 4; 1; 2];

mh_point = MHyProZonotope(cen_point, gen_point);
cora_point = zonotope([cen_point, gen_point]);

point = [4;0;4;1;2];

tic;
mh_out = mh_point.contains(point);
stop = toc;
disp(['MHyPro needed ' , num2str(stop), 's']);

tic;
cora_out = cora_point.containsPoint(point);
stop = toc;
disp(['CORA needed ' , num2str(stop), 's']);
...
tic;
mh_out = mh_z1.isempty();
stop = toc;
disp(['MHyPro needed ' , num2str(stop), 's']);

tic;
cora_out = cora_z1.isempty();
stop = toc;
disp(['CORA needed ' , num2str(stop), 's']);
...
disp(' ');
disp('---------------------------------------');
disp('unite equal vectors/delete aligned');
disp('---------------------------------------');
...
disp(' ');
disp('---------------------------------------');
disp('remove empty generators/ delete zeros');
disp('---------------------------------------');

gen4 = [1 0 0; 10 -1 0; 2 0 0; 0 0 0];
cen = [3; 4; 5; 1];

mh_z1 = MHyProZonotope(cen, gen4);
cora_z1 = zonotope([cen, gen4]);

tic;
mh_z1.removeEmptyGenerators();
stop = toc;
disp(['MHyPro needed ' , num2str(stop), 's']);

tic;
cora_out = cora_z1.deleteZeros();
stop = toc;
disp(['CORA needed ' , num2str(stop), 's']);
