...
offset = 0.003; % How much away from the marker the text should be.
...
for k = 1 : length(y)
  if index(k) && mod(k,2)== 1
    ...
    text(x(k) + offset, y(k) + offset, caption, 'BackgroundColor', 'w', 'FontSize',15);
      ...
      caption = sprintf('t=%.2f, x=%.2f', x(k), y(k));
        text(x(k) - 0.043, y(k) + offset, caption, 'BackgroundColor', 'w', 'FontSize',15);
  end
end
plot(1.435, 0, 'ok', 'LineWidth', 3);
caption = sprintf('t=%.3f, x=%.3f',1.435, 0);
text(1.435 - 0.045, 0 + 0.0033, caption, 'BackgroundColor', 'y', 'FontSize',15);
end
