...
template<class T> class ClassHandle{
public:
    ClassHandle(T *ptr) : signature_m(CLASS_HANDLE_SIGNATURE), name_m(typeid(T).name()), ptr_m(ptr) {}
...
template<class T> inline mxArray *convertPtr2Mat(T *ptr){
    mexLock();
...
template<class T> inline ClassHandle<T> *convertMat2HandlePtr(const mxArray *in){
