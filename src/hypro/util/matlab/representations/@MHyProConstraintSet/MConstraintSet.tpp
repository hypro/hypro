...
void MConstraintSet::new_empty(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    if(nlhs != 1)
        mexErrMsgTxt("MConstraintSet - new_empty: One output expected.");
    if(nrhs > 2)
        mexWarnMsgTxt("MConstraintSet - new_empty: One or more input arguments were ignored.");
...
void MConstraintSet::isAxisAligned(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]){
    if(nlhs < 1)
        mexErrMsgTxt("MConstraintSet - isAxisAligned: Expecting an output!");
    if(nrhs < 3)
        mexErrMsgTxt("MConstraintSet - isAxisAligned: One argument missing!");
    if(nrhs > 3)
        mexWarnMsgTxt("MConstraintSet - isAxisAligned: One or more input arguments were ignored.");
...
void MConstraintSet::addConstraint(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]){
    if(nrhs < 5)
        mexErrMsgTxt("MConstraintSet - addConstraint: One or more arguments are missing!");
    if(nrhs > 5)
        mexWarnMsgTxt("MConstraintSet - addConstraint: One or more input arguments were ignored.");
...
void MConstraintSet::type(int nlhs, mxArray* plhs[], int rhs, const mxArray* prhs[]){
    if(nlhs != 1)
        mexErrMsgTxt("MConstraintSet - type: Expecting one output value!");
...
void MConstraintSet::process(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    ...
    if (nrhs < 1 || mxGetString(prhs[1], cmd, sizeof(cmd)))
        mexErrMsgTxt("MConstraintSet - First input should be a command string less than 64 characters long.");
