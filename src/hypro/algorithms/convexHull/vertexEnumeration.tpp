...
namespace hypro {
	...
	void VertexEnumeration<Number>::enumerateVerticesNaive() {
		if(!mHsv.empty()) {
			...
			while(!permutator.end()) {
				...
				if ( lu_decomp.rank() < A.rows() ) { // if the problem is not fully determined, continue - the vertex is not described properly.
					continue;
				}
				// get solution
	...
	void VertexEnumeration<Number>::enumerateDictionaries() {
		...
		Eigen::Index& i=a;
		Eigen::Index& j=b;
		Eigen::Index m = Eigen::Index(basisAux.size()); // different than the paper. M holds the number of constraints which originally went through the origin.
	...
	Dictionary<Number> VertexEnumeration<Number>::findFirstVertex() {
		...
		Eigen::Index n0 = Eigen::Index(mHsv.size()); 				// holds the original size (note: lineality adds half-spaces later).
		...
		while(dictionary.fixOutOfBounds()) {} // Pivot and update assignments until a point in the satisfyable region is reached. Throws exception if unsat.
		...
		for(Eigen::Index colIndex=0;colIndex<=d;++colIndex) {//copy
			...
			dictio(mHsv.size(),colIndex) = dictionary.get(n0,colIndex); // fresh last row contains column indices/var indices.
		}
	...
	bool VertexEnumeration<Number>::findPositiveConstrains() {
		try{
			...
			for(Eigen::Index rowIndex=0; rowIndex<constraintsCount; ++rowIndex) {
				if(dictionary.basis()[rowIndex]-1<constraintsCount) { // pick indices, which correspond to an original constraint in mHsv and skip original variables.
					for(std::size_t colIndex=0; colIndex<dimension; ++colIndex) {
			...
			matrix_t<Number> newDictionary = matrix_t<Number>::Zero(constraintsCount-dimension+1, dimension+1);// create new dictionary
			skipped = 0;
			for(Eigen::Index rowIndex=0; rowIndex<constraintsCount; ++rowIndex) {
				if(dictionary.basis()[rowIndex]-1<constraintsCount) { // again only pick those indices, which correspond to original constraints.
					for(std::size_t colIndex=0; colIndex<dimension; ++colIndex) {
