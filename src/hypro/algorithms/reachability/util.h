                                           ...
                                           const matrix_t<Number>& trafoMatrix, const Box<Number>& externalInput)
{
    ...
    auto b1 = boost::get<Box<Number>>( boost::apply_visitor( genericConversionVisitor<typename State::repVariant,Number,Box<Number>>(representation_name::box), transformedInitialSet.getSet(0)));
    if(b1.empty()) {
    	return std::vector<Box<Number>>{};
    }
    ...
    auto b2 = boost::get<Box<Number>>( boost::apply_visitor( genericConversionVisitor<typename State::repVariant,Number,Box<Number>>(representation_name::box), tmp.getSet(0)));
