#pragma once
#include "../IHandler.h"
#include "../../../../datastructures/reachability/SettingsProvider.h"

namespace hypro
{
    template<typename State>
	class rectangularGuardHandler : public IGuardHandler<State> {
		using Number = typename State::NumberType;
	protected:
		CONTAINMENT mContainment = CONTAINMENT::NO;
		bool mMarkedForDelete =false;
		std::shared_ptr<State> mState;
		Transition<Number>* mTransition;

		size_t mIndex;
		double mSortIndex;
	public:
		rectangularGuardHandler() = delete;
		rectangularGuardHandler(std::shared_ptr<State> state, size_t index, Transition<Number>* transition)
			: mState(state)
			, mTransition(transition)
			, mIndex(index)
		{
			if(SettingsProvider<State>::getInstance().decomposeAutomaton()){
				mSortIndex = SettingsProvider<State>::getInstance().getSubspaceDecomposition().at(mIndex).size();
			}else{
				mSortIndex = 0;
			}
		}

		void handle();
		const char* handlerName() {return "rectangularGuardHandler";}
		// return containment status
		virtual bool satisfiesGuard() override {return mContainment != CONTAINMENT::NO;}
		virtual std::shared_ptr<State> getState() override{return mState;}
		virtual void setState(std::shared_ptr<State> ptr) override{mState = ptr;}
		void reinitialize();
		virtual double getSortIndex() override {return mSortIndex;}
		virtual void setSortIndex(double sortIndex) override { mSortIndex = sortIndex;}

		virtual bool getMarkedForDelete() override {return mMarkedForDelete;}
		virtual void setMarkedForDelete(bool toDelete) override { mMarkedForDelete = toDelete;}
	};
} // hypro

#include "rectangularGuardHandler.tpp"
