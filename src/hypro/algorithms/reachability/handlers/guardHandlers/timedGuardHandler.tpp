...
namespace hypro {
	...
	void timedGuardHandler<Representation,Number>::handle() {
	    TRACE("hydra.worker.discrete","Check guard for transition " << this->mTransition->getSource()->getName() << "(" << this->mTransition->getSource()->hash() << ")"  << " -> " << this->mTransition->getTarget()->getName() << "(" << this->mTransition->getTarget()->hash() << ")");
