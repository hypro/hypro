...
namespace hypro {
    ...
    {
        /*
        if(SettingsProvider<State>::getInstance().useDecider() && SettingsProvider<State>::getInstance().getLocationTypeMap().find(t->treeNode->getStateAtLevel(t->btInfo.btLevel).getLocation())->second == hydra::LOCATIONTYPE::TIMEDLOC){
