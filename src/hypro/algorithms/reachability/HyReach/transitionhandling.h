/*
* This file is included by HyReach.h and contains the implementation of functionality and data related to the
* determination process of the transitions possible to take
...
std::vector<possibleTransition*>* getPossibleTransitions( location* loc, FlowpipeSegment* flowpipe ) {
	...
	bool transitionadded = false;  // stores wether the actually considered transition has already a detected
								   // intersection with the flowpipe
	...
	for ( auto iterator = temp.begin(); iterator != temp.end(); ++iterator )  // iterate over outgoing transitions
	{
		...
		for ( unsigned int i = 0; i < flowpipe->size(); i++ )  // iterate over sets in flowpipe
		{
			...
			if ( testIntersection( t->g_star_values, full, flowpipe->getSet( i ), L.size(),
								   true ) )  // test for intersection
			{
// set intersects a guard
				...
				if ( transitionadded ) {
					...
					if ( indices->end + 1 == i ) {
						...
						result->at( result->size() - 1 )
							  ->sets.push_back( *indices );  // list copies values when adding the struct
		...
		if ( transitionadded )  // there is a set of the flowpipe which intersects the considered transition and has not
								// yet been added
		{
