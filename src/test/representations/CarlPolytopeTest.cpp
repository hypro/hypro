...
TYPED_TEST(CarlPolytopeTest, Substitution)
{
    ...
    c1.substituteVariable(hypro::VariablePool::getInstance().carlVarByIndex(0), hypro::VariablePool::getInstance().carlVarByIndex(2));
...
TYPED_TEST(CarlPolytopeTest, TimeElapse)
{
    ...
    // let time pass: substitute original variables by ^pre-versions, add constraints for time being positive, the flow and the invariants.
