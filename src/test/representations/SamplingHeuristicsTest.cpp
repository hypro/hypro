...
TYPED_TEST(SamplingHeuristicsTest, Set)
{
    ...
    std::vector< hypro::SimulationSample<TypeParam> > vecCenter = sample<hypro::Box<TypeParam>, TypeParam, CENTER>(boost::get<hypro::Box<TypeParam>>(stateSet.getSet()));
	EXPECT_EQ( std::vector< hypro::SimulationSample<TypeParam> > ({SimulationSample<TypeParam>({3,2})}), vecCenter);

    std::vector< hypro::SimulationSample<TypeParam> > vecVert = sample<hypro::Box<TypeParam>, TypeParam, VERTICES>(boost::get<hypro::Box<TypeParam>>(stateSet.getSet()));
	EXPECT_EQ( std::vector< hypro::SimulationSample<TypeParam> > ({SimulationSample<TypeParam>({2,1}), SimulationSample<TypeParam>({4,1}), SimulationSample<TypeParam>({2,3}), SimulationSample<TypeParam>({4,3})}), vecVert);

    // std::vector< hypro::SimulationSample<TypeParam> > vecRand = sample<hypro::Box<TypeParam>, TypeParam, RANDOM_SET_K>(boost::get<hypro::Box<TypeParam>>(stateSet.getSet()),10);
	// EXPECT_EQ( 10, vecRand.size());
}
