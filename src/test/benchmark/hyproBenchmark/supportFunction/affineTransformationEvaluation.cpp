...
namespace sf {
    ...
    Results<std::size_t> affineTransformationEvaluation(const Settings& settings) {
        ...
        for(std::size_t d = 1; d <= settings.maxDimension; d += settings.stepSize) {
            ...
            workingQueue.emplace(std::async(std::launch::async, affineTrafoHelper<hypro::SupportFunctionSetting>, mVector, vVector, dVector, d, "affineTransformationEval", settings, sFunct));
            workingQueue.emplace(std::async(std::launch::async, affineTrafoHelper<hypro::NoTrafoReduction>, mVector, vVector, dVector, d, "affineTransformationEvalNoReduction", settings, sFunctNoReduction));
        }
