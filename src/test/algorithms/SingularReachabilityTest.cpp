...
hypro::HybridAutomaton<Number> createSingularHA3() {
    ...
    Matrix resetMat = Matrix::Zero( 2, 2 );
    Vector resetVec = Vector::Zero( 2 );
    hypro::Reset<Number> reset( resetMat, resetVec );

    std::unique_ptr<hypro::Transition<Number>> trans0 = std::make_unique<hypro::Transition<Number>>( loc0.get(), loc0.get(), guard, reset );
    ...
    resetMat = Matrix::Zero( 2, 2 );
    resetMat( 0, 0 ) = 1;
    resetMat( 1, 1 ) = 1;
    reset = hypro::Reset<Number>( resetMat, resetVec );

    std::unique_ptr<hypro::Transition<Number>> trans1 = std::make_unique<hypro::Transition<Number>>( loc0.get(), loc1.get(), guard, reset );
...
TEST( SingularRechabilityTest, SingularAnalyzer ) {
    ...
    automaton.setGlobalBadStates( { hypro::Condition<Number>( badStateMat, badStateVec ) } );
    analyzer = hypro::SingularAnalyzer<VPoly>( automaton, hypro::Settings{ {}, { 0, hypro::tNumber( 0.2 ), hypro::tNumber( 0.01 ) }, {} } );
    result = analyzer.run();
    EXPECT_EQ( hypro::REACHABILITY_RESULT::SAFE, result );
    ...
    analyzer = hypro::SingularAnalyzer<VPoly>( automaton, hypro::Settings{ {}, { 5, hypro::tNumber( 10 ), hypro::tNumber( 0.01 ) }, {} } );
    result = analyzer.run();
    EXPECT_EQ( hypro::REACHABILITY_RESULT::SAFE, result );
