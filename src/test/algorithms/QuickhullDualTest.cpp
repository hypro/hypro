...
template<bool Euclidian = true>
using qh = hypro::Quickhull<number_t, Euclidian>;

// bool checkContainment(qh::pointVector_t& inputs, qh& quickhull);
template<bool Euclidian>
bool nonFancyCheckContainment(qh<Euclidian>::pointVector_t& inputs, qh<Euclidian>& quickhull);
...
TEST_F(QuickhullDualTest, RandomVertices) {
    ...
    qh<false>::pointVector_t dualInputs;

    for(qh::Facet facet : quickhull.getFacets()) {
        dualInputs.emplace_back(D + 1);

        dualInputs.back().head(D) = facet.mNormal;
        dualInputs.back()[D] = facet.mOffset;
    }

    qh<false> dualQuickhull(dualInputs, D);
    dualQuickhull.compute();

    EXPECT_TRUE(nonFancyCheckContainment(dualInputs, dualQuickhull));

    qh<false>::pointVector_t reconstructedPoints;

    if constexpr(is_exact<number_t>) {
        for(qh<false>::Facet facet : dualQuickhull.getFacets()) {
            reconstructedPoints.emplace_back(D);
            //The resulting points can't be points at infinity
            EXPECT_FALSE(facet.mScalar == 0);
            facet.mNormal /= facet.mScalar;
            reconstructedPoints.back() = facet.mNormal;
        }

        //Each of the original points should exist in the reconstructed points aswell.
        for(qh::point_t point : inputs) {
            EXPECT_TRUE(std::find(reconstructedPoints.begin(), reconstructedPoints.end(), point) != reconstructedPoints.end());
        }
    }
}
