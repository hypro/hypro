...
{
	VertexContainer<number> test1;
	SUCCEED();
}

TEST_F(VertexContainerTest, Insertion)
{
	VertexContainer<number> test1 = VertexContainer<number>();
	std::cout << (test1.insert(Vertex<number>(p1, true))).second << std::endl;
        std::cout << test1.size() << std::endl; 
        std::cout << (test1.insert(Vertex<number>(p2, true))).second << std::endl;
        std::cout << test1.size() << std::endl; 
	std::cout << (test1.insert(Vertex<number>(p3, false))).second << std::endl;
        std::cout << test1.size() << std::endl; 
        std::cout << test1 << std::endl;
        
        std::cout << "ITEM: " << *test1.find(p3) << std::endl;
        
        std::cout << "Manual comparison: p1 < p3: " << ( Vertex<number>(p1, true) < Vertex<number>(p3, false) ) << ", p1 > p3: " << ( Vertex<number>(p3, false) < Vertex<number>(p1, true) ) << std::endl;
        std::cout << "Manual comparison: p1 == p3: " << ( Vertex<number>(p1, true) == Vertex<number>(p3, false) ) << std::endl;
...
{
	VertexContainer<number> test1 = VertexContainer<number>();
	test1.insert(Vertex<number>(p1, true));
	test1.insert(Vertex<number>(p2, true));
	test1.insert(Vertex<number>(p3, false));
	EXPECT_EQ(test1.size(), (unsigned) 3);
	test1.erase(Vertex<number>(p2, true));
...
TEST_F(VertexContainerTest, Assignment)
{
	...
	test1.insert(Vertex<number>(p2, true));
	test1.insert(Vertex<number>(p3, false));
	VertexContainer<number> test2 = test1;
...
TEST_F(VertexContainerTest, Access)
{
	...
	EXPECT_NE(test1.end(), test1.find(Vertex<number>(p1, true)));
	EXPECT_NE(test1.end(), test1.find(Vertex<number>(p2, true)));
	EXPECT_NE(test1.end(), test1.find(Vertex<number>(p3, false)));

	EXPECT_EQ(test1.end(), test1.find(Vertex<number>(p1, false)));
	EXPECT_EQ(test1.end(), test1.find(Vertex<number>(p2, false)));
	EXPECT_EQ(test1.end(), test1.find(Vertex<number>(p3, true)));

	EXPECT_TRUE(test1.find(p1, true)->color());
	EXPECT_TRUE(test1.find(p2, true)->color());
	EXPECT_FALSE(test1.find(p3, false)->color());

	EXPECT_TRUE(test1.find(Vertex<number>(p1, true))->color());
	EXPECT_TRUE(test1.find(Vertex<number>(p2, true))->color());
	EXPECT_FALSE(test1.find(Vertex<number>(p3, false))->color());
}
...
TEST_F(VertexContainerTest, OriginIsVertex)
{
    ...
    test1.erase(p1);
    test1.insert(Vertex<number>(p1, false));
