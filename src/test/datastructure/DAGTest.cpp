...
TEST(DAGNodeTest, Construction)
{
    ...
    EXPECT_TRUE(std::find(root->getChildren().begin(), root->getChildren().end(), child) != root->getChildren().end());
    EXPECT_TRUE(std::find(root->getChildren().begin(), root->getChildren().end(), child2) != root->getChildren().end());
}
