...
//#define FILE_OUTPUT  //defining FILE_OUTPUT will lead to the creation of text files with the generated flowpipe
//segments in matlab syntax
...
struct options {
	...
	//	       double fixpoint_tolerance;	// in case fixpoint detection is enabled, two values are considered equal
...
#include "CleanupOnExit.h"
	...
	/*
	* This method computes the results for a single direction and its opposite and stores the evaluation results
...
#endif
		while ( results->sets( indexL, j ) >= I_minus && j < params.setsToCompute &&
				results->sets( indexL_opposite, j ) >= I_plus ) {
			j++;
			...
			if ( ( p_omega_minus > I_plus && results->sets( indexL, j ) < I_minus ) ||
				 ( p_omega > I_minus && results->sets( indexL_opposite, j ) < I_plus ) ) {
#ifdef PARALLHELPER_VERBOSE
	...
	*/
	FlowpipeSegment* constructFlowpipe( location* loc, LocationInfo* locInfo, SupportFunctionContent* X0, SupportFunctionContent* U,
		...
		if ( testIntersection( locInfo->scaledConstraintValues, locInfo->mirrored_invariant_constraints_in_L, X0_values,
							   locInfo->scaledConstraintValues->size(), false ) ) {
#ifdef FLOWPIPE_VERBOSE
			...
			omega0_values = intersectWithInvariant( omega0_values, locInfo->invariant_constraints_in_L,
													locInfo->scaledConstraintValues );
...
#endif
					unsigned int j =
						  parallelizationHelper( flowpipe, locInfo, &set0, omega0.getEpsilonpsi(), &L.at( i ), i,
												 &L.at( oppositeOfDirectionI ), oppositeOfDirectionI );
					maxNumberOfCompleteSets = MIN( maxNumberOfCompleteSets, j );
#ifdef FLOWPIPE_VERBOSE
					std::cout << method << " maxNumberOfCompleteSets = MIN(" << maxNumberOfCompleteSets << ", " << j
							  << ") " << BL;
#endif
	...
	void analyze( location* loc, unsigned int recursionNumber, SupportFunctionContent* X0, SupportFunctionContent* U,
		...
		if ( recursionNumber > 0 )  // compute flowpipe only if there is a further recursion to performe
		{
			...
			FlowpipeSegment* flowpipe =
				  constructFlowpipe( loc, ( locationMap.find( loc )->second ), X0, U, timeStep );  // algoInv
#ifdef HYREACH_VERBOSE
			...
			// detect possible transitions -> flowpipe might NOT be shortened dependend on the chosen transition -> due
			// to use of algoinv only reachable (in the invariant) sets are computed!
			...
			for ( auto iterator = possibleTransitions->begin(); iterator != possibleTransitions->end(); ++iterator ) {
				...
				valuesForNextSet = clustering( **iterator );  // cluster all sets which are relevant for the
															  // construction of the initial set for the next node
#ifdef HYREACH_VERBOSE
				...
				// Setting the evaluation of W() along those directions to 0 yields in a worst case scenario (everything
				// is zero in a PolytopeSupportFunction)
				...
				analyze( ( *iterator )->transition_pt->transition().locTarget, recursionNumber - 1, &nextX0, U,
						 ( timeStep + 1 + ( *iterator )->sets.begin()->start ) ); /*flowpipe->size()*/
	...
	void start( HybridAutomaton<double, valuation_t<double>>* model, parameters params, options opt,
		...
		if ( U == 0 )  // use zero function if none provided
		{
			U = new ZeroSupportFunction( dimensionality, 0 );  // 2nd parameter 0 will disable special handling for
															   // artificial directions during evaluation (since U does
															   // not know any additional dimension - will be introduced
															   // using B)
#ifdef HYREACH_VERBOSE
		...
		oppositeDirectionMapping = computeDirectionMapping(
			  &L );  // can only be done after preprocessing because preprocessing may introduce additional directions
#ifdef HYREACH_VERBOSE
...
// Create X0 SupportFunctionContent (extended by additional dimension)
	...
	*/
	std::list<FlowpipeSegment*> getResults() { return results; }
