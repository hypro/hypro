...
/**
 * checks if the given valuation fulfills the guard of the given transition, if yes, executes the reset (that belongs to
 * the transition)
 */
template <typename Number, typename Representation>
static bool computePostCondition( const hypro::Transition<Number>& _trans, Representation _val,
...
static std::vector<flowpipe_t<Representation>> computeReach(
	...
	for ( typename std::vector<flowpipe_t<Representation>>::iterator it_pipe = _init.begin(); it_pipe != _init.end();
		  ++it_pipe ) {
		...
		for ( typename std::set<Transition<Number>*>::iterator it_trans = loc_transSet.begin();
			...
			for ( typename flowpipe_t<Representation>::iterator it_val = it_pipe->begin(); it_val != it_pipe->end();
				  ++it_val ) {
