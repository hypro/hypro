...
class PreprocessingObject {
  private:
	...
	*/
	unsigned int addToL( std::vector<matrix_t<double>>* L_pt, matrix_t<double>* direction ) {
		if ( L_pt->at( 0 ).rows() >
			 direction->size() )  // the entries of L are of higher dimensionality -> fill direction with 0
		{
			int dirRows = direction->rows();
			direction->resize( L_pt->at( 0 ).rows(), 1 );
			direction->block( dirRows, 0, L_pt->at( 0 ).rows() - dirRows, 1 ) =
				  matrix_t<double>::Zero( 1, 1 );  // fill with 0
		}
  ...
  protected:
	/*
	* Normalizes the length of constraints to one (row-wise) and scales the constraintValues correspondingly.
	* Furthermore,
	* the constraints are rounded regarding global settings.
	*/
	void normalizeLinearConstraint( matrix_t<double> constraints, matrix_t<double> constraintValues,
									std::vector<matrix_t<double>>* directions,
									std::vector<double>* scaledConstraintValues ) {
	...
	*/
	unsigned int* extendMirroredDirections( std::vector<matrix_t<double>>* L_pt,
											std::vector<matrix_t<double>>* directions ) {
		// create value to return
		unsigned int* result = new unsigned int[directions->size()];
	...
	*/
	void removeZeroRows( matrix_t<double>* constraints, matrix_t<double>* values ) {
		// std::cout << "original constraints: " << BL << *constraints << BL << "values: " << (*values).transpose() <<
		// BL;
  ...
  public:
	virtual void
	freeAllocatedMemory() = 0;  // has to be overwritten and will be called to destroy previously created heap objects
};
...
class LocationInfo : public PreprocessingObject {
  private:
	location* origLocation;  // stores the original location from the model where the missing informations from
							 // LocationInfo are stored
  ...
  public:
	...
	std::vector<double>* scaledConstraintValues;  // since during the analysis the correspondent directions from L are
												  // used it is necessary to use scaled constraint values
	...
	*/
	LocationInfo( location* loc, std::vector<matrix_t<double>>* L_pt, double delta, SupportFunction* U,
		...
		unsigned int constraints =
			  (unsigned int)( ( *loc ).invariant() ).mat.rows();  // Assuption: the model contains only <= constraints
...
//}
...
#endif
			// note: no need to use normed directions and correspondent scaled values because the invariant support
			// function will always
			// be evaluated to retrieve values from it an thus the values are computed correctly
			invariantSP = new PolytopeSupportFunction( m, ( *loc ).invariant().vec, ( *loc ).invariant().op,
													   (unsigned int)m.cols(), aD );
		} else {
#ifdef LOCATIONINFO_VERBOSE
		...
		normalizeLinearConstraint( ( *loc ).invariant().mat, ( *loc ).invariant().vec, constraintsAsDirections,
								   scaledConstraintValues );
...
class TransitionInfo : public PreprocessingObject {
  private:
	transition* origTransition;  // stores the original transition from the model where the missing informations from
								 // TransitionInfo are stored
  ...
  public:
	...
	// The values are only used to compute the intersection with the guards and during that processing step, the
	// reconstruction and evaluation will be performed
	...
	*/
	...
	TransitionInfo( transition* trans, std::vector<matrix_t<double>>* L_pt, artificialDirections* aD ) {
		...
		unsigned int guards =
			  (unsigned int)( ( *trans ).guard() ).mat.rows();  // Assuption: the model contains only <= guards
		...
		for ( int i = 0; i <= trans->assignment().translationVec.size(); i++ ) {
			// The values for the additional dimension for w are set to 1, because 0 for all entries (would be correct)
			// leads to errors during the evaluation of the support function
			// in order to provide correct results after a reset, the evaluation along correspondent directions for the
			// reset-set is explicitely set to 1 at time of computation
			w_local( i, 0 ) =
				  i < trans->assignment().translationVec.size() ? trans->assignment().translationVec( i ) : 1;
			w_local( i + dim, 0 ) =
				  i < trans->assignment().translationVec.size() ? -trans->assignment().translationVec( i ) : -1;
		}
		this->w = w_local;
		...
		normalizeLinearConstraint( ( ( *trans ).guard() ).mat, ( ( *trans ).guard() ).vec, guardsAsDirections,
								   scaledGuardValues );
#ifdef TRANSITIONINFO_VERBOSE
		...
		matrix_t<double> I_star_constraints =
			  trans->targetLoc()->invariant().mat;  // initialize I* (values are not important, size is)
		matrix_t<double> I_star_values( I_star_constraints.rows(), 1 );

		matrix_t<double> temp;
		for ( unsigned int i = 0; i < targetInv; i++ ) {
			I_star_constraints.block( i, 0, 1, I_star_constraints.cols() ) =
				  trans->targetLoc()->invariant().mat.row( i ) * trans->assignment().transformMat;
			temp = -trans->targetLoc()->invariant().mat.row( i ).transpose();
			I_star_values( i, 0 ) =
				  trans->targetLoc()->invariant().vec( i ) + wfunction->evaluate( &temp ).supportValue;
		}
#ifdef TRANSITIONINFO_VERBOSE
...
#ifdef LOCATIONINFO_VERBOSE
		std::cout << "preprocessing_recursion(...): considered location " << loc << " has already been preprocessed"
				  << BL;
#endif
...
#endif
			preprocessing_recursion( ( *iterator )->targetLoc(), L_pt, delta, U,
									 additionalDirections );  // preprocess connected location
		}
	}
}
...
//                std::cout << "transitionMap.size(): " << transitionMap.size() << BL;
	...
	for ( auto iterator = transitionMap.begin(); iterator != transitionMap.end(); ++iterator ) {
		...
		for ( unsigned int i = 0; i < iterator->second->guardsAsDirections->size(); i++ ) {
			temp( iterator->second->mirrored_guard_constraints_in_L[i], 0 ) =
				  MIN( temp( iterator->second->mirrored_guard_constraints_in_L[i], 0 ),
					   iterator->second->scaledGuardValues->at( i ) );
		}

		// intersect with istar
		for ( unsigned int i = 0; i < iterator->second->IstarAsDirections->size(); i++ ) {
			temp( iterator->second->mirrored_I_star_constraints_in_L[i], 0 ) =
				  MIN( temp( iterator->second->mirrored_I_star_constraints_in_L[i], 0 ),
					   iterator->second->scaledIstarValues->at( i ) );
		}
#ifdef TRANSITIONINFO_VERBOSE
		...
		// intersect with guards
		for ( unsigned int i = 0; i < iterator->second->guardsAsDirections->size(); i++ ) {
			temp( iterator->second->guard_constraints_in_L[i], 0 ) =
				  MIN( temp( iterator->second->guard_constraints_in_L[i], 0 ),
					   iterator->second->scaledGuardValues->at( i ) );
		}
		iterator->second->sortedValues = temp;
#ifdef TRANSITIONINFO_VERBOSE
		std::cout << "preprocess(...): "
				  << "sorted Values: " << ( iterator->second->sortedValues ) << BL;
		std::cout << '\n';
#endif
	}
