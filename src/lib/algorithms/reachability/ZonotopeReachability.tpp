...
ZonotopeReachability<Number>::ZonotopeReachability( unsigned int dimension )
	: mDimension( dimension ), mInitialized(), mReadjusted() {
}
					...
					switch ( option.testCase ) {
						...
						case 1:  // using Girard's method used for initialization of zonotope
						{
							...
							for ( unsigned i = 0; i < intersect_zonotopes_[offset].size(); i++ ) {
								...
								overApproximatedIntersectZonotope.minkowskiSum( overApproximatedIntersectZonotope,
																				temp_V );
							}
							this->checkForIntersection( overApproximatedIntersectZonotope, hp, finalIntersect,
														option.intersectMethod );
							break;
						}
...
void ZonotopeReachability<Number>::constructIntersectZonotopeFromMinMax( unsigned iteration, const Zonotope<Number> &Q,
	...
	if ( mDimension != 2 ) {
		...
		for ( unsigned i = 0; i < min_intersect_[iteration].rows(); i++ ) {
			finalGenerators.col( i ) << ( ( globalMinMaxMatrix( 1, i ) - globalMinMaxMatrix( 0, i ) ) / 2 ) *
											  kernel.col( i );
		}

		result.setCenter( kernel * ( ( globalMinMaxMatrix.row( 1 ) + globalMinMaxMatrix.row( 0 ) ) / 2 ).transpose() +
						  hp.scalar() * hp.vector() );
...
bool ZonotopeReachability<Number>::fulfillsInvariant( const Zonotope<Number> &inputZonotope,
	...
	if ( inv.mat.rows() == 0 || inv.vec.rows() == 0 )
		return true;
	else {
...
void ZonotopeReachability<Number>::loadNewState( hypro::Transition<Number> &transition,
	...
	hypro::matrix_t<Number> new_transformedMat, transformMat = transition.assignment().transformMat;
...

{
	...
	if ( mReadjusted ) {
		...
		tempZonotope.changeDimension( tempZonotope.dimension() - 1 );
		if ( result.dimension() > 1 ) result.changeDimension( result.dimension() - 1 );
	}
