...
class FlowpipeSegment {
  private:
	...
	transition* previousTransition =
		  0;  // Transition that has been taken in order to enter the location which generated this flowpipeSegment
  ...
  public:
	...
	matrix_t<double> sets;  // Every column describes a set of the flowpipe
							...
							*/
	FlowpipeSegment( location* generatingLocation, unsigned int startTimestep, unsigned int numberOfDirections,
	...
	FlowpipeSegment( location* generatingLocation, transition* generatingTransition, unsigned int startTimestep,
					 unsigned int numberOfDirections, unsigned int timeHorizon )
		: FlowpipeSegment( generatingLocation, startTimestep, numberOfDirections, timeHorizon ) {
  ...
  */ if ( position < (unsigned int)sets.rows() ) {
			/*                       #ifdef FLOWPIPE_VERBOSE
								   ...
								   #ifdef FLOWPIPE_VERBOSE
									   std::cout << "sets: rows() = " << sets.rows() << BL << "sets: cols() = " <<
			   sets.cols() << BL;
								   #endif
			*/
  ...
  */           }
	...
	*/
	location* getLocation() { return generatingLocation; }
	...
	unsigned int getLastTimestep() { return startTimestep + ( (unsigned int)sets.size() ); }
};
}
