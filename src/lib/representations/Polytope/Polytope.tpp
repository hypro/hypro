...
namespace hypro
{
	...
	Polytope<Number> Polytope<Number>::intersectHyperplane(const Hyperplane<Number>& rhs) const {
		if(rhs.dimension() == 0){
			...
			for(unsigned rowIndex = 0; rowIndex < rhs.dimension(); ++rowIndex) {
				ls.set_coefficient(hypro::VariablePool::getInstance().pplVarByIndex(rowIndex), double(rhs.normal()(rowIndex)*fReach_DENOMINATOR));
			}
	...
	template<typename Number>
	Polytope<Number> Polytope<Number>::intersectHyperplanes(const matrix_t<Number>& _mat, const vector_t<Number>& _vec) const {
