...
namespace hpolytope
{
     ...
     */
    template<typename Number>
    static bool selectPivot(const Eigen::Matrix<Number, Eigen::Dynamic, Eigen::Dynamic>& A, std::vector<unsigned> B, std::vector<unsigned> N, unsigned& i, unsigned& j, unsigned f, unsigned g)
    {
        // step 1
        unsigned r = 0;
        unsigned s = 0;
        ...
        for(; s < N.size(); ++s)
        {
            ...
            if(A(f,N.at(s)) > 0)
            {
                for( ; r < B.size(); ++r)
                {
                    if(B.at(r) != f && A(B.at(r),N.at(s))<0 && -(A(B.at(r),g)/A(B.at(r),N.at(s))) < min )
                    {
                        secondSet = true;
                        break;
                    }
