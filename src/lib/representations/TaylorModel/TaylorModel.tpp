...
template<typename Number>
TaylorModel<Number>::TaylorModel(const std::initializer_list<Term<Interval<Number>> > & terms, const Interval<Number> & r)
{
	MultivariatePolynomial<Interval<Number>> p(terms);
	expansion = p;
...
template<typename Number>
TaylorModel<Number>::TaylorModel(std::shared_ptr<const Term<Interval<Number>> > t)
{
...
template<typename Coeff, typename Number>
TaylorModel<Coeff, Number>::TaylorModel(const std::initializer_list<Variable> & terms)
{
...
TaylorModel<Number> TaylorModel<Number>::pow(unsigned exp, Domain<Number> & domain, const exponent order) const
{
	...
	if(exp == 0)
		return TaylorModel(intOne);

	if(isZero())
		return TaylorModel(intZero);
...
void TaylorModel<Number>::normalize(Domain<Number> & domain)
{
	...
	for(auto iter=assignments.begin(); iter!=assignments.end(); ++iter)
	{
		...
		TaylorModel<Number> tmReplace({(Term<Interval<Number>>)intCenter, (Interval<Number>)intMagnitude*(iter->first)}, intZero);
...
template<typename Number>
TaylorModel<Number> TaylorModel<Number>::multiply(const TaylorModel<Number> & tm, Domain<Number> & domain, const exponent order) const
{
	TaylorModel<Number> result;
...
template<typename Number>
TaylorModel<Number> TaylorModel<Number>::multiply(const Term<Interval<Number>> & term, Domain<Number> & domain, const exponent order) const
{
...
template<typename Number>
TaylorModel<Number> TaylorModel<Number>::multiply(Variable::Arg v, Domain<Number> & domain, const exponent order) const
{
...
template<typename Number>
void TaylorModel<Number>::multiply_assign(const TaylorModel<Number> & tm, Domain<Number> & domain, const exponent order)
{
	TaylorModel<Number> result;
...
template<typename Number>
void TaylorModel<Number>::multiply_assign(const Term<Interval<Number>> & term, Domain<Number> & domain, const exponent order)
{
	TaylorModel<Number> result, tm(term);
...
template<typename Number>
TaylorModel<Number> TaylorModel<Number>::substitute(const std::map<Variable, TaylorModel<Number>> & substitutions, Domain<Number> & domain, const exponent order) const
{
	TaylorModel<Number> result(*this);
    ...
    if(!expResults.empty())
    {
    	...
    	while(expResultB != expResults.end())
    	{
    		...
    		{
    			TaylorModel<Number> tmTemp = sub->second.pow(expResultB->first.second-expResultA->first.second, domain, order);
    	...
    	if(term.monomial())
    	{
    		...
    		for(unsigned i = 0; i < m.nrVariables(); ++i)
    		{
    			...
    			else
    			{
    				...
    				{
    					termResult.multiply_assign(Term<Interval<Number>>(intOne, m[i].first, m[i].second), domain, order);
    				}
    			}
    		}
    	}
...
template<typename Number>
TaylorModel<Number> TaylorModel<Number>::substitute(const TaylorModelVec<Number> & substitutions, Domain<Number> & domain, const exponent order) const
{
...
template<typename Number>
TaylorModelVec<Number> TaylorModelVec<Number>::substitute(const std::map<Variable, TaylorModel<Number>> & substitutions, Domain<Number> & domain, const exponent order) const
{
...
template<typename Number>
TaylorModelVec<Number> TaylorModelVec<Number>::substitute(const TaylorModelVec<Number> & substitutions, Domain<Number> & domain, const exponent order) const
{
	TaylorModelVec<Number> result;
...
template<typename Number>
TaylorModelVec<Number> TaylorModelVec<Number>::Picard(const TaylorModelVec<Number> & x0, const PolynomialODE<Number> & ode, Domain<Number> & domain, Variable::Arg t, const Interval<Number> & range_of_t, const exponent order) const
{
...
template<typename Number>
void TaylorModelVec<Number>::Picard_assign(const TaylorModelVec<Number> & x0, const PolynomialODE<Number> & ode, Domain<Number> & domain, Variable::Arg t, const Interval<Number> & range_of_t, const exponent order)
{
	TaylorModelVec<Number> tmvTemp;
