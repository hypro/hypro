...
class TaylorModel {
  ...
  public:
	...
	TaylorModel<Number> multiply( const Term<Interval<Number>>& term, Domain<Number>& domain,
	...
	TaylorModel<Number> substitute( const std::map<Variable, TaylorModel<Number>>& substitutions,
...
class TaylorModelVec {
  ...
  public:
	...
	TaylorModelVec<Number> substitute( const std::map<Variable, TaylorModel<Number>>& substitutions,
									   Domain<Number>& domain, const exponent order ) const;
	...
	void Picard_assign( const TaylorModelVec<Number>& x0, const PolynomialODE<Number>& ode, Domain<Number>& domain,
						Variable::Arg t, const Interval<Number>& range_of_t, const exponent order );
