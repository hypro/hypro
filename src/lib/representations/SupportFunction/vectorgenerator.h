...
namespace hypro {
	...
	*/
	...
	matrix_t<Number> getRotationMatrix(unsigned int dimensions, unsigned int dim1, unsigned int dim2, Number angle)
	{
		matrix_t<Number> rotationMatrix(dimensions,dimensions);	//matrix defining the rotation to performe
	...
	*/
	...
	static matrix_t<Number> getZeroVector(unsigned int dimensions)
	{
		...
		matrix_t<Number> result(dimensions,1);
	...
	*/
	template<typename Number>
	std::vector<matrix_t<Number>> vectorgenerator( unsigned int dimensions)
	{
		std::string method = "Vectorgenerator: ";
		std::vector<matrix_t<Number>> result;
		...
		for( unsigned int i=0; i<dimensions; i++)
		{
			...
			matrix_t<Number> vector1(dimensions,1);
			vector1.setZero();
			vector1(i) = Number(1);

			matrix_t<Number> vector2(dimensions,1);
			vector2.setZero();
			vector2(i) = Number(-1);
	...
	*/
	template<typename Number>
	matrix_t<Number> round(matrix_t<Number> direction, double precision)
	{
        matrix_t<Number> result(direction.rows(),1);             
	...
	*/ 
	template<typename Number>
	std::vector<matrix_t<Number>> vectorgeneratorEXT( unsigned int dimensions, Number angle, double precision)
	{
		std::string method = "vectorgeneratorExt: ";

		std::vector<matrix_t<Number>> result;	//the result of the computation
		matrix_t<Number> last_considered;
		...
		for(iteration = 0; iteration<dimensions; iteration++)
		{
			for(iteration2 = iteration+1; iteration2 < dimensions; iteration2++)
			{
				...
				matrix_t<Number> start = getZeroVector<Number>(dimensions);
				start(iteration,0) = 1;
				...
				matrix_t<Number> rotationMatrix = getRotationMatrix(dimensions, iteration, iteration2, angle);
				...
				matrix_t<Number> nextDirection = start;
	...
	struct artificialDirections {
        matrix_t<Number> dir1; // positive direction (0 ....0  1)
        matrix_t<Number> dir2; // negative direction (0 ....0 -1)
        
        // pointer to the correspondent entries in the global directions list
        matrix_t<Number>* dir1_pt;
        matrix_t<Number>* dir2_pt;
        
        // constant evaluation results
        const Number dir1_eval = Number(1);
        const Number dir2_eval = Number(-1);
    };
	...
	*/
	template<typename Number>
	artificialDirections<Number> extendDimensions(std::vector<matrix_t<Number>>* directions, std::vector<matrix_t<Number>>** extendedDirections)
	{
         std::vector<matrix_t<Number>>* newDirections = new std::vector<matrix_t<Number>>(directions->size()+2);
         *extendedDirections = newDirections;
                      
         matrix_t<Number> l;
         
         matrix_t<Number> newl(directions->at(0).size() + 1,1);
         newl(l.rows(),0) = Number(0);        // add final 0 entry
         ...
         artificialDirections<Number> result;
