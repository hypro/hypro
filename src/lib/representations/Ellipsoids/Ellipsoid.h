...
class Ellipsoid {
  private:
	...
	matrix_t<Number> mShapeMatrix;
        bool mIsEmpty;
  public:
	Ellipsoid( Number _radius, std::size_t _dimension );
	Ellipsoid( matrix_t<Number> _shapeMatrix );
	Ellipsoid( const Ellipsoid<Number>& _orig );
	~Ellipsoid();
	 ...
	 *
	 * @param rhs An Ellipsoid.
	 */

	Ellipsoid<Number>& operator=( const Ellipsoid<Number>& rhs ) = default;

	friend bool operator==( const Ellipsoid<Number>& b1, const Ellipsoid<Number>& b2 ) { return (b1.mShapeMatrix == b2.mShapeMatrix ? true : false); }
	friend bool operator!=( const Ellipsoid<Number>& b1, const Ellipsoid<Number>& b2 ) { return !( b1 == b2 ); }

	friend std::ostream& operator<<( std::ostream& ostr, const Ellipsoid<Number>& b ) { return ostr; }
	void print() const;
