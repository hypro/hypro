...
template <typename Number>
std::vector<hypro::vector_t<Number>> getCornersRecursive( hypro::matrix_t<Number> _remainingGenerators,
														  hypro::vector_t<Number>& _current ) {
	...
	if ( _remainingGenerators.cols() == 1 ) {
		...
		res = getCornersRecursive( hypro::matrix_t<Number>( _remainingGenerators.block(
										 1, 0, _remainingGenerators.rows(), _remainingGenerators.cols() - 1 ) ),
								   pos );
		res2 = getCornersRecursive( hypro::matrix_t<Number>( _remainingGenerators.block(
										  1, 0, _remainingGenerators.rows(), _remainingGenerators.cols() - 1 ) ),
									neg );
