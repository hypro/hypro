...
struct InitialParser : public qi::grammar<Iterator, Initial<Number>(), Skipper> {
	...
	InitialParser() : InitialParser::base_type( start ) {
		start = qi::lit( "initial" ) > qi::lit( "(" ) > mLocationParser > qi::lit( "," ) >
				// mNumberParser >
				mFloatParser > qi::lit( ")" );
	}
...
struct StateParser : public qi::grammar<Iterator, State<Number>(), Skipper> {
	...
	StateParser() : StateParser::base_type( start ) {
		start = qi::lit( "location" ) > qi::lit( "(" ) > mNameParser > -( qi::lit( "," ) > mFlowParser ) >
				-( qi::lit( "," ) > mInvariantParser ) > qi::lit( ")" );
	}
...
struct MainParser : public qi::grammar<Iterator, Skipper> {
	...
	MainParser() : MainParser::base_type( main ) {
		...
		// main = mInitialParser[mInitial(spirit::_val)] > *(mStateParser[ boost::phoenix::push_back(mStates,
		// parser::State(spirit::_1)) ] | mTransitionParser[ boost::phoenix::push_back(mTransitions, spirit::_1) ]);
		main = mInitialParser[phoenix::bind( &hypro::parser::MainParser<Number, Representation>::setInitial,
											 phoenix::ref( *this ), qi::_1 )] >
			   *( mStateParser[phoenix::bind( &hypro::parser::MainParser<Number, Representation>::push_back_State,
											  phoenix::ref( *this ), qi::_1 )] |
	...
	void setInitial( const Initial<Number>& _in ) { mInitial = _in; }
