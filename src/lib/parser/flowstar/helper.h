...
static matrix_t<Number> createMatrix( const std::vector<std::vector<Number>>& _rawMatrix ) {
	assert(!_rawMatrix.empty() && !_rawMatrix.begin()->empty());
	matrix_t<Number> res = matrix_t<Number>(_rawMatrix.size(), _rawMatrix.begin()->size());
	unsigned rowIndex = 0;
	unsigned colIndex = 0;
	for(const auto& row : _rawMatrix) {
		for(const auto& entry : row) {
			res(rowIndex,colIndex) = entry;
			++colIndex;
		}
		++rowIndex;
	}
