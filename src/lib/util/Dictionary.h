...
public:
	 ...
	 */
	Dictionary(const matrix_t<Number>& A,
				const std::map<std::size_t, std::size_t>& B,
				const std::map<std::size_t, std::size_t>& N,
				std::size_t F,
				std::size_t G) :
	mDictionary(A),
	mF(F),
	 ...
	 * @return
	 */
	inline const matrix_t<Number>& tableau() const;
	 ...
	 * @return
	 */
	Point<Number> vertex(const matrix_t<Number>& substitution) const;
