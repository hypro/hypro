...
namespace hypro {
	...
	antlrcpp::Any CIFExprVisitor<Number>::visitCompareExpression(HybridSystemParser::CompareExpressionContext *ctx){
		...
		for (size_t i = 0; i < n-1; i++) {
			...
			if(actOperator=="<"||actOperator==">"||actOperator=="!="||actOperator=="in"||actOperator=="sub"){
			...
			//allowed operators
			if(actOperator=="<"){
				std::pair<vector_t<Number>,Number> res = std::make_pair(leftVector - rightVector, rightNumber - leftNumber);
				result.push_back(res);
			}
			if(actOperator==">"){
				std::pair<vector_t<Number>,Number> res = std::make_pair(rightVector - leftVector, leftNumber - rightNumber);
				result.push_back(res);
			}
			if(actOperator=="="){
				std::pair<vector_t<Number>,Number> res = std::make_pair(leftVector - rightVector, rightNumber - leftNumber);
	...
	template<typename Number>
	antlrcpp::Any CIFExprVisitor<Number>::visitEquationNonDerivate(HybridSystemParser::EquationNonDerivateContext *ctx){
