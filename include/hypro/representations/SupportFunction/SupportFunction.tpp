...
SupportFunctionT<Number, Converter, Setting> SupportFunctionT<Number, Converter, Setting>::intersectHalfspace( const Halfspace<Number>& hs ) const {
	...
	vec << hs.offset();
	return SupportFunctionT<Number, Converter, Setting>( content->intersect( SupportFunctionT<Number, Converter, Setting>( matrix_t<Number>( hs.normal().transpose() ), vec ).content ) );
}
