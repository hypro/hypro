...
template <typename Number>
Dictionary<Number>::Dictionary( const matrix_t<Number>& rhs, std::vector<Eigen::Index> basis, std::vector<Eigen::Index> cobasis )
	: mDictionary( rhs )
	, mB( basis )
	, mN( cobasis ) {}

template <typename Number>
Dictionary<Number>::Dictionary( const matrix_t<Number>& rhs, std::vector<Eigen::Index> basis, std::vector<Eigen::Index> cobasis, ConstrainSet<Number> constrains )
	: mDictionary( rhs )
	, mB( basis )
	, mN( cobasis )
...
bool Dictionary<Number>::selectBlandPivot( Eigen::Index& i, Eigen::Index& j ) const {
	...
	for ( unsigned candidateRowIndex = 1; candidateRowIndex < unsigned( goodIndices.size() ); ++candidateRowIndex ) {  // select the candidate row
		...
		if ( mDictionary( goodIndices[candidateRowIndex], mDictionary.cols() - 1 ) / mDictionary( goodIndices[candidateRowIndex], j ) == currentLambda && minIndex > mB[goodIndices[candidateRowIndex]] ) {
			i = goodIndices[candidateRowIndex];
			currentLambda = mDictionary( i, mDictionary.cols() - 1 ) / mDictionary( i, j );
			minIndex = mB[goodIndices[candidateRowIndex]];
		}
...
template <typename Number>
bool Dictionary<Number>::selectDualBlandPivot( Eigen::Index& i, Eigen::Index& j, const std::vector<Eigen::Index> availableIndices ) {
	...
	for ( unsigned colIndex = 1; colIndex < unsigned( goodIndices.size() ); ++colIndex ) {
		...
		if ( mDictionary( mDictionary.rows() - 1, goodIndices[colIndex] ) / mDictionary( availableIndices[i], goodIndices[colIndex] ) == currentLambda && minIndex > mN[goodIndices[colIndex]] ) {
			minIndex = mN[goodIndices[colIndex]];
			j = goodIndices[colIndex];
			currentLambda = mDictionary( mDictionary.rows() - 1, j ) / mDictionary( availableIndices[i], j );
		}
...
template <typename Number>
bool Dictionary<Number>::reverseDual_old( const Eigen::Index i, const Eigen::Index j, const std::vector<Eigen::Index> availableIndices ) {
	Eigen::Index a = Eigen::Index( 0 );
	Eigen::Index b = Eigen::Index( 0 );
	Eigen::Index& i3 = a;
	Eigen::Index& j3 = b;
...
template <typename Number>
bool Dictionary<Number>::reverseDual( const Eigen::Index i, const Eigen::Index j, const std::vector<Eigen::Index>& availableIndices ) {
	...
	for ( Eigen::Index colIndex = 0; colIndex < Eigen::Index( mDictionary.cols() - 1 ); ++colIndex ) {
		if ( mDictionary( availableIndices[i], colIndex ) > 0 && maxRatio < mDictionary( mDictionary.rows() - 1, colIndex ) / mDictionary( availableIndices[i], colIndex ) ) {
			return false;
		}
	}
	for ( Eigen::Index colIndex = 0; colIndex < Eigen::Index( mDictionary.cols() - 1 ); ++colIndex ) {
		if ( colIndex != j && mN[colIndex] < mB[i] && ( mDictionary( mDictionary.rows() - 1, colIndex ) == 0 && mDictionary( availableIndices[i], colIndex ) < 0 ) ) {
			return false;
		}
	}
	for ( Eigen::Index rowIndex = 0; rowIndex < Eigen::Index( availableIndices.size() ); ++rowIndex ) {
