...
void ConvexHull<Number>::conicHull() {
	if ( mCone.size() == 0 ) {
		vector_t<Number> h1 = vector_t<Number>( 2 );  // in case the function is called in the polyhedric hull, the absurd answer is in dimension 2, so a dimension can be removed
