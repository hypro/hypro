...
struct Node {
    ...
    Node(const MatrixT& m, const std::vector<ColIndex>& cols)
    : matrix(m)
    , cols_to_elim(cols) {}

    Node(const MatrixT& m, const std::vector<ColIndex>& cols, const std::set<RowIndex>& ign)
    : matrix(m)
    , cols_to_elim(cols)
    , ignored(ign) {}

    Node(MatrixT&& m, const std::vector<ColIndex>& cols)
    : matrix(std::move(m))
    , cols_to_elim(cols) {}
