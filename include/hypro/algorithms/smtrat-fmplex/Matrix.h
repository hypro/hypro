...
public:
    ...
    struct RowEntry {
        ColIndex col_index;
        Number value;
        RowEntry(ColIndex col, const Number& v) : col_index(col), value(v) {}
...
public:
    ...
    Number coeff(const RowIndex i, const ColIndex j) const {
    ...
    */
    std::vector<RowEntry> combine(const RowIndex row_idx_1, const Number& scale_1,
                                  const RowIndex row_idx_2, const Number& scale_2) const {
        ...
        while (idx_1 != end_1 && idx_2 != end_2) {
            if (m_data[idx_1].col_index < m_data[idx_2].col_index) {
                ...
                Number new_value = scale_1*m_data[idx_1].value + scale_2*m_data[idx_2].value;
...
public:
    ...
    struct col_iterator {
    private:
        typename Column::const_iterator m_it;
        const std::vector<RowEntry>& mr_data;
    public:
        col_iterator(typename Column::const_iterator it, const std::vector<RowEntry>& data)
...
template<typename Number, std::enable_if_t<std::is_same<Number, mpq_class>::value, bool> = true>
inline void gcd_normalize(std::vector<typename Matrix<Number>::RowEntry>& row) {
    Number gcd = row.front().value.get_num();
    Number lcm = row.front().value.get_den();
    for (const auto& e : row) {
        gcd = carl::gcd(gcd, e.value.get_num());
        lcm = carl::lcm(lcm, e.value.get_den());
    }
    Number scale = lcm/gcd;
    if (!carl::is_one(scale)) {
        for (auto& e : row) e.value *= scale;
    }
}

template<typename Number, std::enable_if_t<std::is_same<Number, double>::value, bool> = true>
inline void gcd_normalize(std::vector<typename Matrix<Number>::RowEntry>& row) {}
