...
template <typename Number>
ReachabilityNode<Number>* ReachabilityTree<Number>::computeReachTree( const Starset<Number>& inputSet, const HPolytope<Number>& safeSet, NN_REACH_METHOD method, SEARCH_STRATEGY strategy, int layerNumber, int neuronNumber ) {
	ReachabilityNode<Number>* rootNode = new ReachabilityNode<Number>(inputSet, method, layerNumber, neuronNumber);
	...
	while ( !jobQueue.empty() && mIsSafe ) {
		...
		for ( auto newJob : newJobs ) {
			if ( newJob.isFinalResult() ) {
				...
				if(!leafNode->isSafe(safeSet)) {
					leafNode->setIsSafe(false);
					mIsSafe = false;
				} else {
					leafNode->setIsSafe(true);
				}
			} else {
				switch ( strategy ) {
					case SEARCH_STRATEGY::BFS:
...
bool ReachabilityTree<Number>::verify( NN_REACH_METHOD method, SEARCH_STRATEGY strategy ) {
	...
	switch ( method ) {
		...
		case NN_REACH_METHOD::OVERAPPRX:
			...
			root = computeReachTree( starInput, safeOutput, method, strategy, 0, 0 );
		...
		case NN_REACH_METHOD::CEGAR:
			...
			root = computeReachTree( starInput, safeOutput, NN_REACH_METHOD::OVERAPPRX, strategy, 0, 0 );
			break;
		default:
	...
	// // create the root_job and add to the queue
	// SearchJob<Number> root_job( starInput, mNetwork.layers( 0 ), mNetwork.layers(), 0 );
	// std::deque<SearchJob<Number>> jobQueue;
	// jobQueue.push_back( root_job );
