...
%vague overview
Fukuda's algorithm allows to enumerate all the vertices of an $H$-polytope included in the positive orthant (all its points have non-negative coordinates) for which the origin is a vertex. This algorithm uses the pivoting scheme of the simplex to explore the polytope. 
...
\begin{example}
	Let $h$ be the polytope included in the positive orthant and the hyperplane $x+2y\leq 4$ (plus the positve orthant, i.e. $-x\leq 0$ and $-y\leq 0$) in $\mathbb{R}^2$.\\
