...
class REPRESENTATIONT : public GeometricObject<Number, REPRESENTATIONT<Number,Converter,Setting>> {
  ...
  public:
	...
	Setting getSettings() const { return Setting{}; }
	 ...
	 /**
	  * @brief Static method for the construction of an empty REPRESENTATION of required dimension.
	  * @param dimension Required dimension.
	  * @return Empty REPRESENTATION.
	  */
	static REPRESENTATIONT<Number,Converter,Setting> Empty(std::size_t dimension = 1) {
	...
	/**
	 * @brief Method returning the supremum point of the REPRESENTATION, i.e. the maximal point.
	...
	friend bool operator==( const REPRESENTATIONT<Number,Converter,Setting>& b1, const REPRESENTATIONT<Number,Converter,SettingRhs>& b2 ) {
		return false;
	}
	 ...
	 */
	friend bool operator!=( const REPRESENTATIONT<Number,Converter,Setting>& b1, const REPRESENTATIONT<Number,Converter,Setting>& b2 ) { return !( b1 == b2 ); }

	/**
	 * @brief Assignment operator.
	 * @param rhs A REPRESENTATION.
	 */
	REPRESENTATIONT<Number,Converter,Setting>& operator=( const REPRESENTATIONT<Number,Converter,Setting>& rhs ) = default;

	/**
	 * @brief Move assignment operator.
	 * @param rhs A REPRESENTATION.
	 */
	REPRESENTATIONT<Number,Converter,Setting>& operator=(REPRESENTATIONT<Number,Converter,Setting>&& rhs) = default;
	 ...
	 */
#ifdef HYPRO_LOGGING
	friend std::ostream& operator<<( std::ostream& ostr, const REPRESENTATIONT<Number,Converter,Setting>& b ) {
		// Put outstream operations here.
#else
	friend std::ostream& operator<<( std::ostream& ostr, const REPRESENTATIONT<Number,Converter,Setting>& ) {
#endif
		return ostr;
	}
	...
	static representation_name type() { return representation_name::REPRESENTATION_SHORT; }

	/**
	 * @brief      Function to reduce the number representation (over-approximate).
	 * @param[in]  limit      The limit
	 */
	const REPRESENTATIONT<Number,Converter,Setting>& reduceNumberRepresentation();


	std::pair<CONTAINMENT, REPRESENTATIONT> satisfiesHalfspace( const Halfspace<Number>& rhs ) const;
	std::pair<CONTAINMENT, REPRESENTATIONT> satisfiesHalfspaces( const matrix_t<Number>& _mat, const vector_t<Number>& _vec ) const;
	REPRESENTATIONT<Number,Converter,Setting> project(const std::vector<std::size_t>& dimensions) const;
	REPRESENTATIONT<Number,Converter,Setting> linearTransformation( const matrix_t<Number>& A ) const;
	REPRESENTATIONT<Number,Converter,Setting> affineTransformation( const matrix_t<Number>& A, const vector_t<Number>& b ) const;
	REPRESENTATIONT<Number,Converter,Setting> minkowskiSum( const REPRESENTATIONT<Number,Converter,Setting>& rhs ) const;
	 ...
	 */
	REPRESENTATIONT<Number,Converter,Setting> intersect( const REPRESENTATIONT<Number,Converter,Setting>& rhs ) const;

	REPRESENTATIONT<Number,Converter,Setting> intersectHalfspace( const Halfspace<Number>& hspace ) const;
	REPRESENTATIONT<Number,Converter,Setting> intersectHalfspaces( const matrix_t<Number>& _mat, const vector_t<Number>& _vec ) const;
	bool contains( const Point<Number>& point ) const;

	/**
	 * @brief      Containment check for a REPRESENTATION.
	 * @param[in]  REPRESENTATION   The REPRESENTATION.
	 * @return     True, if the given REPRESENTATION is contained in the current REPRESENTATION, false otherwise.
	 */
	bool contains( const REPRESENTATIONT<Number,Converter,Setting>& REPRESENTATION ) const;
	 ...
	 * @return     The resulting REPRESENTATION.
	 */
	REPRESENTATIONT<Number,Converter,Setting> unite( const REPRESENTATIONT<Number,Converter,Setting>& rhs ) const;

	/**
	 * @brief      Computes the union of the current REPRESENTATION with a set of REPRESENTATIONes.
	 * @param[in]  REPRESENTATIONes  The REPRESENTATIONes.
	 * @return     The resulting REPRESENTATION.
	 */
	static REPRESENTATIONT<Number,Converter,Setting> unite( const std::vector<REPRESENTATIONT<Number,Converter,Setting>>& REPRESENTATIONes );
