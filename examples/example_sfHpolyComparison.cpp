...
int main() {
    ...
    matrix_t<double> constraints = matrix_t<double>::Zero(2*dimension,dimension);
    ...
    //SupportFunctionT<double,Converter<double>,NoTrafoReduction> sf = SupportFunctionT<double,Converter<double>,NoTrafoReduction>(constraints, constants);
    ...
    //}
    matrix << cos(30*3.141592654/180),-sin(30*3.141592654/180),sin(30*3.141592654/180),cos(30*3.141592654/180);


    std::cout << "Trafo matrix: " << matrix << ", trafo vector " << vector << std::endl;
    ...
    std::vector<EvaluationResult<double>> evalResults = sf.multiEvaluate(evaldirections);

    {
	HPolytope<double> tmp;
	for(int i = 0; i < resolution; ++i) {
		tmp.insert(Halfspace<double>(evaldirections.row(i), evalResults[i].supportValue));
	}
