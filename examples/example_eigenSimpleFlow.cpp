...
void trajectory_plot(std::vector<VPoly> &exactflow, std::size_t DIM_PLOT,
                     bool ORIG_SYS_PLOT, const Ind_f &ind_f, const int n,
                     const Matrix<Number> &V, const std::size_t traj_scale,
                     Dep_f &in_traj);
void plot_addTimeSegment(Flow_seg &safe_seg, std::size_t DIM_PLOT,
                         bool ORIG_SYS_PLOT, const std::size_t traj_time,
                         const Matrix<Number> &x_tr, const Matrix<Number> &V,
                         const int n);
...
int main(int argc, char **argv) {
  ...
  Matrix<Number> V = Matrix<Number>(n,n); //backtransformation Matrix<Number>
  Vinv = Matrix<Number>(n,n);                          //dumped after use
  ...
  //###### INPUT SYSTEM  d/dx = A*x + b  ######
      in_eq1.A << 	0.001, 1,
                              0.001, -0.002;
  in_eq1.b <<    0, -9.81;
  in_eq1.x0<<    100, 0;        //we require in_eq1.x0 > .in_eq1.x0_2
  elementwise in_eq1.x0_2 << 10, 0;          //else we swap values
  ...
  Eigen::EigenSolver<Matrix<Number>> es(in_eq1.A);    //decompose matrix
  directly + constructor V << es.eigenvectors().real(); ind_f.D.diagonal() <<
  es.eigenvalues().real(); Vinv = V.inverse();
  //std::cout << "Eigenvectors(V): "<< std::endl << V << std::endl;
      //std::cout << "Vinverse: "<< std::endl << Vinv << std::endl;
      //std::cout << "Eigenvalues: "<< std::endl << ind_f.D.diagonal() <<
  std::endl;
  ...
  if (flag1.TRAJECTORY_PLOT) {
      trajectory_plot(exactflow, flag1.DIM_PLOT, flag1.ORIG_SYS_PLOT, ind_f, n,
  V, traj_scale, in_traj);
  }
  if (flag1.HULLCONSTR) {
      loop (safeflow, error_flow, flag1, ind_f, n, V, eval_f, dep_f, safe_seg,
  error_seg);
  }
...
void trajectory_plot(std::vector<VPoly> &exactflow, std::size_t DIM_PLOT,
  ...
  Vector<Number> factor = Vector<Number>::Zero(n);
  plot_addTimeSegment(traj_flow_seg, DIM_PLOT, ORIG_SYS_PLOT, 0, traj_tr.x_tr,
                      V, n);
  for (std::size_t traj_time = 1; traj_time <= traj_scale * ind_f.deltalimit;
       traj_time += 1) {
    for (int i = 0; i < n; ++i) {
      factor(i) = std::exp(ind_f.D.diagonal()(i) * traj_time * ind_f.delta /
                           traj_scale);
    }
    // std::cout<<"traj_time"<<traj_time<<" factor
    // "<<std::endl<<factor<<std::endl; depends what we want to plot here
    traj_tr.x_tr.col(0) =
        traj_tr.xhom.col(0).array() * factor.array() - ind_f.xinhom.array();
    traj_tr.x_tr.col(1) =
        traj_tr.xhom.col(1).array() * factor.array() - ind_f.xinhom.array();
    plot_addTimeSegment(traj_flow_seg, DIM_PLOT, ORIG_SYS_PLOT, traj_time,
                        traj_tr.x_tr, V, n);
    addSegment(exactflow, true, traj_flow_seg,
               plotting::colors[plotting::green],
               plotting::colors[plotting::green]);
  }
}
void plot_addTimeSegment(Flow_seg &flow_segment, std::size_t DIM_PLOT,
                         bool ORIG_SYS_PLOT, std::size_t traj_time,
                         const Matrix<Number> &x_tr, const Matrix<Number> &V,
                         const int n) {
  Vector<Number> plot_vector = Vector<Number>(n);
