...
int main(int argc, char **argv) {
  ...
  std::pair<hypro::HybridAutomaton<Number>, hypro::ReachabilitySettings>
      parsedInput = hypro::parseFlowstarFile<Number>(filename);
  settingsProvider.addStrategyElement<hypro::Box<Number>>(
  ...
  hypro::EventTimingProvider<Number>::getInstance().initialize(
      parsedInput.first,
      settingsProvider.getReachabilitySettings().timeBound *
          settingsProvider.getReachabilitySettings().jumpDepth);
  ...
  hypro::HybridAutomaton<Number>::locationStateMap initialStates =
      settingsProvider.getHybridAutomaton().getInitialStates();
  ...
  for (auto stateMapIt = initialStates.begin();
    ...
    if (settingsProvider.useDecider()) {
      if (settingsProvider.getLocationSubspaceTypeMap().size() > 0) {
        std::map<const hypro::Location<Number> *,
                 std::shared_ptr<std::vector<hypro::SUBSPACETYPE>>>::iterator
            it;
        it = settingsProvider.getLocationSubspaceTypeMap().find(
            stateMapIt->first);
        if (it != settingsProvider.getLocationSubspaceTypeMap().end()) {
          if (it->second->at(0) == hypro::SUBSPACETYPE::TIMED) {
            // timed subspace
            _temp = hypro::DifferenceBounds<Number>(
                stateMapIt->second.matrix(), stateMapIt->second.vector());
          } else {
            // standard subspace
            _temp = hypro::Box<Number>(stateMapIt->second.matrix(),
                                       stateMapIt->second.vector());
          }
        } else {
          // no entry in location subspace type map for this location -> compute
          // type
          ...
          /*
          repName =
          hypro::DecisionEntity<Number>::getInstance().getRepresentationForSubspace(*(stateMapIt->first),
          0); if(repName == hypro::representation_name::UNDEF) { _temp =
          hypro::Box<Number>(stateMapIt->second.matrix(),
          stateMapIt->second.vector());
          }
          */
        }
  ...
  hypro::ContextBasedReachabilityWorker<State> worker =
      hypro::ContextBasedReachabilityWorker<State>(parsedInput.second);
