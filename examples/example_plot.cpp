...
int main() {
	...
	// as we only plot in two dimensions, project on the dimension which will be plotted
	auto projectedBox = box.project({0,1});
	...
	// we may adjust the output file name (default out.XYZ), the file type is determined by the used plot function (see below)
	// the filename also determines the title of the plot
	plotter.setFilename("plotTest");

	// write plot files - gnuplot files with different output terminals are supported: pdf (plot2d), tex (plotTex), and eps (plotEps).
	// plain gnuplot files (*.gen) may also be created via plotGen().
	plotter.plot2d();
	plotter.plotTex();
	plotter.plotEps();
