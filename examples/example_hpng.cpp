...
int main() {
    ...
    std::pair<bool, HPolytope<Number>> intersectionPair = hpoly1.satisfiesHalfspaces(hspMat, hspVec);

    std::cout << "Intersection not empty: " << intersectionPair.first << std::endl;
    std::cout << "Result after intersection: " << intersectionPair.second << std::endl;
    ...
    HPolytope<Number> projected = intersectionPair.second.project(projectionVector);
