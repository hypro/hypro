...
int main() {
  ...
  // original segment (-1*pc1 <= 0 and -2*time + pc1 <= 0 and -2 + time <= 0 and
  // -7/2 + pc1 <= 0)
  ...
  if (newStatePair.first == hypro::CONTAINMENT::NO || boundary == 0) {
    ...
    std::cout << "carl polytope is " << carlPolytope << " and "
              << carlPolytope.empty() << std::endl;
    std::cout << "carl polytope matrix is " << carlPolytope.matrix()
              << std::endl;
    std::cout << "carl polytope vector is " << carlPolytope.vector()
              << std::endl;

    carlPolytope.make_rectangular();
    std::vector<carl::Interval<Number>> carlIntervalVector =
        carlPolytope.getIntervals();
