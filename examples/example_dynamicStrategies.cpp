...
int main() {
  ...
  // hypro::reachability::Reach<Number,boxValuation> boxReach(ha.first,
  // ha.second); hypro::reachability::Reach<Number,hpValuation>
  // hpReach(ha.first, ha.second);
  ...
  vector<std::pair<unsigned, hypro::reachability::flowpipe_t>> boxFlowpipes =
      boxReach.computeForwardReachability();
  vector<std::pair<unsigned, hypro::reachability::flowpipe_t>> hpFlowpipes =
      hpReach.computeForwardReachability();
  vector<std::pair<unsigned, hypro::reachability::flowpipe_t>> sfFlowpipes =
      sfReach.computeForwardReachability();

  // if(boxReach.reachedBadStates()) {
  // hypro::reachability::Reach<Number,sfValuation> sfReach(ha.first,
  // ha.second); sfFlowpipes = sfReach.computeForwardReachability();
  //}
  ...
  // bad states plotting
  hypro::HybridAutomaton<Number>::locationConditionMap badStateMapping =
      ha.first.getLocalBadStates();
  for (const auto &bsPair : badStateMapping) {
    // unsigned bs = plotter.addObject(hpValuation(bsPair.second.set.first,
    // bsPair.second.set.second).vertices());
    unsigned bs = plotter.addObject(
        hpValuation(bsPair.second.getMatrix(), bsPair.second.getVector())
            .vertices());
  ...
  for (const auto &flowpipe : hpFlowpipes) {
    for (const auto &segment : flowpipe.second) {
      unsigned tmp = plotter.addObject(segment.vertices());
      plotter.setObjectColor(tmp,
                             hypro::plotting::colors[hypro::plotting::lila]);
    }
  }

  std::cout
      << "Hpoly flowpipes plotted! \nPlotting SupportFunction flowpipes...\n";
