...
int main() {
	...
	State<Number, ConstraintSet<Number>, ConstraintSet<Number>> s3(l1, ConstraintSet<Number>(matr, vec), ConstraintSet<Number>(matr, vec));

	std::cout << "Equal: " << (s1.getLocation()->getId() == l1->getId()) << std::endl;
	std::cout << "Equal: " << (s2.getLocation()->getId() == l1->getId()) << std::endl;
	...
	s3.setSet<1>(setUpd);
	std::cout << "Equal: " << (s3.getSet<1>().matrix() == matr) << std::endl;
	std::cout << "Equal: " << (s3.getSet<1>().vector() == vec) << " (should be false) " << std::endl;
	std::cout << "Equal: " << (s3.getSet<1>().matrix() == matr2) << std::endl;
	std::cout << "Equal: " << (s3.getSet<1>().vector() == vec2) << std::endl;

	State<Number,ConstraintSet<Number>, ConstraintSet<Number>> s4(l1, ConstraintSet<Number>(matr, vec), ConstraintSet<Number>(matr, vec));

	State<Number,ConstraintSet<Number>,ConstraintSet<Number>> unionResult = s4.aggregate(s3);
